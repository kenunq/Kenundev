...
class Test_charpage(TestCase):
    ...
    def test_create_char(self):
        ...
        response = self.client.post(reverse('createchar', args=(self.room_id,)), self.create_char_data, 'application/json')
        ...
        response = self.client.post(reverse('char_page_room', args=(self.room_id,)), self.update_char_data,
                                    'application/json')
        ...
        self.assertEquals(False,
                          response.context_data['is_creator'])  # анонимный пользователь не является создателем комнаты
        self.assertNotIn('Добавить таланты',
                         response.content.decode())  # анонимный пользователь не может добавить таланты персонажу
        response = self.client.post(reverse('char_page_room', args=(self.room_id,)), self.update_char_data,
                                    'application/json')
        ...
        self.assertEquals(response.status_code, 200)  # страница с персонажем успешно отобразилась
        self.assertEquals(False,
                          response.context_data['is_creator'])  # другой пользователь не является создателем комнаты
        self.assertNotIn('Добавить таланты',
                         response.content.decode())  # другой пользователь не может добавить таланты персонажу
        response = self.client.post(reverse('char_page_room', args=(self.room_id,)), self.update_char_data,
                                    'application/json')
        self.assertNotIn('data was successfully saved', response.content.decode())  # конфигурация персонажа не
        # обновилась
        response = self.client.post(reverse('char_list'), self.delate_char_data,
                                    'application/json')
