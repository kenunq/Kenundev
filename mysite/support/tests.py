...
class Test_SupportPage(TestCase):
    ...
    def setUp(cls):
        ...
        cls.user2 = User.objects.create_user(
            cls.user2_username, cls.user2_email, cls.user2_password, is_superuser=True
        )

    def test_support_status_anonym(self):
        self.client.get(
            reverse("home")
        )  # устанавливаем куки для входа в комнату анонимным пользователем
        ...
        )  # анонимного пользователя не пустит на страницу чата админа
    ...
    def test_support_status_auth(self):
        ...
        self.assertEquals(
            response.status_code, 302
        )  # обычного пользователя не пустит на страницу чата админа
    ...
    def test_support_status_admin(self):
        ...
        self.assertEquals(
            response.status_code, 302
        )  # админа перенаправит на страницу чата админа
        ...
        )  # страница чата с пользователем успешно отобразилась
    ...
    def test_status_admin_chat_post(self):
        ...
        self.assertIn(
            'status changed successfully"', response.content.decode()
        )  # статус чата успешно поменялся
...
class ChatConsumerTest(ChannelsLiveServerTestCase):
    ...
    async def test_chat_consumer(self):
        ...
        communicator_user1 = WebsocketCommunicator(
            ChatConsumer.as_asgi(), f"ws/support/chat/{user1.id}/"
        )
        communicator_user2 = WebsocketCommunicator(
            ChatConsumer.as_asgi(), f"ws/support/chat/{user1.id}/"
        )
        communicator_admin = WebsocketCommunicator(
            AdminChatConsumer.as_asgi(), "ws/support/chat/admin/"
        )
        ...
        await communicator_user1.send_json_to(
            {"type": "message", "message": message_content, "user_id": str(user1.id)}
        )
    ...
    def create_user(self, username, password, admin=False):
        return User.objects.create_user(
            username=username, password=password, is_superuser=admin
        )
