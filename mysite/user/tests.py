...
class Test_UsersViews(TestCase):
    ...
    def setUp(cls):
        ...
        cls.user1 = User.objects.create_user(username=cls.login_form_data['username'],
    ...
    def test_profile_status(self):
        ...
        self.assertTrue(self.client.login(username=self.login_form_data['username'], password=self.login_form_data['password']))
        response = self.client.get(reverse('user:profile'))
        self.assertEquals(response.status_code, 200)

    def test_profile_form(self):
        self.assertTrue(
            self.client.login(username=self.login_form_data['username'], password=self.login_form_data['password']))
    ...
    def test_profile_view(self):
        self.assertTrue(
            self.client.login(username=self.login_form_data['username'], password=self.login_form_data['password']))
        response = self.client.post(reverse('user:profile'), {'update_email': 'testemail2@yandex.ru'}, 'application/json')
        self.assertEquals(response.status_code, 200)
        ...
        response = self.client.post(reverse('user:profile'), {'update_email': 'testemail1@yandex.ru'},
                                    'application/json')
        self.assertEquals(response.status_code, 200)
        ...
        response = self.client.post(reverse('user:profile'), {'update_discord': 'testdiscord'}, 'application/json')
