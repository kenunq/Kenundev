...
function TalentCalc() {
  var MODE_DEFAULT = 0,
    ...
    swfModel,
    _encoding =
      "0zMcmVokRsaqbdrfwihuGINALpTjnyxtgevElBCDFHJKOPQSUWXYZ123456789",
    _blizzEncoding =
      "aZbYcXdWeVfUgThSiRjQkPlOmNnMoLpKqJrIsHtGuFvEwDxCyBzA0123456789_=+-.",
    _newTree = "Z",
    _onChangeCallback,
    _info = {};
  ...
  this.getWhBuild = function () {
    ...
    if (_mode == MODE_PET) {
      res =
        _encoding.charAt(Math.floor(_currentClass / 10)) +
        _encoding.charAt(2 * (_currentClass % 10) + (_bonusPoints ? 1 : 0));
    } else {
  ...
  this.initialize = function (container, opt) {
    ...
    if (_opt.mode == MODE_PET) {
      ...
      _nTrees = 3;
      _nTiers = 11;
      _glyphLevels = { 1: [15, 30, 80], 2: [15, 50, 70] };
      _pointsPerTier = 5;
      _basePoints = 71;
      _pointsFromBonus = 0;
  ...
  this.promptBlizzBuild = function () {
    ...
    if (
      url.match(
        /talent-calculator#([\w=+-.])c[0-9]{2}(![\w=+-.]*![\w=+-.]*![\w=+-.]*!?[\w=+-.]*)/,
      )
    ) {
      classId = _blizzEncoding.indexOf(RegExp.$1);
      _setBlizzBuild(classId, RegExp.$2, !0);
      return;
    } else if (url.match(/\?cid=([0-9]+)&tal=([0-9]+)/)) {
  ...
  this.reset = function (tree) {
    ...
    _locked = !1;
    _resetTree(tree, _currentClass, !0);
  };
  ...
  this.setPetModel = function (npcId) {
    _updateModel(_currentClass, npcId);
  };
  this.showSummary = function (mode) {
    ...
    if (mode) {
      ...
      if (_mode == MODE_PET) {
        ...
        buffer +=
          $WH.sprintf(LANG.tc_printh, _getRequiredLevel(), g_chr_classes[c.n]) +
          " (" +
          c[0].k +
          "/" +
          c[1].k +
      ...
      if ($WowheadTalentCalculator !== undefined) {
        ...
        buffer +=
          "<blockquote style=\"font-family: 'Courier New';\">?cid=" +
          _currentClass +
          "&tal=" +
          $WowheadTalentCalculator.getBlizzBuild() +
          "</blockquote>";
        buffer += "<p></p>";
      }
      for (i = 0; i < _nTrees; ++i) {
        buffer +=
          ...
          c[i].k +
          " " +
          LANG[c[i].k == 1 ? "tc_point" : "tc_points"] +
          ")</h3>";
        buffer += "<blockquote>";
        k = 0;
        for (j = 0; j < c[i].t.length; ++j) {
          if (c[i].t[j].k) {
            ...
            buffer +=
              ...
              "</b>" +
              LANG.hyphen +
              $WH.sprintf(LANG.tc_rank, c[i].t[j].k, c[i].t[j].m) +
              "<br />";
  ...
  function _bonusPointsOnMouseOver(_this, e) {
    if (_mode == MODE_PET) {
      $WH.Tooltip.showAtCursor(
        e,
        LANG[_bonusPoints ? "tc_rembon" : "tc_addbon"],
        null,
        null,
        "q",
      );
  ...
  function _setLevelCapOnClick(_this, e) {
    _setLevelCap(
      prompt(
        $WH.sprintf(LANG.prompt_ratinglevel, MIN_LEVEL, MAX_LEVEL),
        _maxLevel,
      ),
    );
    ...
    for (var i = 0, len = g_pet_talents.length; i < len; ++i) {
      ...
      if ($WH.in_array(talents.f, classId) >= 0) {
        ...
        for (var j = 0, bf = talents.t.length; j < bf; ++j) {
          ...
          _ = data[0].t[j] = {};
          $WH.cO(_, talent);
          if (talent.f && $WH.in_array(talent.f, classId) == -1) _.hidden = !0;
        }
        break;
      }
    }
  ...
  function _createTrees(classId) {
    ...
    for (var tree = 0; tree < _nTrees; ++tree) {
      ...
      if (tree > 0) {
        d.style.borderLeft = "1px solid #404040";
      }
      ...
      if (_mode == MODE_PET) {
        d.style.backgroundImage =
          "url(" +
          "static" +
          "/images/wow/hunterpettalents/bg_" +
          (c[0].i + 1) +
          ".jpg" +
          afterUrl +
          ")";
        iconBg =
          "static" +
          "/images/wow/hunterpettalents/icons_" +
          (c[0].i + 1) +
          ".jpg" +
          afterUrl;
      } else {
        d.style.backgroundImage =
          "url(" +
          "static" +
          "/images/wow/talents/backgrounds/" +
      ...
      for (var i = c[tree].t.length - 1; i >= 0; --i) {
        ...
        if (talent.r) {
          var requiredTalent = c[tree].t[talent.r[0]],
    ...
    if (talent.k > 0) {
      ...
      for (var i = 0; i < c[talent.tree].t.length; ++i) {
        ...
        if (fooTalent.k && talent.y != fooTalent.y) {
          if (sum < fooTalent.y * _pointsPerTier) {
            talent.k++;
            return;
          }
        }
      ...
      if (refresh) {
        ...
        if (i >= _pointsToSpend) {
          for (var tree = 0; tree < _nTrees; ++tree) {
            _updateTree(tree, !0, null, talent.classId);
          }
        }
        _onChange();
      }
  ...
  function _isGlyphLocked(slot) {
    var notFound =
      $WH.in_array(_glyphSlots[_getGlyphTypeFromSlot(slot)], slot) == -1;
  ...
  function _glyphClick(slot, right) {
    if (!_locked) {
      if (right) {
        ...
        Lightbox.show("glyphpicker", { onShow: _glyphPickerOnShow });
      }
    }
  }
  ...
  function _iconClick(talent, right) {
    ...
    if (right) {
      _decrementTalent(talent, !0, this);
    } else {
      _incrementTalent(talent, !0, this);
    }
  }
  function _incrementTalent(talent, refresh, _this) {
    ...
    if (c.k < _pointsToSpend) {
      ...
      _setPoints(-1, _pointsFromBonus, !0);
      _incrementTalent(talent, refresh, _this);
    }
  }
  ...
  function _isValidGlyph(slot, glyph) {
    ...
    var valid =
      ...
      glyph.type ==
        _getGlyphTypeFromSlot(slot != null ? slot : _currentGlyphSlot) &&
  ...
  function _createGlyphPickerNote(div) {
    ...
    if (_filteredGlyphs) {
      div.innerHTML +=
        '<small><b class="q10">' + LANG.tc_glyphnote + "</b></small>";
    }
  }
  function _onChange() {
    var c = _data[_currentClass];
    ...
    $WH.st(
      _lblSpec,
      "(" + (_mode == MODE_PET ? c.k : _info.pointsSpent.join("/")) + ")",
    );
    ...
    if (_lnkExport) {
      _lnkExport.href =
        (_mode == MODE_PET ? "?petcalc#" : "?talent#") +
        _self.getWhBuild() +
        (_mode == MODE_PET ? "" : ":" + _self.getWhGlyphs());
    }
    for (var tree = 0; tree < _nTrees; ++tree) {
    ...
    for (var tree = 0; tree < _nTrees; ++tree) {
      ...
      if (_mode == MODE_DEFAULT) {
        span.style.backgroundImage =
          "url(" +
          "static" +
          "/images/wow/talents/trees/" +
          g_file_classes[_currentClass] +
          "_" +
          (tree + 1) +
          ".gif)";
      }
  ...
  function _parseBlizzGlyphs(glyphs) {
    var glyphIds = ("" + glyphs).split(":", _nGlyphs),
      nextOfType = {};
  ...
  function _parseWhGlyphs(glyphs) {
    ...
    for (var i = 0, len = glyphs.length; i < len && i < _nGlyphs; ++i) {
      ...
      _addGlyph(
        slot,
        _glyphLookup[_currentClass][_getGlyphTypeFromSlot(slot)][
          _encoding.indexOf(c)
        ],
        !0,
      );
  ...
  function _refreshAll(classId) {
    _refreshGlyphs();
    for (var tree = 0; tree < _nTrees; ++tree) {
      _updateTree(tree, !0, null, classId);
    }
  }
  function _refreshGlyphs() {
    ...
    _glyphDiv.style.display =
      nGlyphs == 0 && _locked && _opt.profiler ? "none" : "";
  }
  function _registerClass(classId, data) {
    if (_data[classId] == null) {
      ...
      if (_build && _build.classId == classId) {
        for (var tree = 0; tree < _nTrees; ++tree) {
          _updateTree(tree, !1, null, classId);
        }
  ...
  function _showGlyphTooltip(_this, slot) {
    ...
    if (glyph && !locked) {
      ...
      lower +=
        '<span class="q10">' +
        $WH.sprintf(LANG.tc_lockgly, _getGlyphSlotLevel(slot)) +
        "</span>";
    }
  ...
  function _showTooltip(_this, talent) {
    ...
    if (talent.r) {
      if (c[talent.tree].t[talent.r[0]].k < talent.r[1]) {
        ...
        buffer += $WH.sprintf(
          LANG[talent.r[1] == 1 ? "tc_prereq" : "tc_prereqpl"],
          talent.r[1],
          c[talent.tree].t[talent.r[0]].n,
        );
        buffer += "</span><br />";
      }
    }
    if (c[talent.tree].k < talent.y * _pointsPerTier) {
      buffer +=
        '<span class="q10">' +
        $WH.sprintf(LANG.tc_tier, talent.y * _pointsPerTier, c[talent.tree].n) +
        "</span><br />";
    }
    ...
    buffer +=
      '<span class="q">' + _getTalentDescription(talent) + "</span><br />";
    if (_locked) {
  ...
  function _updateTree(tree, refresh, talent, classId) {
    ...
    if (!talent || c.k == _pointsToSpend) {
      ...
      bc = talent.i;
      highestLevel =
        Math.floor(c[tree].k / _pointsPerTier) * _pointsPerTier + 5;
    }
  ...
  function _updateModel(classId, npcId) {
    ...
    var attributes = {};
    _swfModel.parentNode.setAttribute(
      "style",
      "background-image:url(" +
        swfUrl +
        "/images/wow/hunterpettalents/lg_" +
        c.n +
        ".jpg);",
    );
  }
  ...
  columns: [
    {
      ...
      compute: function (glyph, td, tr) {
        ...
        var icon = Icon.create(
            ...
            0,
            null,
            "https://www.wowhead.com/wotlk/ru/item=" + glyph.id,
          ),
          link = Icon.getLink(icon);
        $WH.ae(i, icon);
        $WH.ae(tr, i);
        ...
        a.style.color = "#FFD100";
        a.href = "https://www.wowhead.com/wotlk/ru/item=" + glyph.id;
        $WH.ae(
          a,
          $WH.ct($WowheadTalentCalculator.simplifyGlyphName(glyph.name)),
        );
