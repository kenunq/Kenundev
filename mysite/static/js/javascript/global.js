...
var User = new function () {
        var self = this; self.hasPermissions = function (roles) {
                if (!roles) { return !0 }
...
}; var DomContentLoaded = new function () { var _now = []; var _del = []; this.now = function () { $WH.array_apply(_now, function (f) { f() }) }; this.delayed = function () { $WH.array_apply(_del, function (f) { f() }); DomContentLoaded = null }; this.addEvent = function (f) { _now.push(f) }; this.addDelayedEvent = function (f) { _del.push(f) } }; function g_addCss(css) {
        var style = $WH.ce('style'); style.type = 'text/css'; if (style.styleSheet) { style.styleSheet.cssText = css } else { $WH.ae(style, $WH.ct(css)) }
...
function g_getFirstTextContent(node) {
        for (var i = 0; i < node.childNodes.length; ++i) {
                if (node.childNodes[i].nodeName == '#text') { return node.childNodes[i].nodeValue }
                var ret = g_getFirstTextContent(node.childNodes[i]); if (ret) { return ret }
        }
        return !1
}
function g_getTextContent(el) {
        var txt = ''; for (var i = 0; i < el.childNodes.length; ++i) {
                if (el.childNodes[i].nodeValue) { txt += el.childNodes[i].nodeValue } else if (el.childNodes[i].nodeName == 'BR') { if ($WH.Browser.ie67) { txt += '\r' } else { txt += '\n' } }
                txt += g_getTextContent(el.childNodes[i])
        }
        return txt
}
function g_toggleDisplay(el) {
        el = $(el); el.toggle(); if (el.is(':visible')) { return !0 }
        return !1
}
...
g_enableScroll.F = function (e) {
        ...
        e.returnValue = !1; e.cancelBubble = !0; return !1
...
function g_insertTag(where, tagOpen, tagClose, repFunc) {
        var n = $WH.ge(where); n.focus(); if (n.selectionStart != null) {
                var
                ...
                n.value = n.value.substr(0, s) + tagOpen + selectedText + tagClose + n.value.substr(e); n.selectionStart = n.selectionEnd = e + tagOpen.length; n.scrollLeft = sL; n.scrollTop = sT
        } else if (document.selection && document.selection.createRange) {
                var range = document.selection.createRange(); if (range.parentElement() != n) { return }
...
function g_onAfterTyping(input, func, delay) {
        var timerId; var ldsgksdgnlk623 = function () {
                if (timerId) { clearTimeout(timerId); timerId = null }
                timerId = setTimeout(func, delay)
        }; input.onkeyup = ldsgksdgnlk623
}
function g_onClick(el, func) {
        var firstEvent = 0; function rightClk(n) {
                if (firstEvent) { if (firstEvent != n) { return } } else { firstEvent = n }
                func(!0)
        }
        el.onclick = function (e) {
                e = $WH.$E(e); if (e._button == 2) { return !0 }
                return !1
        }
        el.oncontextmenu = function () { rightClk(1); return !1 }; el.onmouseup = function (e) {
                e = $WH.$E(e); if (e._button == 3 || e.shiftKey || e.ctrlKey) { rightClk(2) } else if (e._button == 1) { func(!1) }
                return !1
        }
}
...
var PageTemplate = new function () {
        var self = this; self.init = function () { initFavorites(); initUserMenu(); initFeedbackLink(); initLanguageMenu(); initFilterDisclosure(); initFloatingStuff(); initTopTabs(); initTopBar(); initBreadcrumb(); inited = !0 }
        self.get = function (name) { return opt[name] }
        self.set = function (options) {
                if (!options)
                        return; var old = {}; $.extend(old, opt); $.extend(opt, options); opt.activeTab = parseInt(opt.activeTab); if (inited) {
                                if (opt.activeTab != old.activeTab) { updateTopTabs(); updateTopBar() }
        ...
        function initFavorites() {
                var favMenu = $('#toplinks-favorites > a'); favMenu.text(LANG.favorites); if (!Favorites.hasFavorites())
                        favMenu.parent().hide(); Favorites.refreshMenu()
        }
        function initUserMenu() {
                var $link = $('#toplinks-user'); if (!$link.length)
        ...
        function generateCharactersSubmenu(menuItem) {
                var submenu = []; g_user.characters.sort(function (a, b) { return $WH.strcmp(a.realmname, b.realmname) || $WH.strcmp(a.region, b.region) || $WH.strcmp(a.name, b.name) }); var lastKey; $.each(g_user.characters, function (idx, character) {
                        ...
                        var menuItem = [character.id, character.name, g_getProfileUrl(character), null, { className: (character.pinned ? 'icon-star-right ' : '') + 'c' + character.classs, tinyIcon: $WH.g_getProfileIcon(character.race, character.classs, character.gender, character.level, 0, 'tiny') }]; submenu.push(menuItem)
        ...
        function initLanguageMenu() {
                var linkBefore = 'http://'; var linkAfter = location.pathname + location.search + location.hash; var localeId = Locale.getId(); var menu = []; var currentLocale; $.each(Locale.getAllByName(), function (idx, locale) {
                        var menuItem = [locale.id, locale.description, g_host + '/?locale=' + locale.id, null, null]; if (typeof g_pageInfo != 'undefined' && g_pageInfo.type && g_types[g_pageInfo.type])
                        ...
                        menu.push(menuItem)
                }); setLanguageMenu($('#toplinks-language'), menu, currentLocale); $(document).ready(function () { setLanguageMenu($('#footer-links-language'), menu, currentLocale) })
        }
        ...
        function initTopTabs() {
                var $topTabs = $('#toptabs'); if (!$topTabs.length)
        ...
        function initTopBar() {
                var $topBar = $('#topbar'); if (!$topBar.length)
        ...
        function updateTopTabs() {
                if (!$tabs)
                        return; var $as = $('a', $tabs); $.each(mn_path, function (idx, menuItem) {
                                var $a = $($as.get(idx)); var isActiveTab = (menuItem[MENU_IDX_ID] == opt.activeTab); if (isActiveTab) { $a.addClass('active'); Menu.remove($a) } else {
                                        $a.removeClass('active'); if (menuItem[MENU_IDX_SUB])
                                                Menu.add($a, menuItem[MENU_IDX_SUB])
                                }
                        })
        }
        function updateTopBar() {
                var $buttons = $('#topbar div.topbar-buttons'); if (!$buttons.length)
                        return; $buttons.empty(); switch (opt.activeTab) {
                                case 0: Menu.addButtons($buttons, [[0, LANG.menu_browse, null, mn_database], Menu.findItem(mn_database, [0]), Menu.findItem(mn_database, [2]), Menu.findItem(mn_database, [4]), Menu.findItem(mn_database, [3]), Menu.findItem(mn_database, [1]), Menu.findItem(mn_database, [9]), Menu.findItem(mn_database, [7]),]); break; case 1: var calculators = [[, LANG.calculators], Menu.findItem(mn_tools, [0]), Menu.findItem(mn_tools, [2]), Menu.findItem(mn_tools, [3])]; if (mn_tools[2][0] == 10) { var tmp = calculators[3]; calculators[3] = Menu.findItem(mn_tools, [10]); calculators[4] = tmp; Menu.addButtons($buttons, Menu.implode(calculators)); Menu.addButtons($buttons, Menu.implode(mn_tools.slice(4))) } else { Menu.addButtons($buttons, Menu.implode(calculators)); Menu.addButtons($buttons, Menu.implode(mn_tools.slice(3))) }
        ...
        function updateBreadcrumb() {
                if (!opt.breadcrumb || !opt.breadcrumb.length) { $bread.hide(); return }
                ...
                var path = Menu.getFullPath(mn_path, opt.breadcrumb); if (!path.length)
                        return; var lastIdx = (path.length - 1); $.each(path, function (idx, menuItem) {
                                var menuOpt = Menu.getItemOpt(menuItem); menuItem.checked = !0; checkedMenuItems.push(menuItem); Menu.updateItem(menuItem); var $span = expandBreadcrumb(); var $textHolder = $span; if (menuItem[MENU_IDX_URL]) { $textHolder = $('<a/>', { href: Menu.getItemUrl(menuItem) }).appendTo($span) }
                        ...
                        }); $bread.trigger('update'); $bread.show()
        }
        ...
        function expandButtonClick() {
                $('#sidebar, #header-expandsite').remove(); if ($('#layout').hasClass('nosidebar'))
...
function g_addTooltip(element, text, className) {
        if (!className && text.indexOf('<table>') == -1) { className = 'q' }
...
function g_formatTimeElapsed(delay) {
        function OMG(value, unit, abbrv) {
                if (abbrv && LANG.timeunitsab[unit] == '') { abbrv = 0 }
                if (abbrv) { return value + ' ' + LANG.timeunitsab[unit] } else { return value + ' ' + (value == 1 ? LANG.timeunitssg[unit] : LANG.timeunitspl[unit]) }
        }
        var range = [31557600, 2629800, 604800, 86400, 3600, 60, 1], subunit = [1, 3, 3, -1, 5, -1, -1]; delay = Math.max(delay, 1); for (var i = 3, len = range.length; i < len; ++i) {
                if (delay >= range[i]) {
                        var i1 = i; var v1 = Math.floor(delay / range[i1]); if (subunit[i1] != -1) { var i2 = subunit[i1]; delay %= range[i1]; var v2 = Math.floor(delay / range[i2]); if (v2 > 0) { return OMG(v1, i1, 1) + ' ' + OMG(v2, i2, 1) } }
                        return OMG(v1, i1, 0)
                }
        }
        return '(n/a)'
...
function g_GetStaffColorFromRoles(roles) {
        if (roles & U_GROUP_ADMIN) { return 'comment-blue' }
...
function g_formatDate(sp, elapsed, theDate, time, alone) {
        var today = new Date(); var event_day = new Date(); event_day.setTime(today.getTime() - (1000 * elapsed)); var txt; var event_day_midnight = new Date(event_day.getYear(), event_day.getMonth(), event_day.getDate()); var today_midnight = new Date(today.getYear(), today.getMonth(), today.getDate()); var delta = (today_midnight.getTime() - event_day_midnight.getTime()); delta /= 1000; delta /= 86400; delta = Math.round(delta); if (elapsed >= 2592000) { txt = LANG.date_on + g_formatDateSimple(theDate, time) } else if (delta > 1) { txt = $WH.sprintf(LANG.ddaysago, delta); if (sp) { var _ = new Date(); _.setTime(theDate.getTime() + (g_localTime - g_serverTime)); sp.className += ' tip'; sp.title = _.toLocaleString() } } else if (elapsed >= 43200) {
                if (today.getDay() == event_day.getDay()) { txt = LANG.today } else { txt = LANG.yesterday }
                txt = g_formatTimeSimple(event_day, txt); if (sp) { var _ = new Date(); _.setTime(theDate.getTime() + (g_localTime - g_serverTime)); sp.className += ' tip'; sp.title = _.toLocaleString() }
        } else { var txt = $WH.sprintf(LANG.date_ago, g_formatTimeElapsed(elapsed)); if (sp) { var _ = new Date(); _.setTime(theDate.getTime() + (g_localTime - g_serverTime)); sp.className += ' tip'; sp.title = _.toLocaleString() } }
...
function g_formatDateSimple(d, time) {
        function __twoDigits(n) { return (n < 10 ? '0' + n : n) }
...
function g_createGlow(txt, cn) {
        var s = $WH.ce('span'); for (var i = -1; i <= 1; ++i) {
                for (var j = -1; j <= 1; ++j) {
                        var d = $WH.ce('div'); d.style.position = 'absolute'; d.style.whiteSpace = 'nowrap'; d.style.left = i + 'px'; d.style.top = j + 'px'; if (i == 0 && j == 0) { d.style.zIndex = 4 } else { d.style.color = 'black'; d.style.zIndex = 2 }
...
function g_createProgressBar(opt) {
        ...
        el.className = 'progressbar'; if (opt.text || opt.hoverText) {
                div = $WH.ce('div'); div.className = 'progressbar-text'; if (opt.text) { var del = $WH.ce('del'); $WH.ae(del, $WH.ct(opt.text)); $WH.ae(div, del) }
...
function g_createReputationBar(totalRep) {
        var P = g_createReputationBar.P; if (!totalRep) { totalRep = 0 }
        totalRep += 42000; if (totalRep < 0) { totalRep = 0 } else if (totalRep > 84999) { totalRep = 84999 }
        var currentRep = totalRep, maxRep, standing = 0; for (var i = 0, len = P.length; i < len; ++i) {
                if (P[i] > currentRep) { break }
...
g_createReputationBar.P = [36000, 3000, 3000, 3000, 6000, 12000, 21000, 999]; function g_createAchievementBar(points, outOf, overall, bonus) {
        if (!points) { points = 0 }
...
function g_GetExpansionClassName(expansion) {
        switch (expansion) { case 0: return null; case 1: return "icon-bc-right"; case 2: return "icon-wotlk-right" }
        return null
}
...
function g_setCheckedRow(n, group) {
        if (!g_setCheckedRow.groups) { g_setCheckedRow.groups = {} }
...
function g_addPages(d, opt) {
        function createPageSquare(n, customText) {
                var foo; if (n == opt.page) { foo = $WH.ce('span'); foo.className = 'selected' } else { foo = $WH.ce('a'); foo.href = (n > 1 ? opt.url + opt.sep + n + opt.pound : opt.url + opt.pound) }
        ...
        if (opt.nPages > 1) {
                pagesNumbers = $WH.ce('div'); pagesNumbers.className = 'pages-numbers'; var minPage = Math.max(2, opt.page - 2); var maxPage = Math.min(opt.nPages - 1, opt.page + 2); var elements = []; if (opt.page != opt.nPages) { elements.push(createPageSquare(opt.page + 1, LANG.lvpage_next + String.fromCharCode(8250))) }
        ...
        var para = $WH.ce('var'); $WH.ae(para, $WH.ct($WH.sprintf(LANG[opt.wording[opt.nItems == 1 ? 0 : 1]], opt.nItems))); if (opt.nPages > 1) {
                var sp = $WH.ce('span'); $WH.ae(sp, $WH.ct(String.fromCharCode(8211))); $WH.ae(para, sp); var pageIcon = $WH.ce('a'); pageIcon.className = 'gotopage'; pageIcon.href = 'javascript:;'; $WH.ns(pageIcon); if ($WH.Browser.ie) { $WH.ae(pageIcon, $WH.ct(' ')) }
        ...
        if (leftAligned) { $WH.ae(divPages, para); if (pagesNumbers) { $WH.ae(divPages, pagesNumbers) } } else {
                if (pagesNumbers) { $WH.ae(divPages, pagesNumbers) }
                $WH.ae(divPages, para)
        }
        $WH.ae(d, divPages)
}
...
function g_setupChangeWarning(form, elements, warningMessage) {
        if ($WH.Browser.ie) { return }
        if (!form) { return }
        function ShowWarning() { return warningMessage }
        form.submit(function () { window.onbeforeunload = null }); var initialText = []; for (var idx in elements) {
                var text = elements[idx]; if (!text) { continue }
                initialText[idx] = text.val(); text.keydown(function () {
...
function SetupAddEditComment(post, comment, edit) {
        ...
        function Submit() {
                if (!Active || Submitting)
                        return; if (Body.val().length < MIN_LENGTH || Body.val().length > MAX_LENGTH) {
                                if (!Flashing) { Flashing = !0; TextCounter.animate({ opacity: '0.0' }, 150); TextCounter.animate({ opacity: '1.0' }, 150, null, function () { Flashing = !1 }) }
                                return !1
                        }
        ...
        function UpdateTextCounter() {
                var text = '(error)'; var cssClass = 'q0'; var chars = Body.val().replace(/(\s+)/g, ' ').replace(/^\s*/, '').replace(/\s*$/, '').length; var charsLeft = MAX_LENGTH - chars; if (chars == 0)
                        text = $WH.sprintf(LANG.replylength1_format, MIN_LENGTH); else if (chars < MIN_LENGTH)
                        text = $WH.sprintf(LANG.replylength2_format, MIN_LENGTH - chars); else {
                                text = $WH.sprintf(charsLeft == 1 ? LANG.replylength4_format : LANG.replylength3_format, charsLeft); if (charsLeft < 120)
...
function SetupShowMoreComments(post, comment) {
        var ShowMoreCommentsLink = post.find('.show-more-replies'); var CommentCell = post.find('.comment-replies'); ShowMoreCommentsLink.click(function () { ShowMoreComments() }); function ShowMoreComments() { ShowMoreCommentsLink.hide(); CommentCell.append(CreateAjaxLoader()); $.ajax({ type: 'GET', url: '?comment=show-replies', data: { id: comment.id }, success: function (replies) { comment.replies = replies; Listview.templates.comment.updateReplies(comment) }, dataType: 'json', error: function () { OnFetchFail() } }) }
...
function SetupRepliesControls(post, comment) {
        var CommentId = post.attr('data-replyid'); var VoteUpControl = post.find('.reply-upvote'); var VoteDownControl = post.find('.reply-downvote'); var FlagControl = post.find('.reply-report'); var CommentScoreText = post.find('.reply-rating'); var CommentActions = post.find('.reply-controls'); var DetachButton = post.find('.reply-detach'); var DeleteButton = post.find('.reply-delete'); var EditButton = post.find('.reply-edit'); var Container = comment.repliesCell; var Voting = !1; var Detaching = !1; var Deleting = !1; EditButton.click(function () { SetupAddEditComment(post, comment, !0) }); FlagControl.click(function () {
                if (Voting || !confirm(LANG.replyreportwarning_tip))
                        return; Voting = !0; $.ajax({ type: 'POST', url: '?comment=flag-reply', data: { id: CommentId }, success: function () { OnFlagSuccessful() }, error: function (jqXHR) { OnError(jqXHR.responseText) } })
        }); VoteUpControl.click(function () {
                if (VoteUpControl.attr('data-hasvoted') == 'true' || VoteUpControl.attr('data-canvote') != 'true' || Voting)
                        return; Voting = !0; $.ajax({ type: 'POST', url: '?comment=upvote-reply', data: { id: CommentId }, success: function () { OnVoteSuccessful(1) }, error: function (jqXHR) { OnError(jqXHR.responseText) } })
        }); VoteDownControl.click(function () {
                if (VoteDownControl.attr('data-hasvoted') == 'true' || VoteDownControl.attr('data-canvote') != 'true' || Voting)
                        return; Voting = !0; $.ajax({ type: 'POST', url: '?comment=downvote-reply', data: { id: CommentId }, success: function () { OnVoteSuccessful(-1) }, error: function (jqXHR) { OnError(jqXHR.responseText) } })
        }); DetachButton.click(function () {
                if (Detaching) { MessageBox(CommentActions, LANG.message_cantdetachcomment); return }
                if (!confirm(LANG.confirm_detachcomment)) { return }
                Detaching = !0; $.ajax({ type: 'POST', url: '?comment=detach-reply', data: { id: CommentId }, success: function () { OnDetachSuccessful() }, error: function (jqXHR) { OnError(jqXHR.responseText) } })
        }); DeleteButton.click(function () {
                if (Deleting)
                        return; if (!confirm(LANG.deletereplyconfirmation_tip))
                        return; Deleting = !0; $.ajax({ type: 'POST', url: '?comment=delete-reply', data: { id: CommentId }, success: function () { OnDeleteSuccessful() }, error: function (jqXHR) { OnError(jqXHR.responseText) } })
        }); function OnVoteSuccessful(ratingChange) {
                var rating = parseInt(CommentScoreText.text()); rating += ratingChange; CommentScoreText.text(rating); if (ratingChange > 0)
        ...
        function OnError(text) {
                Voting = !1; Detaching = !1; Deleting = !1; if (!text)
...
var g_screenshots = {}; var ScreenshotViewer = new function () {
        ...
        function render(resizing) {
                if (resizing && (scale == desiredScale) && $WH.g_getWindowSize().h > container.offsetHeight) { return }
                ...
                html += '>'; imgDiv.innerHTML = html; if (!resizing) {
                        g_trackEvent('Screenshots', 'Show', screenshot.id + ((screenshot.caption && screenshot.caption.length) ? ' (' + screenshot.caption + ')' : '')); if (screenshot.url) { aOriginal.href = url } else { aOriginal.href = g_staticUrl + '/uploads/screenshots/normal/' + screenshot.id + '.jpg' }
                        if (!screenshot.user && typeof g_pageInfo == 'object') { screenshot.user = g_pageInfo.username }
                        var hasFrom1 = (screenshot.date && screenshot.user), hasFrom2 = (screenshots.length > 1); if (hasFrom1) {
                                var
                                postedOn = new Date(screenshot.date), elapsed = (g_serverTime - postedOn) / 1000; var a = divFrom.firstChild.childNodes[1]; a.href = '?user=' + screenshot.user; a.innerHTML = screenshot.user; var s = divFrom.firstChild.childNodes[3]; $WH.ee(s); g_formatDate(s, elapsed, postedOn); divFrom.firstChild.style.display = ''
                        } else { divFrom.firstChild.style.display = 'none' }
                        var s = divFrom.childNodes[1]; $WH.ee(s); s = divFrom.childNodes[2]; if (hasFrom2) {
                                var buff = ''; if (screenshot.user) { buff = LANG.dash }
                                buff += (pos + 1) + LANG.lvpage_of + screenshots.length; s.innerHTML = buff; s.style.display = ''
                        } else { s.style.display = 'none' }
                        divFrom.style.display = (hasFrom1 || hasFrom2 ? '' : 'none'); var hasCaption = (screenshot.caption != null && screenshot.caption.length); var hasSubject = (screenshot.subject != null && screenshot.subject.length && screenshot.type && screenshot.typeId); if (hasCaption || hasSubject) {
                                var html = ''; if (hasSubject) { html += LANG.types[screenshot.type][0] + LANG.colon; html += '<a href="?' + g_types[screenshot.type] + '=' + screenshot.typeId + '">'; html += screenshot.subject; html += '</a>' }
                                if (hasCaption) {
                                        if (hasSubject) { html += LANG.dash }
                                        html += (screenshot.noMarkup ? screenshot.caption : Markup.toHtml(screenshot.caption, { mode: Markup.MODE_SIGNATURE }))
                                }
                                divCaption.innerHTML = html; divCaption.style.display = ''
                        } else { divCaption.style.display = 'none' }
        ...
        function peekPos(change) {
                var foo = pos; foo += change; if (foo < 0) { foo = screenshots.length - 1 } else if (foo >= screenshots.length) { foo = 0 }
                return foo
        }
        ...
        this.checkPound = function () {
                if (location.hash && location.hash.indexOf('#screenshots') == 0) {
                        if (!g_listviews.screenshots) {
                                var parts = location.hash.split(':'); if (parts.length == 3) {
                                        var
...
}; var vi_thumbnails = { 1: 'https://i3.ytimg.com/vi/$1/default.jpg' }; var vi_siteurls = { 1: 'https://www.youtube.com/watch?v=$1' }; var vi_sitevalidation = { 1: /^https?:\/\/www\.youtube\.com\/watch\?v=([^& ]{11})/ }; function vi_submitAVideo() { tabsContribute.focus(2) }
function vi_validateForm(f) {
        if (!f.elements.videourl.value.length) { alert(LANG.message_novideo); return !1 }
...
function vi_appendSticky() {
        var _ = $WH.ge('infobox-sticky-vi'); var type = g_pageInfo.type; var typeId = g_pageInfo.typeId; var pos = $WH.in_array(lv_videos, 1, function (x) { return x.sticky }); if (pos != -1) {
                var video = lv_videos[pos]; var a = $WH.ce('a'); a.href = '#videos:id=' + video.id; a.onclick = function (e) { VideoViewer.show({ videos: lv_videos, pos: pos }); return $WH.rf2(e) }; var img = $WH.ce('img'); img.src = $WH.sprintf(vi_thumbnails[video.videoType], video.videoId); img.className = 'border'; $WH.ae(a, img); $WH.ae(_, a); var th = $WH.ge('infobox-videos'); var a = $WH.ce('a'); if (!th) { var sections = $('th', _.parentNode); th = sections[sections.length - (lv_videos && lv_videos.length ? 2 : 1)] }
                $WH.ae(a, $WH.ct(th.textContent + ' (' + lv_videos.length + ')')); a.href = '#videos'
                a.title = $WH.sprintf(LANG.infobox_showall, lv_videos.length); a.onclick = function () { tabsRelated.focus(-1); return !1 }; $WH.ee(th); $WH.ae(th, a)
        } else {
                var a; if (g_user.id > 0) { a = '<a href="javascript:;" onclick="vi_submitAVideo(); return false">' } else { a = '<a href="?account=signin">' }
                _.innerHTML = $WH.sprintf(LANG.infobox_noneyet, a + LANG.infobox_suggestone + '</a>')
        }
}
var g_videos = []; var VideoViewer = new function () {
        var
        ...
        function render(resizing) {
                if (resizing && (scale == 1) && $WH.g_getWindowSize().h > container.offsetHeight) { return }
                container.style.visibility = 'hidden'; var video = videos[pos]; computeDimensions(); if (!resizing) {
                        g_trackEvent('Videos', 'Show', video.id + (video.caption.length ? ' (' + video.caption + ')' : '')); if (video.videoType == 1) { imgDiv.innerHTML = Markup.toHtml('[youtube=' + video.videoId + ' width=' + imgWidth + ' height=' + imgHeight + ' autoplay=true]', { mode: Markup.MODE_ARTICLE }) }
                        ...
                        var hasFrom1 = (video.date && video.user), hasFrom2 = (videos.length > 1); if (hasFrom1) {
                                var
                                postedOn = new Date(video.date), elapsed = (g_serverTime - postedOn) / 1000; var a = divFrom.firstChild.childNodes[1]; a.href = '?user=' + video.user; a.innerHTML = video.user; var s = divFrom.firstChild.childNodes[3]; $WH.ee(s); g_formatDate(s, elapsed, postedOn); divFrom.firstChild.style.display = ''
                        } else { divFrom.firstChild.style.display = 'none' }
                        var s = divFrom.childNodes[1]; $WH.ee(s); if (video.user) {
                                if (hasFrom1) { $WH.ae(s, $WH.ct(' ' + LANG.dash + ' ')) }
                        ...
                        s = divFrom.childNodes[2]; if (hasFrom2) {
                                var buff = ''; if (video.user) { buff = LANG.dash }
                                buff += (pos + 1) + LANG.lvpage_of + videos.length; s.innerHTML = buff; s.style.display = ''
                        } else { s.style.display = 'none' }
                        divFrom.style.display = (hasFrom1 || hasFrom2 ? '' : 'none'); var hasCaption = (video.caption != null && video.caption.length); var hasSubject = (video.subject != null && video.subject.length && video.type && video.typeId); if (hasCaption || hasSubject) {
                                var html = ''; if (hasSubject) { html += LANG.types[video.type][0] + LANG.colon; html += '<a href="?' + g_types[video.type] + '=' + video.typeId + '">'; html += video.subject; html += '</a>' }
                                if (hasCaption) {
                                        if (hasSubject) { html += LANG.dash }
        ...
        function peekPos(change) {
                var foo = pos; foo += change; if (foo < 0) { foo = videos.length - 1 } else if (foo >= videos.length) { foo = 0 }
                return foo
        }
        ...
        function onResize() { render(1) }
        function onHide() {
                $WH.ee(imgDiv); if (videos.length > 1) { $WH.dE(document, 'keyup', onKeyUp) }
                if (oldHash && mode == 0) {
                        if (oldHash.indexOf(':id=') != -1) { oldHash = '#videos' }
                        location.replace(oldHash)
                } else { location.replace('#.') }
                fixTitle()
        }
        function onShow(dest, first, opt) {
                if (typeof opt.videos == 'string') { videos = g_videos[opt.videos]; mode = 1; collectionId = opt.videos } else { videos = opt.videos; mode = 0; collectionId = null }
        ...
        var
        _self = this, _template, _onSubmit = null, _templateName, _funcs = {}, _data, _inited = !1, _form = $WH.ce('form'), _elements = {}; _form.onsubmit = function () { _processForm(); return !1 }; this.show = function (template, opt) {
                if (template) { _templateName = template; _template = Dialog.templates[_templateName]; _self.template = _template } else { return }
                ...
                _self.data = _data; Lightbox.show('dialog-' + _templateName, { onShow: _onShow, onHide: _onHide })
        }; this.getValue = function (id) { return _getValue(id) }; this.setValue = function (id, value) { _setValue(id, value) }; this.getSelectedValue = function (id) { return _getSelectedValue(id) }; this.getCheckedValue = function (id) { return _getCheckedValue(id) }; function _onShow(dest, first) {
                if (first || !_inited) { _initForm(dest) }
        ...
        function _initForm(dest) {
                $WH.ee(dest); $WH.ee(_form); var container = $WH.ce('div'); container.className = 'text'; $WH.ae(dest, container); $WH.ae(container, _form); if (_template.title) { var h = $WH.ce('h1'); $WH.ae(h, $WH.ct(_template.title)); $WH.ae(_form, h) }
                var t = $WH.ce('table'), tb = $WH.ce('tbody'), mergeCell = !1; $WH.ae(t, tb); $WH.ae(_form, t); for (var i = 0, len = _template.fields.length; i < len; ++i) {
                        var
                        ...
                        var options; if (field.options) {
                                options = []; if (field.optorder) { $WH.cO(options, field.optorder) } else { for (var j in field.options) { options.push(j) } }
                                if (field.sort) { options.sort(function (a, b) { return field.sort * $WH.strcmp(field.options[a], field.options[b]) }) }
                        }
                        switch (field.type) {
                                case 'caption': th.colSpan = 2; th.style.textAlign = 'left'; th.style.padding = 0; if (field.compute) { (field.compute.bind(_self, null, _data[field.id], _form, th, tr))() } else if (field.label) { $WH.ae(th, $WH.ct(field.label)) }
                                        ...
                                        $WH.ae(td, f); break; case 'dynamic': td.colSpan = 2; td.style.textAlign = 'left'; td.style.padding = 0; if (field.compute) (field.compute.bind(_self, null, _data[field.id], _form, td, tr))(); $WH.ae(tr, td); $WH.ae(tb, tr); element = td; break; case 'checkbox': case 'radio': var k = 0; element = []; for (var j = 0, len2 = options.length; j < len2; ++j) {
                                                var
        ...
        function _updateForm() {
                for (var i = 0, len = _template.fields.length; i < len; ++i) {
                        var
                        field = _template.fields[i], f = _elements[field.id]; switch (field.type) {
                                case 'caption': break; case 'select': for (var j = 0, len2 = f.options.length; j < len2; j++) { f.options[j].selected = (f.options[j].value == _data[field.id] || $WH.in_array(_data[field.id], f.options[j].value) != -1) }
        ...
        function _onHide() {
                if (_template.onHide) { _template.onHide() }
                if (_funcs.onHide) { _funcs.onHide() }
        }
        function _processForm(button) {
                if (button == 'cancel') { return Lightbox.hide() }
                for (var i = 0, len = _template.fields.length; i < len; ++i) {
                        var
                        field = _template.fields[i], newValue; switch (field.type) {
                                case 'caption': continue; case 'select': newValue = _getSelectedValue(field.id); break; case 'checkbox': case 'radio': newValue = _getCheckedValue(field.id); break; case 'dynamic': if (field.getValue) { newValue = field.getValue(field, _data, _form); break }
        ...
        function _submitData(button) {
                var ret; if (_onSubmit) { ret = _onSubmit(_data, button, _form) }
...
}; Dialog.templates = {}; var Slider = new function () {
        var
        ...
        function onMouseMove(e) {
                e = $WH.$E(e); if (!start || !handleObj) { return }
        ...
        function onMouseUp(e) {
                e = $WH.$E(e); if (handleObj.onMove) { handleObj.onMove(e, handleObj, getCurrentPosition(handleObj)) }
        ...
        function setPosition(obj, offset) {
                var outside = !1; if (offset < 0) { offset = 0; outside = !0 } else if (offset > getMaxPosition(obj)) { offset = getMaxPosition(obj); outside = !0 }
        ...
        function getTrackerWidth(obj) {
                if (obj._tsz > 0) { return obj._tsz }
                if (obj._dir == 'y') { return obj.parentNode.offsetHeight }
                return obj.parentNode.offsetWidth
        }
        function getHandleWidth(obj) {
                if (obj._hsz > 0) { return obj._hsz }
                if (obj._dir == 'y') { return obj.offsetHeight }
                return obj.offsetWidth
        }
        this.setPercent = function (obj, percent) { setPosition(obj, parseInt(percent * getMaxPosition(obj))) }; this.setValue = function (obj, value) {
                if (value < obj._min) { value = obj._min } else if (value > obj._max) { value = obj._max }
                if (obj.input) { obj.input.value = value }
                this.setPercent(obj, (value - obj._min) / (obj._max - obj._min))
        }; this.setSize = function (obj, length) {
                var
                current = getCurrentPosition(obj), resized = getMaxPosition(obj); obj.parentNode.style[(obj._dir == 'y' ? 'height' : 'width')] = length + 'px'; if (resized != getMaxPosition(obj)) { this.setValue(obj, current.value) }
        }; this.init = function (container, opt) {
                var obj = $WH.ce('a'); obj.href = 'javascript:;'; obj.onmousedown = onMouseDown; obj.className = 'handle'; var track = $WH.ce('a'); track.href = 'javascript:;'; track.onmousedown = onClick.bind(0, obj); track.className = 'track'; $WH.ae(container, $WH.ce('span')); $WH.ae(container, track); $WH.ae(container, obj); obj._dir = 'x'; obj._min = 1; obj._max = 100; obj._pos = 0; obj._tsz = 0; obj._hsz = 0; if (opt != null) {
                        if (opt.direction == 'y') { obj._dir = 'y' }
                        ...
                        if (opt.showLabels !== !1) {
                                var label = $WH.ce('div'); label.innerHTML = obj._min; label.className = 'label min'; $WH.ae(container, label); label = $WH.ce('div'); label.innerHTML = obj._max; label.className = 'label max'; $WH.ae(container, label); obj.input = $WH.ce('input'); $(obj.input).attr({ value: obj._max, type: 'text' }).bind('click', function () { this.select() }).keypress(function (e) {
                                        var allowed = []; var usedKey = e.which; for (i = 48; i < 58; i++) { allowed.push(i) }
                                        if (!($WH.in_array(allowed, usedKey) >= 0) && usedKey != 13) { e.preventDefault() }
                                }).bind('keyup keydown change', onKeyPress.bind(0, obj)); obj.input.className = 'input'; $WH.ae(container, obj.input)
                        }
                }
                container.className = 'slider-' + obj._dir + (opt == null || opt.showLabels !== !1 ? ' has-labels' : ''); return obj
        }
}; var suDialog; function su_addToSaved(items, nItems, newWindow, level) {
        if (!items) { return }
        if (!suDialog) { suDialog = new Dialog() }
        var doCompare = function (data) {
                var saved = g_getWowheadCookie('compare_groups'), url = '?compare'; if (data.action > 1) {
                        if (saved) { items = saved + ';' + items }
                        g_setWowheadCookie('compare_groups', items, !0); if (level) { g_setWowheadCookie('compare_level', level, !0) }
                } else { url += '=' + items + (level ? '&l=' + level : '') }
                if (data.action < 3) { if (newWindow) { window.open(url) } else { location.href = url } }
        }; suDialog.show('docompare', { data: { selecteditems: nItems, action: 1 }, onSubmit: doCompare })
}
Dialog.templates.docompare = { title: LANG.dialog_compare, width: 400, buttons: [['okay', LANG.ok], ['cancel', LANG.cancel]], fields: [{ id: 'selecteditems', type: 'caption', compute: function (field, value, form, td) { td.innerHTML = $WH.sprintf((value == 1 ? LANG.dialog_selecteditem : LANG.dialog_selecteditems), value) } }, { id: 'action', type: 'radio', label: '', value: 3, submitOnDblClick: 1, options: { 1: LANG.dialog_nosaveandview, 2: LANG.dialog_saveandview, 3: LANG.dialog_saveforlater } }] }; var ctrEnter; var ctrEnterWidget = function () { var ctrEnterMainStyle = 'position:fixed;z-index:5;right:1px;bottom:1px;background:#242424;;border-radius:5px;padding:5px;font-size:10px;display:inline-block;white-space:nowrap;color:#fff;opacity:0.7;'; var ctrEnterCloseStyle = 'position:absolute;top:0;cursor:pointer;'; var ctrEnterImgStyle = 'float:left;margin-right:5px;margin-top:5px;'; var ctrEnterImgSrc = 'data:image/png;base64,'; document.write('<div id="ctrEnterWidget" style="' + ctrEnterMainStyle + '"><img src="' + ctrEnterImgSrc + '" style="' + ctrEnterImgStyle + '"/>' + LANG.title_ctrEnter + '<br>' + LANG.desc_ctrEnter + '<br><span>Ctrl/Cmd&nbsp;+&nbsp;Enter</span><div onclick="ctrEnterWidgetClose()" title="' + LANG.tc_lock + '" style="' + ctrEnterCloseStyle + '">x</div></div>') }
...
function modal_ctrEnter(text) {
        if (!ctrEnter) { ctrEnter = new Dialog() }
...
Dialog.templates.sbm_ctrEnter = { title: LANG.title_ctrEnter, width: 420, buttons: [['okay', LANG.ok], ['cancel', LANG.cancel]], fields: [{ id: 'login_ctrEnter', type: 'text', size: 0, compute: function (field, value, form, td) { td.style.display = 'none' } }, { id: 'email_ctrEnter', type: 'email', placeholder: 'E-mail', required: 1, size: 0, compute: function (field, value, form, td) { field.style.width = '100%'; field.setAttribute('title', 'E-mail') } }, { id: 'text_ctrEnter', type: 'textarea', placeholder: LANG.text_ctrEnter, required: 1, size: 0, compute: function (field, value, form, td) { field.style.width = '100%'; field.setAttribute('title', LANG.text_ctrEnter); field.setAttribute('readonly', 'readonly') } }, { id: 'msg_ctrEnter', type: 'textarea', placeholder: LANG.msg_ctrEnter, required: 1, size: 0, compute: function (field, value, form, td) { field.style.width = '100%'; field.setAttribute('title', LANG.msg_ctrEnter) } }, { id: 'bot_ctrEnter', type: 'text', placeholder: LANG.robot_ctrEnter, required: 1, size: 0, compute: function (field, value, form, td) { td.style.cssText = 'text-align:center'; var img = document.createElement('img'); var img_src = g_host + '/config/?auth2_captcha=' + Math.random() * 1000; img.src = img_src; img.style.cssText = 'width:130px;height:50px;cursor:pointer'; img.setAttribute('onclick', 'this.src = "' + img_src + '"'); td.appendChild(img); field.style.cssText = 'margin:6px;padding:8px;font-size:16px;vertical-align:top;'; field.setAttribute('title', LANG.robot_ctrEnter) } }] }; $(document).ready(function () { $(document).keydown(function (e) { if ((e.ctrlKey && e.keyCode == 13) || (e.metaKey && e.keyCode == 13)) { e.preventDefault(); var text = ""; if (window.getSelection) text = window.getSelection().toString(); else if (document.selection && document.selection.type != "Control") text = document.selection.createRange().text; if (text !== '') modal_ctrEnter(text) } }) }); var Favorites = new function () {
        ...
        this.hasFavorites = function () { return !!g_favorites.length }; this.getMenu = function () {
                ...
                Menu.sort(favMenu); if ((nGroups == 1 && nEntries <= 45) || (nGroups == 2 && nGroups + nEntries <= 30) || (nGroups > 2 && nGroups + nEntries <= 15)) {
                        var list = []; for (var i = 0; subMenu = favMenu[i]; i++) {
                                list.push([, subMenu[MENU_IDX_NAME]]); for (var j = 0, subEntry; subEntry = subMenu[MENU_IDX_SUB][j]; j++) {
                                        var listEntry = [subEntry[MENU_IDX_ID], subEntry[MENU_IDX_NAME], subEntry[MENU_IDX_URL]]; if (subEntry[MENU_IDX_OPT])
                ...
                var menuRoot = $('#toplinks-favorites'); if (!menuRoot.length)
                        return; var favMenu = Favorites.getMenu(); if (!favMenu.length) { menuRoot.hide(); return }
                Menu.add(menuRoot, favMenu); menuRoot.show()
        }; function createIcon(heading) { }
        function updateIcon(type, typeId) {
                if (_favIcon) {
                        var rmv = 'fav-star-0'; var add = 'fav-star-1'; if (!isFaved(type, typeId)) { rmv = 'fav-star-1'; add = 'fav-star-0' }
                        _favIcon.removeClass(rmv).addClass(add)
                }
        }
        function isFaved(type, typeId) {
                var idx = getIndex(type); if (idx == -1)
        ...
        function addEntry(type, typeId, name) {
                var idx = getIndex(type, !0); if (idx == -1)
        ...
        function removeEntry(type, typeId) {
                var idx = getIndex(type); if (idx == -1)
                        return; for (var i = 0, j; j = g_favorites[idx].entities[i]; i++) {
                                if (j[0] == typeId) {
                                        sendUpdate('remove', type, typeId); g_favorites[idx].entities.splice(i, 1); if (!g_favorites[idx].entities.length)
        ...
        $WH.cO(this, opt); if (this.parent) { this.parent = $WH.ge(this.parent) } else { return }
...
Tabs.prototype = {
        add: function (caption, opt) {
                var
                _, index = this.tabs.length; _ = { caption: caption, index: index, owner: this }; $WH.cO(_, opt); this.tabs.push(_); return index
        }, hide: function (index, visible) {
                if (this.tabs[index]) {
                        var selectedTab = this.selectedTab; if (index == 0 && selectedTab == -1) { this.poundedTab = this.selectedTab = selectedTab = 0 }
                        if (index != this.poundedTab) { this.selectedTab = -1 }
                        this.tabs[index].hidden = !visible; this.flush(); if (!visible && index == selectedTab) { this.selectedTab = selectedTab; for (var i = 0, len = this.tabs.length; i < len; ++i) { if (i != index && !this.tabs[i].hidden) { return this.show(i, 1) } } }
                }
        }, unlock: function (index, locked) {
                if (this.tabs[index]) {
                        this.tabs[index].locked = locked; _ = $WH.gE(this.uls[0], 'a'); $('.icon-lock', _[index]).remove(); if (locked) { $('div, b', _[index]).prepend('<span class="icon-lock" />') }
                        var _ = location.hash.substr(1).split(':')[0]; if (this.tabs[index].id == _) { this.show(index, 1) }
                }
        }, focus: function (index) {
                if (index < 0) { index = this.tabs.length + index }
                this.forceScroll = 1; $WH.gE(this.uls[0], 'a')[index].onclick({}, !0); this.forceScroll = null
        }, show: function (index, forceClick) {
                var _; if (isNaN(index) || index < 0) { index = 0 } else if (index >= this.tabs.length) { index = this.tabs.length - 1 }
                ...
                if (this.selectedTab != -1) {
                        _ = this.tabs[this.selectedTab]; if (this.onHide && !this.onHide(_)) { return }
                ...
                var _, l, a, b, d, d2; var container = $WH.ce('div'); container.className = 'tabs-container'; this.uls[0] = $WH.ce('ul'); this.uls[0].className = 'tabs'; d = $WH.ce('div'); d.className = 'tabs-levels'; $WH.ae(container, this.uls[0]); for (var i = 0; i < this.tabs.length; ++i) {
                        var tab = this.tabs[i]; l = $WH.ce('li'); a = $WH.ce('a'); b = $WH.ce('b'); if (tab.hidden) { l.style.display = 'none' }
        ...
        }, setTabName: function (index, name) { this.tabs[index].caption = name; var _ = $WH.gE(this.uls[0], 'a'); g_setTextNodes(_[index], name) }, setTabPound: function (index, pound) {
                if (!this.poundable) { return }
                var _ = $WH.gE(this.uls[0], 'a'); _[index].href = '#' + this.tabs[index].id + (pound ? ':' + pound : '')
        }, setTabTooltip: function (index, text) { this.tabs[index].tooltip = text; var _ = $WH.gE(this.uls[0], 'a'); if (text == null) { _[index].onmouseover = _[index].onmousemove = _[index].onmouseout = null } else { _[index].onmouseover = function (e) { $WH.Tooltip.showAtCursor(e, text, 0, 0, 'q2') }; _[index].onmousemove = $WH.Tooltip.cursorUpdate; _[index].onmouseout = $WH.Tooltip.hide } }, getSelectedTab: function () { return this.selectedTab }
}; Tabs.onClick = function (a, e, forceClick) {
        if (forceClick == null && this.index == this.owner.selectedTab) { return }
        ...
        var _ = location.hash.substr(1).split(':')[0]; if (_) { return $WH.in_array(this.tabs, _, function (x) { if (!x.locked) { return x.id } }) }
}; Tabs.onShow = function (newTab, oldTab) {
        var _; if (newTab.hidden || newTab.locked) { return }
        if (oldTab) { $WH.ge('tab-' + oldTab.id).style.display = 'none' }
        if (this.poundedTab != null || oldTab) { this.trackClick(newTab) }
        _ = $WH.ge('tab-' + newTab.id); _.style.display = ''; if (((this.nShows == 1 && this.poundedTab != null && this.poundedTab >= 0) || this.forceScroll) && !this.noScroll) {
                var el, padd; if (this.__st) { el = this.__st; padd = 15 } else { el = _; padd = this.parent.offsetHeight + 15 }
                setTimeout($WH.g_scrollTo.bind(null, el, padd), 10)
        }
}; Tabs.trackClick = function (tab) {
        if (!this.trackable || tab.tracked) { return }
...
function g_addAnalyticsToNode(node, opts, labelPrefix) {
        if (!opts || !opts.actions || !opts.category) {
                if ($WH.isset('g_dev') && g_dev) { console.log('Tried to add analytics event without appropriate parameters.'); console.log(node); console.log(opts) }
                return
        }
        var category = opts.category; var tags = $WH.gE(node, 'a'); for (var i = 0; i < tags.length; ++i) {
                var node = tags[i]; var action = 'Follow link'; for (var a in opts.actions) { if (opts.actions[a] && opts.actions[a](node)) { action = a; break } }
                var label = (labelPrefix ? labelPrefix + '-' : '') + g_getNodeTextId(node); g_attachTracking(node, category, action, label)
        }
}
$(document).ready(g_addAnalytics); var g_listviews = {}; function Listview(opt) {
        $WH.cO(this, opt); if (this.id) { var divId = (this.tabs ? 'tab-' : 'lv-') + this.id; if (this.parent) { var d = $WH.ce('div'); d.id = divId; $WH.ae($WH.ge(this.parent), d); this.container = d } else { this.container = $WH.ge(divId) } } else { return }
        ...
        this.nFilters = 0; this.resetRowVisibility(); if (this.mode == Listview.MODE_TILED) {
                if (this.nItemsPerRow == null) { var ipr = this.template.nItemsPerRow; this.nItemsPerRow = (ipr != null ? ipr : 4) }
                this.nItemsPerRow |= 0; if (this.nItemsPerRow <= 1) { this.nItemsPerRow = 1 }
        } else if (this.mode == Listview.MODE_CALENDAR) {
                this.dates = []; this.nItemsPerRow = 7; this.nItemsPerPage = 1; this.nDaysPerMonth = []; if (this.template.startOnMonth != null) { this.startOnMonth = this.template.startOnMonth } else { this.startOnMonth = new Date() }
                ...
                if (this.template.rowOffset != null) { this.rowOffset = this.template.rowOffset }
        } else { this.nItemsPerRow = 1 }
        this.columns = []; for (var i = 0, len = this.template.columns.length; i < len; ++i) {
                var
        ...
        if (this.extraCols != null) {
                for (var i = 0, len = this.extraCols.length; i < len; ++i) {
                        var pos = null; var col = this.extraCols[i]; if (col.after || col.before) { var index = $WH.in_array(this.columns, (col.after ? col.after : col.before), function (x) { return x.id }); if (index != -1) { pos = (col.after ? index + 1 : index) } }
        ...
        if (this.sort != null) {
                var sortParam = this.sort; this.sort = []; for (var i = 0, len = sortParam.length; i < len; ++i) {
                        var col = parseInt(sortParam[i]); if (isNaN(col)) {
                                var desc = 0; if (sortParam[i].charAt(0) == '-') { desc = 1; sortParam[i] = sortParam[i].substring(1) }
                                var index = $WH.in_array(this.columns, sortParam[i], function (x) { return x.id }); if (index != -1) { if (desc) { this.sort.push(-(index + 1)) } else { this.sort.push(index + 1) } }
                        } else { this.sort.push(col) }
                }
        } else { this.sort = [] }
        if (this.debug || g_user.debug) {
                this.columns.splice(0, 0, {
                        id: 'debug-id', compute: function (data, td) { if (data.id) { $WH.ae(td, $WH.ct(data.id)) } }, getVisibleText: function (data) { if (data.id) { return data.id } else { return '' } }, getValue: function (data) { if (data.id) { return data.id } else { return 0 } }, sortFunc: function (a, b, col) {
                                if (a.id == null) { return -1 } else if (b.id == null) { return 1 }
                                return $WH.strcmp(a.id, b.id)
                        }, name: 'ID', width: '5%', tooltip: 'ID'
                }); this.visibility.splice(0, 0, -1); for (var i = 0, len = this.visibility.length; i < len; ++i) { this.visibility[i] = this.visibility[i] + 1 }
...
Listview.MODE_DEFAULT = 0; Listview.MODE_CHECKBOX = 1; Listview.MODE_DIV = 2; Listview.MODE_TILED = 3; Listview.MODE_CALENDAR = 4; Listview.MODE_FLEXGRID = 5; Listview.prototype = {
        initialize: function () {
                if (this.data.length) { if (this.computeDataFunc != null) { for (var i = 0, len = this.data.length; i < len; ++i) { this.computeDataFunc(this.data[i]) } } }
                ...
                this.noData = $WH.ce('div'); this.noData.className = 'listview-nodata text'; if (this.mode == Listview.MODE_DIV) { this.mainContainer = this.mainDiv = $WH.ce('div'); if (!this.noStyle) { this.mainContainer.className = 'listview-mode-div' } } else if (this.mode == Listview.MODE_FLEXGRID) {
                        this.mainContainer = this.mainDiv = $WH.ce('div', { className: 'listview-mode-flexgrid' }); this.mainContainer.setAttribute('data-cell-min-width', this.template.cellMinWidth); if (this.clickable)
                        ...
                        this.mainContainer = this.table = $WH.ce('table'); this.thead = $WH.ce('thead'); this.tbody = $WH.ce('tbody'); if (this.clickable) { this.tbody.className = 'clickable' }
                        if (this.mode == Listview.MODE_TILED || this.mode == Listview.MODE_CALENDAR) {
                                ...
                                if (!this.noStyle) { this.table.className = 'listview-mode-default' }
                ...
                this.updateNav(); this.refreshRows(); this.updateBrowseSession(); if (this.onAfterCreate != null) { this.onAfterCreate(obcResult) }
        }, createHeader: function () {
                var tr = $WH.ce('tr'); if (this.mode == Listview.MODE_CHECKBOX) {
                        var
                ...
                if (!$.isArray(this.sortOptions))
                        ...
                        activeSort = this.sort[0]; var a; var sorts = []; for (var i = 0; i < this.sortOptions.length; i++) {
                ...
                for (i = 0; i < sorts.length; i++)
                        $WH.ae(sp, sorts[i]); $WH.ae(div, sp); $WH.aef(parent, div)
        }, sortGallery: function (el, colNo) { var btn = $(el); btn.siblings('a').removeClass('active'); btn.addClass('active'); this.sortBy(colNo) }, createBands: function () {
                var
                ...
                $WH.ae(bandTop, this.navTop); if (this.searchable) {
                        var
                ...
                $WH.ae(bandTop, noteTop); $WH.ae(bandBot, this.navBot); $WH.ae(bandBot, noteBot); if (this.createCbControls || this.mode == Listview.MODE_CHECKBOX) {
                        if (this.note) { noteTop.style.paddingBottom = '5px' }
                ...
                var div = $WH.ce('div'); if (this.createCbControls) { this.createCbControls(div, topBar) }
                if (div.firstChild) { div.className = 'listview-withselected' + (topBar ? '' : '2') }
                return div
        }, refreshRows: function () {
                var target = null; switch (this.mode) { case Listview.MODE_DIV: target = this.mainContainer; break; case Listview.MODE_FLEXGRID: target = this.mainDiv; break; default: target = this.tbody }
                if (!target)
                        return; $WH.ee(target); if (this.nRowsVisible == 0) {
                ...
                if (this.nDaysPerMonth && this.nDaysPerMonth.length) {
                        starti = 0; for (var i = 0; i < this.rowOffset; ++i) { starti += this.nDaysPerMonth[i] }
                        endi = starti + this.nDaysPerMonth[i]
                } else if (this.nItemsPerPage > 0) {
                        starti = this.rowOffset; endi = Math.min(starti + this.nRowsVisible, starti + this.nItemsPerPage); if (this.filtered && this.rowOffset > 0) {
                                for (var i = 0, count = 0; i < this.data.length && count < this.rowOffset; ++i) { var row = this.data[i]; if (row.__hidden || row.__deleted) { ++starti } else { ++count } }
                                endi += (starti - this.rowOffset)
                        }
                } else { starti = 0; endi = this.nRowsVisible }
                var nItemsToDisplay = endi - starti; if (this.mode == Listview.MODE_DIV) {
                        ...
                        var
                        k = 0, tr = $WH.ce('tr'); for (var j = 0; j < nItemsToDisplay; ++j) {
                                var
                                i = starti + j, row = this.data[i]; if (!row) { break }
                                if (row.__hidden || row.__deleted) { ++nItemsToDisplay; continue }
                                $WH.ae(tr, this.getCell(i)); if (++k == this.nItemsPerRow) {
                                        $WH.ae(this.tbody, tr); if (j + 1 < nItemsToDisplay) { tr = $WH.ce('tr') }
                                        k = 0
                                }
                        }
                        if (k != 0) {
                                for (; k < 4; ++k) { var foo = $WH.ce('td'); foo.className = 'empty-cell'; $WH.ae(tr, foo) }
                                $WH.ae(this.tbody, tr)
                        }
                } else if (this.mode == Listview.MODE_CALENDAR) {
                        var tr = $WH.ce('tr'); for (var i = 0; i < 7; ++i) { var th = $WH.ce('th'); $WH.st(th, LANG.date_days[i]); $WH.ae(tr, th) }
                        ...
                        if (k != 0) {
                                for (; k < 7; ++k) { var foo = $WH.ce('td'); foo.className = 'empty-cell'; $WH.ae(tr, foo) }
                                $WH.ae(this.tbody, tr)
                        }
                } else {
                        for (var j = 0; j < nItemsToDisplay; ++j) {
                ...
                var div = this.noData; $WH.ee(div); var result = -1; if (this.onNoData) { result = (this.onNoData.bind(this, div))() }
                if (result == -1) { $WH.ae(this.noData, $WH.ct(this.filtered ? LANG.lvnodata2 : LANG.lvnodata)) }
        }, getDiv: function (i) {
                var row = this.data[i]; if (row.__div == null || this.minPatchVersion != row.__minPatch) { this.createDiv(row, i) }
                return row.__div
        }, createDiv: function (row, i) { var div = $WH.ce('div'); row.__div = div; if (this.minPatchVersion) { row.__minPatch = this.minPatchVersion } (this.template.compute.bind(this, row, div, i))() }, getCell: function (i) {
                var row = this.data[i]; if (row.__td == null) { this.createCell(row, i) }
                return row.__td
        }, createCell: function (row, i) { var td = $WH.ce('td'); row.__td = td; (this.template.compute.bind(this, row, td, i))() }, getEvent: function (i) {
                var row = this.dates[i]; if (row.__td == null) { this.createEvent(row, i) }
                return row.__td
        }, createEvent: function (row, i) {
                row.events = $WH.array_filter(this.data, function (holiday) {
                        if (holiday.__hidden || holiday.__deleted) { return !1 }
                        ...
                        return !1
                }); var td = $WH.ce('td'); row.__td = td; if (row.date.getFullYear() == g_serverTime.getFullYear() && row.date.getMonth() == g_serverTime.getMonth() && row.date.getDate() == g_serverTime.getDate()) { td.className = 'calendar-today' }
                var div = $WH.ce('div'); div.className = 'calendar-date'; $WH.st(div, row.date.getDate()); $WH.ae(td, div); div = $WH.ce('div'); div.className = 'calendar-event'; $WH.ae(td, div); (this.template.compute.bind(this, row, div, i))(); if (this.getItemLink) { td.onclick = this.itemClick.bind(this, row) }
        }, getRow: function (i) {
                var row = this.data[i]; if (row.__tr == null) { this.createRow(row) }
                return row.__tr
        }, setRow: function (newRow) { if (this.data[newRow.pos]) { this.data[newRow.pos] = newRow; this.data[newRow.pos].__tr = newRow.__tr; this.createRow(this.data[newRow.pos]); this.refreshRows() } }, createRow: function (row) {
                var tr = $WH.ce('tr'); row.__tr = tr; if (this.mode == Listview.MODE_CHECKBOX) {
                        var td = $WH.ce('td'); if (!row.__nochk) {
                                td.className = 'listview-cb'; td.onclick = Listview.cbCellClick; var cb = $WH.ce('input'); $WH.ns(cb); cb.type = 'checkbox'; cb.onclick = Listview.cbClick; if (row.__chk) { cb.checked = !0 }
                ...
                if (this.getItemLink) { tr.onclick = this.itemClick.bind(this, row) }
        }, itemClick: function (row, e) {
                e = $WH.$E(e); var i = 0, el = e._target; while (el && i < 3) {
                        if (el.nodeName == 'A') { return }
                        el = el.parentNode
                }
                location.href = this.getItemLink(row)
        }, submitSearch: function (e) {
                e = $WH.$E(e); if (!this.onSearchSubmit || e.keyCode != 13) { return }
                for (var i = 0, len = this.data.length; i < len; ++i) { if (this.data[i].__hidden) { continue } (this.onSearchSubmit.bind(this, this.data[i]))() }
        }, validatePage: function () {
                var
                rpp = this.nItemsPerPage, ro = this.rowOffset, len = this.nRowsVisible; if (ro < 0) { this.rowOffset = 0 } else if (this.mode == Listview.MODE_CALENDAR) { this.rowOffset = Math.min(ro, this.nDaysPerMonth.length - 1) } else { this.rowOffset = this.getRowOffset(ro + rpp > len ? len - 1 : ro) }
        }, getRowOffset: function (rowNo) { var rpp = this.nItemsPerPage; return (rpp > 0 && rowNo > 0 ? Math.floor(rowNo / rpp) * rpp : 0) }, resetRowVisibility: function () {
                for (var i = 0, len = this.data.length; i < len; ++i) { this.data[i].__hidden = !1 }
                this.filtered = !1; this.rowOffset = 0; this.nRowsVisible = this.data.length
        }, getColText: function (row, col) {
                var text = ''; if (this.template.getVisibleText) { text = $WH.trim(this.template.getVisibleText(row) + ' ') }
                ...
                return ""
        }, resetFilters: function () { for (var j = 0, len2 = this.visibility.length; j < len2; ++j) { var realj = this.visibility[j]; var col = this.columns[realj]; if (col.__filter) { col.__th.firstChild.firstChild.className = ''; col.__filter = null; --(this.nFilters) } } }, updateFilters: function (refresh) {
                $WH.Tooltip.hide(); this.resetRowVisibility(); var searchText, parts, nParts; if (this.searchable) { this.quickSearchBox.parentNode.style.display = ''; searchText = $WH.trim(this.quickSearchBox.value); if (searchText) { this.quickSearchGlass.style.display = 'none'; this.quickSearchClear.style.display = ''; searchText = searchText.toLowerCase().replace(/\s+/g, ' '); parts = searchText.split(' '); nParts = parts.length } else { this.quickSearchGlass.style.display = ''; this.quickSearchClear.style.display = 'none' } } else if (this.quickSearchBox) { this.quickSearchBox.parentNode.style.display = 'none' }
                if (!searchText && this.nFilters == 0 && this.customFilter == null) {
                        if (refresh) { this.updateNav(); this.refreshRows(); this.updateBrowseSession() }
                        return
                }
                var filterFuncs = { 1: function (x, y) { return x > y }, 2: function (x, y) { return x == y }, 3: function (x, y) { return x < y }, 4: function (x, y) { return x >= y }, 5: function (x, y) { return x <= y }, 6: function (x, y, z) { return y <= x && x <= z } }; var filterFuncs2 = { 1: function (min, max, y) { return max > y }, 2: function (min, max, y) { return min <= y && y <= max }, 3: function (min, max, y) { return min < y }, 4: function (min, max, y) { return max >= y }, 5: function (min, max, y) { return min <= y }, 6: function (min, max, y, z) { return y <= max && min <= z } }; var nRowsVisible = 0; for (var i = 0, len = this.data.length; i < len; ++i) {
                        var
                        ...
                        for (var j = 0, len2 = this.visibility.length; j < len2; ++j) {
                                var realj = this.visibility[j]; var col = this.columns[realj]; if (col.__filter) {
                                        var
                                        filter = col.__filter, result = !1; if (col.type != null && col.type == 'range') {
                                                var
                                                minValue = col.getMinValue(row), maxValue = col.getMaxValue(row); result = (filterFuncs2[filter.type])(minValue, maxValue, filter.value, filter.value2)
                                        } else if (col.type == null || col.type == 'num' || filter.type > 0) {
                                                var value = null; if (col.getValue) { value = col.getValue(row) } else if (col.value) { value = parseFloat(row[col.value]) }
                                                if (!value) { value = 0 }
                                                result = (filterFuncs[filter.type])(value, filter.value, filter.value2)
                                        } else {
                                                var text = this.getColText(row, col); if (text) {
                                                        text = text.toString().toLowerCase(); if (filter.invert) { result = text.match(filter.regex) != null } else {
                                                                var foo = 0; for (var k = 0, len3 = filter.words.length; k < len3; ++k) { if (text.indexOf(filter.words[k]) != -1) { ++foo } else { break } }
                ...
                this.filtered = (nRowsVisible < this.data.length); this.nRowsVisible = nRowsVisible; if (refresh) { this.updateNav(); this.refreshRows(); this.updateBrowseSession() }
        }, changePage: function () { this.validatePage(); this.refreshRows(); this.updateNav(); this.updatePound(); this.updateBrowseSession(); var scroll = $WH.g_getScroll(), c = $WH.ac(this.container); if (scroll.y > c[1]) { scrollTo(scroll.x, c[1]) } }, firstPage: function () { this.rowOffset = 0; this.changePage(); return !1 }, previousPage: function () { this.rowOffset -= this.nItemsPerPage; this.changePage(); return !1 }, nextPage: function () { this.rowOffset += this.nItemsPerPage; this.changePage(); return !1 }, lastPage: function () { this.rowOffset = 99999999; this.changePage(); return !1 }, addSort: function (arr, colNo) {
                var i = $WH.in_array(arr, Math.abs(colNo), function (x) { return Math.abs(x) }); if (i != -1) { colNo = arr[i]; arr.splice(i, 1) }
                arr.splice(0, 0, colNo)
        }, sortBy: function (colNo) {
                var sorts = this.sortOptions || this.columns; if (colNo <= 0 || colNo > sorts.length) { return }
                if (Math.abs(this.sort[0]) == colNo) { this.sort[0] = -this.sort[0] } else {
                        var defaultSort = -1; if (sorts[colNo - 1].type == 'text') { defaultSort = 1 }
                ...
                this.updateSortIndex(); this.updateBrowseSession()
        }, setSort: function (sort, refresh, updatePound) {
                if (this.sort.toString() != sort.toString()) {
                        this.sort = sort; this.applySort(); if (refresh) { this.refreshRows() }
                        if (updatePound) { this.updatePound() }
                }
        }, readPound: function () {
                if (!this.poundable || !location.hash.length) { return !1 }
                var _ = location.hash.substr(1); if (this.tabs) {
                        var n = _.lastIndexOf(':'); if (n == -1) { return !1 }
                        _ = _.substr(n + 1)
                }
                var num = parseInt(_); if (!isNaN(num)) {
                        this.rowOffset = num; this.validatePage(); if (this.poundable != 2) {
                                var sort = []; var matches = _.match(/(\+|\-)[0-9]+/g); if (matches != null) {
                                        var sorts = this.sortOptions || this.columns; for (var i = matches.length - 1; i >= 0; --i) {
                                                var colNo = parseInt(matches[i]) | 0; var _ = Math.abs(colNo); if (_ <= 0 || _ > sorts.length) { break }
                ...
                if ((!window.JSON) || (!$WH.localStorage.isSupported())) { return }
                ...
                var max = 5; var path = location.pathname + location.search; var expire = (new Date(g_serverTime.getTime() - 1000 * 60 * 60 * 24 * 3)).getTime(); var lv = $WH.localStorage.get('lvBrowse'); if (lv) {
                        lv = window.JSON.parse(lv); for (var i = 0; i < lv.length; i++) { if ((lv[i].path == path) || (lv[i].when < expire)) { lv.splice(i--, 1) } }
                        if (lv.length >= max) { lv.splice(max - 1, lv.length - max - 1) }
                } else { lv = [] }
                ...
                lv.unshift({ path: path, hash: location.hash, when: g_serverTime.getTime(), urls: urls }); $WH.localStorage.set('lvBrowse', window.JSON.stringify(lv))
        }, updateSortArrow: function () {
                if (!this.sort.length || !this.thead || this.mode == Listview.MODE_CALENDAR) { return }
                ...
                span.className = (this.sort[0] < 0 ? 'sortdesc' : 'sortasc'); this.lsa = span
        }, updateSortIndex: function () {
                var _ = this.data; for (var i = 0, len = _.length; i < len; ++i) { _[i].__si = i }
                this.indexCreated = !0
        }, updateTabName: function () { if (this.tabs && this.tabIndex != null) { this.tabs.setTabName(this.tabIndex, this.getTabName()) } }, updatePound: function (useCurrentSort) {
                if (!this.poundable) { return }
                ...
                var
                arr = [this.navTop, this.navBot], _ = this.nItemsPerPage, __ = this.rowOffset, ___ = this.nRowsVisible, first = 0, previous = 0, next = 0, last = 0, date = new Date(); if (___ > 0) {
                        if (!(this.hideNav & 1)) { arr[0].style.display = '' }
                        if (!(this.hideNav & 2)) { arr[1].style.display = '' }
                } else { arr[0].style.display = arr[1].style.display = 'none' }
                ...
                var buffer = ''; buffer += this.rowOffset; if (this.poundable != 2 && this.sort.length) { buffer += ('+' + this.sort.join('+')).replace(/\+\-/g, '-') }
                return buffer
        }, getCheckedRows: function () {
                var checkedRows = []; for (var i = 0, len = this.data.length; i < len; ++i) { var _ = this.data[i]; if ((_.__cb && _.__cb.checked) || (!_.__cb && _.__chk)) { checkedRows.push(_) } }
                return checkedRows
        }, resetCheckedRows: function () {
                for (var i = 0, len = this.data.length; i < len; ++i) {
                        var _ = this.data[i]; if (_.__cb) { _.__cb.checked = !1 } else if (_.__chk) { _.__chk = null }
                        if (_.__tr) { _.__tr.className = _.__tr.className.replace('checked', '') }
                }
        }, deleteRows: function (rows) {
                if (!rows || !rows.length) { return }
                for (var i = 0, len = rows.length; i < len; ++i) {
                        var row = rows[i]; if (!row.__hidden && !row.__hidden) { this.nRowsVisible -= 1 }
                        row.__deleted = !0
                }
                this.updateTabName(); if (this.rowOffset >= this.nRowsVisible) { this.previousPage() } else { this.refreshRows(); this.updateNav(); this.updateBrowseSession() }
        }, setData: function (data) {
                this.data = data; this.indexCreated = !1; this.resetCheckedRows(); this.resetRowVisibility(); if (this.tabs) { this.pounded = (this.tabs.poundedTab == this.tabIndex); if (this.pounded) { this.readPound() } } else { this.readPound() }
                this.applySort(); this.updateSortArrow(); if (this.customFilter != null) { this.updateFilters() }
                this.updateNav(); this.refreshRows(); this.updateBrowseSession()
        }, getClipDiv: function () { return this.clipDiv }, getNoteTopDiv: function () { return this.noteTop }, focusSearch: function () { this.quickSearchBox.focus() }, clearSearch: function () { this.quickSearchBox.value = '' }, getList: function () {
                ...
                if (!this.noteIndicators) { this.noteIndicators = $WH.ce('div'); this.noteIndicators.className = 'listview-indicators'; $(this.noteIndicators).insertBefore($(this.noteTop)) }
                ...
                if (this.noteIndicators) { $(this.noteIndicators).remove(); this.noteIndicators = null }
                $(this.noteTop).css('padding-top', '')
        }
}; Listview.sortRows = function (a, b) {
        var
        sort = Listview.sort, cols = Listview.columns; for (var i = 0, len = sort.length; i < len; ++i) {
                var
        ...
        var
        sort = Listview.sort, cols = Listview.sortOptions || Listview.columns, res; for (var idx in sort) {
                _ = cols[Math.abs(sort[idx]) - 1]; if (!_) { _ = this.template }
        ...
        for (var i = 0, len = this.data.length; i < len; ++i) {
                var _ = this.data[i]; var v2 = v; if (_.__hidden) { continue }
                if (!_.__nochk && _.__cb) {
                        var
                        cb = _.__cb, tr = cb.parentNode.parentNode; if (v2 == null) { v2 = !cb.checked }
                        if (cb.checked != v2) { cb.checked = v2; tr.className = (cb.checked ? tr.className + ' checked' : tr.className.replace('checked', '')) }
                } else if (v2 == null) { v2 = !0 }
                _.__chk = v2
        }
}; Listview.cbClick = function (e) { setTimeout(Listview.cbUpdate.bind(0, 0, this, this.parentNode.parentNode), 1); $WH.sp(e) }; Listview.cbCellClick = function (e) { setTimeout(Listview.cbUpdate.bind(0, 1, this.firstChild, this.parentNode), 1); $WH.sp(e) }; Listview.cbUpdate = function (toggle, cb, tr) {
        if (toggle) { cb.checked = !cb.checked }
        tr.className = (cb.checked ? tr.className + ' checked' : tr.className.replace('checked', ''))
}; Listview.headerClick = function (col, i, e) {
        e = $WH.$E(e); if (this.tabClick) { this.tabClick() }
        ...
        var prefilled = ''; if (col.__filter) {
                if (col.__filter.invert) { prefilled += '!' }
        ...
        if (res != null) {
                var filter = { text: '', type: -1 }; res = $WH.trim(res.replace(/\s+/g, ' ')); if (!res && this.onEmptyFilter) { this.onEmptyFilter(col) } else if (res) {
                        if (res.charAt(0) == '!' || res.charAt(0) == '-') { filter.invert = 1; res = res.substr(1) }
                        ...
                        var value, value2; if (res.match(/(>|=|<|>=|<=)\s*([0-9\.]+)/)) {
                                value = parseFloat(RegExp.$2); if (!isNaN(value)) {
                                        switch (RegExp.$1) { case '>': filter.type = 1; break; case '=': filter.type = 2; break; case '<': filter.type = 3; break; case '>=': filter.type = 4; break; case '<=': filter.type = 5; break }
                                        filter.value = value; filter.text = RegExp.$1 + ' ' + value
                                }
                        } else if (res.match(/([0-9\.]+)\s*\-\s*([0-9\.]+)/)) {
                                value = parseFloat(RegExp.$1); value2 = parseFloat(RegExp.$2); if (!isNaN(value) && !isNaN(value2)) {
                                        if (value > value2) { var foo = value; value = value2; value2 = foo }
                                        if (value == value2) { filter.type = 2; filter.value = value; filter.text = '= ' + value } else { filter.type = 6; filter.value = value; filter.value2 = value2; filter.text = value + ' - ' + value2 }
                                }
                        } else { var parts = res.toLowerCase().split(' '); if (!col.allText && parts.length == 1 && !isNaN(value = parseFloat(parts[0]))) { filter.type = 2; filter.value = value; filter.text = '= ' + value } else if (col.type == 'text') { filter.type = 0; filter.text = res; if (filter.invert) { filter.regex = g_createOrRegex(res) } else { filter.words = parts } } }
                ...
                if (!col.__filter || filter.text != col.__filter.text || filter.invert != col.__filter.invert) {
                        var a = col.__th.firstChild.firstChild; if (res && filter.text) {
                                if (!col.__filter) { a.className = 'q5'; ++(this.nFilters) }
                                col.__filter = filter
                        } else {
                                if (col.__filter) { a.className = ''; --(this.nFilters) }
                                col.__filter = null
                        }
        ...
        var buffer = ''; buffer += '<b class="q1">' + (col.tooltip ? col.tooltip : col.name) + '</b>'; if (col.__filter) { buffer += '<br />' + $WH.sprintf((col.__filter.invert ? LANG.tooltip_colfilter2 : LANG.tooltip_colfilter1), col.__filter.text) }
        ...
        id: { id: 'id', name: 'ID', width: '5%', value: 'id', compute: function (data, td) { if (data.id) { $WH.ae(td, $WH.ct(data.id)) } } }, date: { id: 'obj-date', name: LANG.added, compute: function (data, td) { if (data.date) { if (data.date <= 86400) { $WH.ae(td, $WH.ct('???')) } else { var added = new Date(data.date * 1000); var elapsed = (g_serverTime - added) / 1000; return g_formatDate(td, elapsed, added, null, !0) } } }, sortFunc: function (a, b, col) { if (a.date == b.date) { return 0 } else if (a.date < b.date) { return -1 } else { return 1 } } }, cost: {
                id: 'cost', name: LANG.cost, getValue: function (row) { if (row.cost) { return (row.cost[2] && row.cost[2][0] ? row.cost[2][0][1] : 0) || (row.cost[1] && row.cost[1][0] ? row.cost[1][0][1] : 0) || row.cost[0] } }, compute: function (row, td) {
                        if (row.cost) {
                                var money = row.cost[0]; var side = null; var items = row.cost[2]; var currency = row.cost[1]; if (row.side != null) { side = row.side } else if (row.react != null) { if (row.react[0] == 1 && row.react[1] == -1) { side = 1 } else if (row.react[0] == -1 && row.react[1] == 1) { side = 2 } }
                                Listview.funcBox.appendMoney(td, money, side, items, currency)
                        }
                }, sortFunc: function (a, b, col) {
                        if (a.cost == null) { return -1 } else if (b.cost == null) { return 1 }
                ...
                id: 'count', name: LANG.count, value: 'count', compute: function (row, td) {
                        if (!(this._totalCount > 0 || row.outof > 0)) { return }
                        ...
                        var buff = row.count; if (row.outof) { buff += ' ' + row.outof }
                        return buff
                }, sortFunc: function (a, b, col) {
                        if (a.count == null) { return -1 } else if (b.count == null) { return 1 }
                        return $WH.strcmp(a.count, b.count)
                }
        }, percent: {
                id: 'percent', name: '%', value: 'percent', compute: function (row, td) {
                        if (row.count <= 0) { return '??' }
                        if (row.pctstack) {
                                var text = ''; var data = eval('(' + row.pctstack + ')'); for (var amt in data) {
                                        var pct = (data[amt] * row.percent) / 100; if (pct >= 1.95) { pct = parseFloat(pct.toFixed(0)) } else if (pct >= 0.195) { pct = parseFloat(pct.toFixed(1)) } else { pct = parseFloat(pct.toFixed(2)) }
                                        text += $WH.sprintf(LANG.stackof_format, amt, pct) + '<br />'
                                }
                                td.className += ' tip'; g_addTooltip(td, text)
                        }
                        var value = parseFloat(row.percent.toFixed(row.percent >= 1.95 ? 1 : 2)); if (row.pctstack) { var sp = $WH.ce('span'); sp.className += ' tip'; $WH.ae(sp, $WH.ct(value)); $WH.ae(td, sp) } else { return value }
                }, getVisibleText: function (row) {
                        if (row.count <= 0) { return '??' }
                        if (row.percent >= 1.95) { return row.percent.toFixed(1) } else { return parseFloat(row.percent.toFixed(2)) }
                }, sortFunc: function (a, b, col) {
                        if (a.count == null) { return -1 } else if (b.count == null) { return 1 }
        ...
        }, stock: { id: 'stock', name: LANG.stock, width: '10%', value: 'stock', compute: function (row, td) { if (row.stock > 0) { return row.stock } else { td.style.fontFamily = 'Verdana, sans-serif'; return String.fromCharCode(8734) } }, getVisibleText: function (row) { if (row.stock > 0) { return row.stock } else { return String.fromCharCode(8734) + ' infinity' } } }, currency: {
                id: 'currency', name: LANG.currency, getValue: function (row) { if (row.currency) { return (row.currency[0] ? row.currency[0][1] : 0) } }, compute: function (row, td) {
                        if (row.currency) {
                                var side = null; if (row.side != null) { side = row.side } else if (row.react != null) { if (row.react[0] == 1 && row.react[1] == -1) { side = 1 } else if (row.react[0] == -1 && row.react[1] == 1) { side = 2 } }
                                Listview.funcBox.appendMoney(td, null, side, null, row.currency)
                        }
                }, sortFunc: function (a, b, col) {
                        if (a.currency == null) { return -1 } else if (b.currency == null) { return 1 }
                        var lena = 0, lenb = 0; $WH.array_walk(a.currency, function (x, _, __, i) { lena += Math.pow(10, i) + x[1] }); $WH.array_walk(b.currency, function (x, _, __, i) { lenb += Math.pow(10, i) + x[1] }); return $WH.strcmp(lena, lenb)
                }
        }, mode: {
                id: 'mode', name: 'Mode', after: 'name', type: 'text', compute: function (row, td) {
                        if (row.modes && row.modes.mode) {
                                if ((row.modes.mode & 120) == 120 || (row.modes.mode & 3) == 3) { return LANG.pr_note_all }
                                return Listview.extraCols.mode.getVisibleText(row)
                        }
                }, getVisibleText: function (row) {
                        var modeNormal = !!(row.modes.mode & 26); var modeHeroic = !!(row.modes.mode & 97); var player10 = !!(row.modes.mode & 40); var player25 = !!(row.modes.mode & 80); var specificPlayers; if (player10 && !player25) { specificPlayers = 10 } else if (player25 && !player10) { specificPlayers = 25 }
                ...
                }, sortFunc: function (a, b, col) { if (a.modes && b.modes) { return -$WH.strcmp(a.modes.mode, b.modes.mode) } }
        }, requires: { id: 'requires', name: LANG.requires, type: 'text', compute: function (item, td) { if (item.achievement && g_achievements[item.achievement]) { $WH.nw(td); td.className = 'small'; td.style.lineHeight = '18px'; var a = $WH.ce('a'); a.href = '?achievement=' + item.achievement; a.className = 'icontiny tinyspecial'; a.style.backgroundImage = 'url(' + g_staticUrl + '/images/wow/icons/tiny/' + g_achievements[item.achievement].icon.toLowerCase() + '.gif)'; a.style.whiteSpace = 'nowrap'; $WH.st(a, g_achievements[item.achievement]['name_' + Locale.getName()]); $WH.ae(td, a) } }, getVisibleText: function (item) { if (item.achievement && g_achievements[item.achievement]) { return g_achievements[item.achievement].name } }, sortFunc: function (a, b, col) { return $WH.strcmp(this.getVisibleText(a), this.getVisibleText(b)) } }, reqskill: { id: 'reqskill', name: LANG.skill, width: '10%', value: 'reqskill', before: 'yield' }, yield: {
                id: 'yield', name: LANG.yields, type: 'text', align: 'left', span: 2, value: 'name', compute: function (row, td, tr) { if (row.yield && g_items[row.yield]) { var i = $WH.ce('td'); i.style.width = '1px'; i.style.padding = '0'; i.style.borderRight = 'none'; $WH.ae(i, g_items.createIcon(row.yield, 1)); $WH.ae(tr, i); td.style.borderLeft = 'none'; var wrapper = $WH.ce('div'); var a = $WH.ce('a'); a.style.fontFamily = 'Verdana, sans-serif'; a.href = '?item=' + row.yield; a.className = 'q' + g_items[row.yield].quality; $WH.ae(a, $WH.ct(g_items[row.yield]['name_' + Locale.getName()])); $WH.ae(wrapper, a); $WH.ae(td, wrapper) } }, getVisibleText: function (row) { if (row.yield && g_items[row.yield]) { return g_items[row.yield]['name_' + Locale.getName()] } }, sortFunc: function (a, b, col) {
                        if (!a.yield || !g_items[a.yield] || !b.yield || !g_items[b.yield]) { return (a.yield && g_items[a.yield] ? 1 : (b.yield && g_items[b.yield] ? -1 : 0)) }
                        return -$WH.strcmp(g_items[a.yield].quality, g_items[b.yield].quality) || $WH.strcmp(g_items[a.yield]['name_' + Locale.getName()], g_items[b.yield]['name_' + Locale.getName()])
                }
        }, condition: {
                id: 'condition', name: LANG.requires, type: 'text', width: '25%', compute: function (row, td) {
                        if (!row.condition)
                                return; td.className = 'small'; td.style.lineHeight = '18px'; var mText = ConditionList.createCell(row.condition); Markup.printHtml(mText, td); return
                }, getVisibleText: function (row) {
                        var
                        buff = '', mText; if (!row.condition)
                                return buff; mText = ConditionList.createCell(row.condition); return Markup.removeTags(mText)
                }, sortFunc: function (a, b, col) {
                        var
                        text1 = this.getVisibleText(a), text2 = this.getVisibleText(b); if (text1 != '' && text2 == '')
                                return -1; if (text2 != '' && text1 == '')
                                return 1; return $WH.strcmp(text1, text2)
                }
        },
}; Listview.funcBox = {
        createSimpleCol: function (i, n, w, v) { return { id: i, name: (LANG[n] !== undefined ? LANG[n] : n), width: w, value: v } }, initLootTable: function (row) {
                var divider; if (this._totalCount != null) { divider = this._totalCount } else { divider = row.outof }
                if (divider == 0) { if (row.count != -1) { row.percent = row.count } else { row.percent = 0 } } else { row.percent = row.count / divider * 100 } (Listview.funcBox.initModeFilter.bind(this, row))()
        }, initModeFilter: function (row) {
                if (this._lootModes == null)
                        this._lootModes = { 99: 0 }; if (this._distinctModes == null)
                        this._distinctModes = { 99: 0 }; if ((!row.modes || row.modes.mode == 4) && row.classs != 12 && row.commondrop) { this._lootModes[99]++; this._distinctModes[99]++ } else if (row.modes) {
                                for (var i = -2; i <= 5; ++i) {
                                ...
                                if (this._distinctModes[row.modes.mode] == null)
                                        this._distinctModes[row.modes.mode] = 0; this._distinctModes[row.modes.mode]++
                        }
        }, addModeIndicator: function () {
                var nModes = 0; for (var i in this._distinctModes) {
                ...
                if (nModes < 2)
                        return; var pm = location.hash.match(/:mode=([^:]+)/), order = [0, -1, -2, 1, 3, 2, 4, 5, 99], langref = { "-2": LANG.tab_heroic, "-1": LANG.tab_normal, 0: LANG.tab_noteworthy, 1: $WH.sprintf(LANG.tab_normalX, 10), 2: $WH.sprintf(LANG.tab_normalX, 25), 3: $WH.sprintf(LANG.tab_heroicX, 10), 4: $WH.sprintf(LANG.tab_heroicX, 25), 5: LANG.tab_raidfinder, 99: '' }; var f = function (mode, dm, updatePound) {
                                g_setSelectedLink(this, 'lootmode'); lv.customPound = lv.id + (dm != null ? ':mode=' + g_urlize(langref[dm].replace(' ', '')) : ''); lv.customFilter = function (item) { return Listview.funcBox.filterMode(item, lv._totalCount, mode) }; lv.updateFilters(1); lv.applySort(); lv.refreshRows(); if (updatePound)
                                        lv.updatePound(1)
                        }; var lv = this, modes = [], a; a = $('<a><span>' + LANG.pr_note_all + '</span></a>'); a[0].f = f.bind(a[0], null, null, 1); a.click(a[0].f); var firstCallback = f.bind(a[0], null, null, 0); firstCallback(); modes.push($('<span class="indicator-mode"></span>').append(a).append($('<b>' + LANG.pr_note_all + '</b>'))); for (var j = 0, len = order.length; j < len; ++j) {
                                var i = order[j]; if (!this._lootModes[i])
                ...
                var showNoteworthy = !1; for (var i = 0, len = modes.length; i < len; ++i) {
                        a = $('a', modes[i]); if (!$('span', a).html() && modes.length == 3)
                                showNoteworthy = !0; else this.createIndicator(modes[i], null, a[0].f)
                }
                if (showNoteworthy)
                        firstCallback(); $(this.noteTop).append($('<div class="clear"></div>'))
                ...
                return (mode != null ? ((!row.modes || row.modes.mode == 4) && row.classs != 12 && row.commondrop ? (mode == 32) : (row.modes && (row.modes.mode & mode))) : !0)
        }, initSubclassFilter: function (row) {
                var i = row.classs || 0; if (this._itemClasses == null)
                        this._itemClasses = {}; if (this._itemClasses[i] == null)
                        this._itemClasses[i] = 0; this._itemClasses[i]++
        }, addSubclassIndicator: function () {
                var it = location.hash.match(/:type=([^:]+)/), order = []; for (var i in g_item_classes)
                        order.push({ i: i, n: g_item_classes[i] }); order.sort(function (a, b) { return $WH.strcmp(a.n, b.n) }); var f = function (itemClass, updatePound) {
                ...
                var lv = this, classes = [], a; a = $('<a><span>' + LANG.pr_note_all + '</span></a>'); a[0].f = f.bind(a[0], null, 1); a.click(a[0].f); var firstCallback = f.bind(a[0], null, 0); firstCallback(); classes.push($('<span class="indicator-mode"></span>').append(a).append($('<b>' + LANG.pr_note_all + '</b>'))); for (var j = 0, len = order.length; j < len; ++j) {
                        var i = order[j].i; if (!this._itemClasses[i])
                ...
                if (classes.length > 2) {
                        for (var i = 0, len = classes.length; i < len; ++i)
                                this.createIndicator(classes[i], null, $('a', classes[i])[0].f); $(this.noteTop).css('padding-bottom', '12px'); $(this.noteIndicators).append($('<div class="clear"></div>')).insertAfter($(this.navTop))
                }
        }, initSpellFilter: function (row) {
                if (this._spellTypes == null) { this._spellTypes = {} }
                if (this._spellTypes[row.cat] == null) { this._spellTypes[row.cat] = 0 }
                this._spellTypes[row.cat]++
        }, addSpellIndicator: function () {
                var it = location.hash.match(/:type=([^:]+)/); var f = function (spellCat, updatePound) { g_setSelectedLink(this, "spellType"); lv.customPound = lv.id + (spellCat != null ? ":type=" + spellCat : ""); lv.customFilter = function (spell) { return spellCat == null || spell.cat == spellCat }; lv.updateFilters(1); lv.applySort(); lv.refreshRows(); if (updatePound) { lv.updatePound(1) } }; var lv = this, categories = [], a; a = $("<a><span>" + LANG.pr_note_all + "</span></a>"); a[0].f = f.bind(a[0], null, 1); a.click(a[0].f); var firstCallback = f.bind(a[0], null, 0); firstCallback(); categories.push($('<span class="indicator-mode"></span>').append(a).append($("<b>" + LANG.pr_note_all + "</b>"))); for (var i in g_spell_categories) {
                        if (!this._spellTypes[i]) { continue }
                ...
                if (categories.length > 2) {
                        for (var i = 0, len = categories.length; i < len; ++i) { this.createIndicator(categories[i], null, $("a", categories[i])[0].f) }
                        $(this.noteTop).css("padding-bottom", "12px"); $(this.noteIndicators).append($('<div class="clear"></div>')).insertAfter($(this.navTop))
                }
        }, initStatisticFilter: function (row) {
                if (this._achievTypes == null)
                        this._achievTypes = {}; if (this._achievTypes[row.type] == null)
                        this._achievTypes[row.type] = 0; this._achievTypes[row.type]++
        }, addStatisticIndicator: function () {
                var it = location.hash.match(/:type=([^:]+)/), order = []; for (var i in g_achievement_types)
                        order.push({ i: i, n: g_achievement_types[i] }); order.sort(function (a, b) { return $WH.strcmp(a.n, b.n) }); var f = function (achievType, updatePound) {
                                g_setSelectedLink(this, 'achievType'); lv.customPound = lv.id + (achievType != null ? ':type=' + achievType : ''); lv.customFilter = function (achievement) { return achievType == null || achievType == achievement.type }; lv.updateFilters(1); lv.applySort(); lv.refreshRows(); if (updatePound)
                                        lv.updatePound(1)
                        }; var lv = this, types = [], a; a = $('<a><span>' + LANG.pr_note_all + '</span></a>'); a[0].f = f.bind(a[0], null, 1); a.click(a[0].f); var firstCallback = f.bind(a[0], null, 0); firstCallback(); types.push($('<span class="indicator-mode"></span>').append(a).append($('<b>' + LANG.pr_note_all + '</b>'))); for (var j = 0, len = order.length; j < len; ++j) {
                                var i = order[j].i; if (!this._achievTypes[i])
                ...
                if (types.length > 2) {
                        for (var i = 0, len = types.length; i < len; ++i)
                                this.createIndicator(types[i], null, $('a', types[i])[0].f); $(this.noteTop).append($('<div class="clear"></div>'))
                }
        }, initQuestFilter: function (row) {
                if (this._questTypes == null)
                        this._questTypes = {}; for (var i = 1; i <= 4; ++i) {
                                if (this._questTypes[i] == null)
                                        this._questTypes[i] = 0; if (row._type && (row._type & 1 << i - 1))
                                        this._questTypes[i]++
                        }
        }, addQuestIndicator: function () {
                var it = location.hash.match(/:type=([^:]+)/); var f = function (questType, updatePound) {
                        g_setSelectedLink(this, 'questType'); lv.customPound = lv.id + (questType != null ? ':type=' + questType : ''); lv.customFilter = function (quest) { return questType == null || (quest._type & 1 << questType - 1) }; lv.updateFilters(1); lv.applySort(); lv.refreshRows(); if (updatePound)
                                lv.updatePound(1)
                }; var lv = this, types = [], a; a = $('<a><span>' + LANG.pr_note_all + '</span></a>'); a[0].f = f.bind(a[0], null, 1); a.click(a[0].f); var firstCallback = f.bind(a[0], null, 0); firstCallback(); types.push($('<span class="indicator-mode"></span>').append(a).append($('<b>' + LANG.pr_note_all + '</b>'))); for (var i = 1; i <= 4; ++i) {
                ...
                if (types.length > 2) {
                        for (var i = 0, len = types.length; i < len; ++i)
                                this.createIndicator(types[i], null, $('a', types[i])[0].f); $(this.noteTop).css('padding-bottom', '12px'); $(this.noteIndicators).append($('<div class="clear"></div>')).insertAfter($(this.navTop))
                }
        }, assocArrCmp: function (a, b, arr) {
                if (a == null) { return -1 } else if (b == null) { return 1 }
                var n = Math.max(a.length, b.length); for (var i = 0; i < n; ++i) {
                        if (a[i] == null) { return -1 } else if (b[i] == null) { return 1 }
                        var res = $WH.strcmp(arr[a[i]], arr[b[i]]); if (res != 0) { return res }
                }
                return 0
        }, assocBinFlags: function (f, arr) {
                var res = []; for (var i in arr) { if (!isNaN(i) && (f & 1 << i - 1)) { res.push(i) } }
                res.sort(function (a, b) { return $WH.strcmp(arr[a], arr[b]) }); return res
        }, location: function (row, td) {
                if (row.location == null) { return -1 }
                for (var i = 0, len = row.location.length; i < len; ++i) {
                        if (i > 0) { $WH.ae(td, $WH.ct(LANG.comma)) }
                        var zoneId = row.location[i]; if (zoneId == -1) { $WH.ae(td, $WH.ct(LANG.ellipsis)) } else { var a = $WH.ce('a'); a.className = 'q1'; a.href = '?zone=' + zoneId; $WH.ae(a, $WH.ct(g_zones[zoneId])); $WH.ae(td, a) }
                }
        }, arrayText: function (arr, lookup) {
                if (arr == null) { return } else if (!$WH.is_array(arr)) { return lookup[arr] }
                var buff = ''; for (var i = 0, len = arr.length; i < len; ++i) {
                        if (i > 0) { buff += ' ' }
                ...
                if (arr != null) {
                        ...
                        for (var i = 0, len = arr.length; i < len; ++i) {
                                var icon; if (arr[i] == null) { icon = $WH.ce('div'); icon.style.width = icon.style.height = '26px' } else {
                                        var
                                ...
                                if (arr.length == 1 && type == 2) {
                                        if (id && g_items[id]) {
                                                $WH.ee(d); var item = g_items[id], a = $WH.ce('a'), sp = $WH.ce('span'); sp.style.paddingTop = '4px'; a.href = '?item=' + id; a.className = 'q' + item.quality + ' icontiny tinyspecial'; a.style.backgroundImage = 'url(' + g_staticUrl + '/images/wow/icons/tiny/' + item.icon.toLowerCase() + '.gif)'; a.style.whiteSpace = 'nowrap'; $WH.st(a, item['name_' + Locale.getName()]); $WH.ae(sp, a); if (num > 1) { $WH.ae(sp, $WH.ct(' (' + num + ')')) }
                                ...
                                } else { icon.style.cssFloat = icon.style.styleFloat = 'left'; $WH.ae(d, icon); d.style.margin = '0 auto'; d.style.textAlign = 'left'; d.style.width = (26 * arr.length) + 'px' }
                        }
                        d2.className = 'clear'; $WH.ae(td, d); $WH.ae(td, d2); return !0
                }
        }, createSocketedIcons: function (sockets, td, gems, match, text) {
                var
                nMatch = 0, d = $WH.ce('div'), d2 = $WH.ce('div'); for (var i = 0, len = sockets.length; i < len; ++i) {
                        var
                ...
                var offset = 0; if (recurrence == -1) {
                        var nextEvent = new Date(fromWhen.getFullYear(), fromWhen.getMonth(), 1, startDate.getHours(), startDate.getMinutes(), startDate.getSeconds()); for (var i = 0; i < 2; ++i) {
                                nextEvent.setDate(1); nextEvent.setMonth(nextEvent.getMonth() + i); var day = nextEvent.getDay(); var tolerance = 1; if (nextEvent.getYear() == 2009) { tolerance = 0 }
                ...
                } else if (recurrence > 0) { recurrence *= 1000; offset = Math.ceil((fromWhen.getTime() - endDate.getTime()) / recurrence) * recurrence }
                startDate.setTime(startDate.getTime() + offset); endDate.setTime(endDate.getTime() + offset); return [startDate, endDate]
        }, createTextRange: function (min, max) {
                min |= 0; max |= 0; if (min > 1 || max > 1) { if (min != max && max > 0) { return min + '-' + max } else { return min + '' } }
                return null
        }, coGetColor: function (comment, mode, blog) {
                if (comment.user && g_customColors[comment.user])
                        return ' comment-' + g_customColors[comment.user]; switch (mode) {
                                case -1: var foo = null; if (!blog)
                ...
                if (comment.roles & U_GROUP_ADMIN)
                        ...
                        return ' comment-gold'; return ''
        }, coFlagUpOfDate: function (comment) {
                var notificationDiv = comment.commentCell.find('.comment-notification'); var god = comment.user == g_user.name || ((g_user.roles & (U_GROUP_ADMIN | U_GROUP_MOD)) != 0); if (!god) { return }
                ...
                var notificationDiv = comment.commentCell.find('.comment-notification'); var god = comment.user == g_user.name || ((g_user.roles & (U_GROUP_ADMIN | U_GROUP_BUREAU)) != 0); if (god) {
                ...
                var reason = null; while (!0) {
                        reason = prompt(LANG.lvcomment_outofdate_tip); if (reason == null || reason == !1) { return } else if (reason.toString().length > 0) { break }
                        alert(LANG.youmustprovideareason_stc)
                }
                $.post('?comment=out-of-date', { id: comment.id, reason: reason }, function (data) { if (data == 'ok') { comment.outofdate = !0; Listview.templates.comment.updateCommentCell(comment); MessageBox(notificationDiv, LANG.lvcomment_outofdateresponsequeued) } else { MessageBox(notificationDiv, 'Error: ' + data) } })
        }, coDelete: function (comment) {
                var god = comment.user == g_user.name || ((g_user.roles & (U_GROUP_ADMIN | U_GROUP_MOD | U_GROUP_BUREAU)) != 0); var notificationDiv = comment.commentCell.find('.comment-notification'); if (god) {
                ...
                if (!comment.commentv2) { $WH.de(comment.divEdit); comment.divEdit = null }
                ...
                if (mode == 2)
                        ...
                        multiplier = 3; switch (mode) { case 0: case 1: return 7500 * multiplier; case -1: case 3: return 15000 * multiplier }
        }, coUpdateCharLimit: function (ta, span, charLimit) {
                var text = $(ta).val(); if (text.length > charLimit)
                        $(ta).val(text.substring(0, charLimit)); else {
                                $(span).html(' ' + $WH.sprintf(LANG.compose_remaining, charLimit - text.length)).removeClass('q10'); if (text.length == charLimit)
                                        $(span).addClass('q10')
                }
        }, coModeLink: function (mode, m, comment) {
                var charLimit = Listview.funcBox.coGetCharLimit(m); var markupMode = Markup.MODE_COMMENT; $WH.array_walk($WH.gE(this.parentNode, 'a'), function (x) { x.className = '' }); this.className = 'selected'; var tas = $WH.gE(this.parentNode.parentNode, 'textarea'), ta = tas[0], taContainer = ta.parentNode, preview = $('.comment-body', taContainer.parentNode)[0]; if (m == 4)
                        markupMode = Markup.MODE_SIGNATURE; switch (mode) {
                                case 1: taContainer.style.display = ''; preview.style.display = 'none'; taContainer.firstChild.focus(); break; case 2: taContainer.style.display = 'none'; var str = (ta.value.length > charLimit ? ta.value.substring(0, charLimit) : ta.value); if (m == 4) {
                                        var foo; if ((foo = str.indexOf('\n')) != -1 && (foo = str.indexOf('\n', foo + 1)) != -1 && (foo = str.indexOf('\n', foo + 1)) != -1)
                                                str = str.substring(0, foo)
                                }
                                        var allowed = Markup.rolesToClass(comment.roles); var html = Markup.toHtml(str, { allow: allowed, mode: markupMode, roles: comment.roles }); if (tas[1] && tas[1].value.length > 0)
                                                html += Markup.toHtml('[div][/div][wowheadresponse=' + g_user.name + ' roles=' + g_user.roles + ']' + tas[1].value + '[/wowheadresponse]', { allow: Markup.CLASS_STAFF, mode: markupMode, roles: g_user.roles }); preview.innerHTML = html; preview.style.display = ''; break
                        }
        }, coValidate: function (ta, mode) {
                mode |= 0; if (mode == 1 || mode == -1) { if ($WH.trim(ta.value).length < 10) { alert(LANG.message_forumposttooshort); return !1 } } else { if ($WH.trim(ta.value).length < 10) { alert(LANG.message_commenttooshort); return !1 } }
                var charLimit = Listview.funcBox.coGetCharLimit(mode); if (ta.value.length >= charLimit) {
                        if (!confirm($WH.sprintf(mode == 1 ? LANG.confirm_forumposttoolong : LANG.confirm_commenttoolong, charLimit, ta.value.substring(charLimit - 30, charLimit))))
                                return !1
                }
                return !0
        }, coSortNewestFirst: function (me) { $WH.sc('comments_sort', 1000, '1', '/', location.hostname); $(me).parent().find('a.selected').removeClass('selected'); me.className = 'selected'; this.mainDiv.className += ' listview-aci'; this.setSort([-5, 4, 6, -1, -2], !0, !1) }, coSortOldestFirst: function (me) { $WH.sc('comments_sort', 1000, '2', '/', location.hostname); $(me).parent().find('a.selected').removeClass('selected'); me.className = 'selected'; this.mainDiv.className += ' listview-aci'; this.setSort([-5, 4, 6, 1, 2], !0, !1) }, coSortHighestRatedFirst: function (me) { $WH.sc('comments_sort', 1000, '3', '/', location.hostname); $(me).parent().find('a.selected').removeClass('selected'); me.className = 'selected'; this.mainDiv.className = this.mainDiv.className.replace('listview-aci', ''); this.setSort([-5, 4, 6, -3, 2], !0, !1) }, coFilterByPatchVersion: function (select) { this.minPatchVersion = select.value; this.refreshRows() }, coUpdateLastEdit: function (comment) {
                var _ = comment.divLastEdit; if (!_) return; if (comment.lastEdit != null) {
                        var __ = comment.lastEdit; _.childNodes[1].firstChild.nodeValue = __[2]; _.childNodes[1].href = '?user=' + __[2]; var editedOn = new Date(__[0]); var elapsed = (g_serverTime - editedOn) / 1000; if (_.childNodes[3].firstChild)
                                ...
                                extraTxt += LANG.dash + $WH.sprintf(LANG.lvcomment_nedits, __[1]); _.childNodes[4].nodeValue = extraTxt; _.style.display = ''
                } else _.style.display = 'none'
        }, coFormatFileSize: function (size) {
                var i = -1; var units = 'KMGTPEZY'; while (size >= 1024 && i < 7) { size /= 1024; ++i }
                if (i < 0)
                        return size + ' byte' + (size > 1 ? 's' : ''); else return size.toFixed(1) + ' ' + units[i] + 'B'
        }, dateEventOver: function (date, event, e) {
                var dates = Listview.funcBox.getEventNextDates(event.startDate, event.endDate, event.rec || 0, date), buff = ''; if (dates[0] && dates[1]) {
                        var
        ...
        }, ssCellOver: function () { this.className = 'screenshot-caption-over' }, ssCellOut: function () { this.className = 'screenshot-caption' }, ssCellClick: function (i, e) {
                e = $WH.$E(e); if (e.shiftKey || e.ctrlKey) { return }
                var j = 0, el = e._target; while (el && j < 3) {
                        if (el.nodeName == 'A') { return }
                ...
                row.__cb = cb; $WH.ae(div, cb); $WH.ae(td, div)
                ...
                if (costCurrency != null) {
                        for (var i = 0; i < costCurrency.length; ++i) {
                                ...
                                _ = $WH.ce('a'); _.href = '?currency=' + currencyId; _.onmousemove = $WH.Tooltip.cursorUpdate; _.onmouseout = $WH.Tooltip.hide; if (currencyId == 103) { _.className = 'moneyarena tip'; _.onmouseover = Listview.funcBox.moneyArenaOver; $WH.ae(_, $WH.ct($WH.number_format(count))) } else if (currencyId == 104) {
                                        if (side == 3 && icon[0] == icon[1]) { side = 1 }
                                        _.className = 'money' + (side == 1 ? 'alliance' : 'horde') + ' tip'; _.onmouseover = Listview.funcBox.moneyHonorOver; if (side == 3) { __ = $WH.ce('span'); __.className = 'moneyalliance'; $WH.ae(__, $WH.ct($WH.number_format(count))); $WH.ae(_, __) } else { $WH.ae(_, $WH.ct($WH.number_format(count))) }
                                } else { _.className = 'icontinyr tip q1'; _.style.backgroundImage = 'url(' + g_staticUrl + '/images/wow/icons/tiny/' + icon[0].toLowerCase() + '.gif)'; _.onmouseover = Listview.funcBox.moneyCurrencyOver.bind(_, currencyId, count); $WH.ae(_, $WH.ct($WH.number_format(count))) }
                                $WH.ae(d, _)
                        }
                }
                if (typeof achievementPoints == 'number') {
                        if (ns) { $WH.ae(d, $WH.ct(' ')) } else { ns = 1 }
                        _ = $WH.ce('span'); _.className = 'moneyachievement tip'; _.onmouseover = Listview.funcBox.moneyAchievementOver; _.onmousemove = $WH.Tooltip.cursorUpdate; _.onmouseout = $WH.Tooltip.hide; $WH.ae(_, $WH.ct($WH.number_format(achievementPoints))); $WH.ae(d, _)
                }
        }, getUpperSource: function (source, sm) {
                switch (source) {
                        case 2: if (sm.bd) { return LANG.source_bossdrop }
                ...
                switch (source) {
                        case 3: if (sm.p && g_sources_pvp[sm.p]) { return { text: g_sources_pvp[sm.p] } }
                                break
                }
                switch (type) {
                        case 0: case 1: case 2: if (sm.z) {
                                var res = { url: '?zone=' + sm.z, text: g_zones[sm.z] }; if (sm.t && source == 5) { res.pretext = LANG.lvitem_vendorin }
                ...
                var str = ''; if (line.expansion == 1) { str += ' bc' } else if (line.expansion == 2) { str += ' wotlk wrath' }
                return str
        }
}; Listview.templates = {
        faction: {
                sort: [1], nItemsPerPage: -1, searchable: 1, filtrable: 1, columns: [{ id: 'name', name: LANG.name, type: 'text', align: 'left', value: 'name', compute: function (faction, td) { var a = $WH.ce('a'); a.style.fontFamily = 'Verdana, sans-serif'; a.href = this.getItemLink(faction); $WH.ae(a, $WH.ct(faction.name)); if (faction.expansion) { var sp = $WH.ce('span'); sp.className = g_GetExpansionClassName(faction.expansion); $WH.ae(sp, a); $WH.ae(td, sp) } else { $WH.ae(td, a) } }, getVisibleText: function (faction) { var buff = faction.name + Listview.funcBox.getExpansionText(faction); return buff } }, { id: 'side', name: LANG.side, type: 'text', compute: function (item, td) { if (item.side && item.side != 3) { var sp = $WH.ce('span'); sp.className = (item.side == 1 ? 'icon-alliance' : 'icon-horde'); g_addTooltip(sp, g_sides[item.side]); $WH.ae(td, sp) } }, getVisibleText: function (item) { if (item.side) { return g_sides[item.side] } }, sortFunc: function (a, b, col) { return $WH.strcmp(g_sides[a.side], g_sides[b.side]) } }, { id: 'standing', name: LANG.reputation, value: 'standing', compute: function (faction, td) { td.style.padding = 0; $WH.ae(td, g_createReputationBar(faction.standing)) }, hidden: 1 }, {
                        id: 'category', name: LANG.category, type: 'text', width: '16%', compute: function (faction, td) {
                                if (faction.category2 != null) {
                                        td.className = 'small q1'; var a = $WH.ce('a'), href = '?factions=' + faction.category2; if (faction.category) { href += '.' + faction.category }
                                        a.href = href; $WH.ae(a, $WH.ct(Listview.funcBox.getFactionCategory(faction.category, faction.category2))); $WH.ae(td, a)
                                }
                        }, getVisibleText: function (faction) { return Listview.funcBox.getFactionCategory(faction.category, faction.category2) }, sortFunc: function (a, b, col) { var _ = Listview.funcBox.getFactionCategory; return $WH.strcmp(_(a.category, a.category2), _(b.category, b.category2)) }
                }], getItemLink: function (faction) { return '?faction=' + faction.id }
        }, item: {
                sort: [-2], searchable: 1, filtrable: 1, columns: [{
                        id: 'name', name: LANG.name, type: 'text', align: 'left', span: 2, value: 'name', compute: function (item, td, tr) {
                                if (item.upgraded) { tr.className = 'upgraded' }
                                ...
                                $WH.ae(a, $WH.ct(item.name.substring(1))); var wrapper = $WH.ce('div'); $WH.ae(wrapper, a); if (item.reqclass) {
                                        var d = $WH.ce('div'); d.className = 'small2'; var classes = Listview.funcBox.assocBinFlags(item.reqclass, g_chr_classes); for (var i = 0, len = classes.length; i < len; ++i) {
                                                if (i > 0) { $WH.ae(d, $WH.ct(', ')) }
                                ...
                                if (typeof fi_nExtraCols == 'number' && fi_nExtraCols >= 5) {
                                        if (item.source != null && item.source.length == 1) {
                                                if (item.reqclass) { $WH.ae(d, $WH.ct(LANG.dash)) } else { var d = $WH.ce('div'); d.className = 'small2' }
                                                var sm = (item.sourcemore ? item.sourcemore[0] : {}); var type = 0; if (sm.t) {
                                                        type = sm.t; var a = $WH.ce('a'); if (sm.q != null) { a.className = 'q' + sm.q } else { a.className = 'q1' }
                                                        a.href = '?' + g_types[sm.t] + '=' + sm.ti; if (sm.n.length <= 30) { $WH.ae(a, $WH.ct(sm.n)) } else { a.title = sm.n; $WH.ae(a, $WH.ct($WH.trim(sm.n.substr(0, 27)) + '...')) }
                                                        $WH.ae(d, a)
                                                } else { $WH.ae(d, $WH.ct(Listview.funcBox.getUpperSource(item.source[0], sm))) }
                                                var ls = Listview.funcBox.getLowerSource(item.source[0], sm, type); if (ls != null) {
                                                        $WH.ae(d, $WH.ct(LANG.hyphen)); if (ls.pretext) { $WH.ae(d, $WH.ct(ls.pretext)) }
                                ...
                                if (item.heroic || item.reqrace) {
                                        wrapper.style.position = 'relative'; var d = $WH.ce('div'); d.className = 'small'; d.style.fontStyle = 'italic'; d.style.position = 'absolute'; d.style.right = d.style.bottom = '3px'; if (item.heroic) { var s = $WH.ce('span'); s.className = 'q2'; $WH.ae(s, $WH.ct(LANG.lvitem_heroicitem)); $WH.ae(d, s) }
                                        if (item.reqrace) {
                                                if ((item.reqrace & 1791) != 1101 && (item.reqrace & 1791) != 690) {
                                                        if (item.heroic) { $WH.ae(d, $WH.ce('br')); d.style.bottom = '-6px' }
                                                        var races = Listview.funcBox.assocBinFlags(item.reqrace, g_chr_races); for (var i = 0, len = races.length; i < len; ++i) {
                                                                if (i > 0) { $WH.ae(d, $WH.ct(', ')) }
                                ...
                                var buff = item.name.substring(1); if (item.heroic) { buff += ' ' + LANG.lvitem_heroicitem }
                                ...
                                if (typeof fi_nExtraCols == 'number' && fi_nExtraCols >= 5) {
                                        if (item.source != null && item.source.length == 1) {
                                                var sm = (item.sourcemore ? item.sourcemore[0] : {}); var type = 0; if (sm.t) { type = sm.t; buff += ' ' + sm.n } else { buff += ' ' + Listview.funcBox.getUpperSource(item.source[0], sm) }
                                                var ls = Listview.funcBox.getLowerSource(item.source[0], sm, type); if (ls != null) {
                                                        if (ls.pretext) { buff += ' ' + ls.pretext }
                        ...
                        id: 'source', name: LANG.source, type: 'text', compute: function (item, td) {
                                ...
                                if (item.source != null) {
                                        if (item.source.length == 1) {
                                                $WH.nw(td); var sm = (item.sourcemore ? item.sourcemore[0] : {}); var type = 0; if (sm.t) {
                                                        ...
                                                        $WH.ae(a, $WH.ct(sm.n)); $WH.ae(td, a)
                                                } else { $WH.ae(td, $WH.ct(Listview.funcBox.getUpperSource(item.source[0], sm))) }
                                                var ls = Listview.funcBox.getLowerSource(item.source[0], sm, type); if (this.iconSize != 0 && ls != null) {
                                                        var div = $WH.ce('div'); div.className = 'small2'; if (ls.pretext) { $WH.ae(div, $WH.ct(ls.pretext)) }
                                                ...
                                                var buff = ''; for (var i = 0, len = item.source.length; i < len; ++i) {
                                                        if (i > 0) { buff += LANG.comma }
                                ...
                                if (item.source != null) {
                                        if (item.source.length == 1) {
                                                var buff = ''; var sm = (item.sourcemore ? item.sourcemore[0] : {}); var type = 0; if (sm.t) { type = sm.t; buff += ' ' + sm.n } else { buff += ' ' + Listview.funcBox.getUpperSource(item.source[0], sm) }
                                                var ls = Listview.funcBox.getLowerSource(item.source[0], sm, type); if (ls != null) {
                                                        if (ls.pretext) { buff += ' ' + ls.pretext }
                                                        buff += ' ' + ls.text; if (ls.posttext) { buff += ' ' + ls.posttext }
                                                }
                                                return buff
                                        } else { return Listview.funcBox.arrayText(item.source, g_sources) }
                                }
                        }, sortFunc: function (a, b) {
                                var res = Listview.funcBox.assocArrCmp(a.source, b.source, g_sources); if (res != 0) { return res }
                                var na = (a.sourcemore && a.source.length == 1 ? a.sourcemore[0].n : null), nb = (b.sourcemore && b.source.length == 1 ? b.sourcemore[0].n : null); return $WH.strcmp(na, nb)
                        }
                }, {
                        id: 'type', name: LANG.type, type: 'text', compute: function (item, td) {
                                if (item.classs == null) { return }
                                td.className = 'small q1'; $WH.nw(td); var a = $WH.ce('a'); var it = Listview.funcBox.getItemType(item.classs, item.subclass, item.subsubclass); a.href = it.url; $WH.ae(a, $WH.ct(it.text)); $WH.ae(td, a)
                        }, getVisibleText: function (item) { return Listview.funcBox.getItemType(item.classs, item.subclass, item.subsubclass).text }, sortFunc: function (a, b, col) { var _ = Listview.funcBox.getItemType; return $WH.strcmp(_(a.classs, a.subclass, a.subsubclass).text, _(b.classs, b.subclass, b.subsubclass).text) }
                }], getItemLink: function (item) { return item.name.charAt(0) == '@' ? 'javascript:;' : '?item=' + item.id }, onBeforeCreate: function () {
                        var nComparable = !1; for (var i = 0, len = this.data.length; i < len; ++i) { var item = this.data[i]; if ((item.slot > 0 && item.slot != 18) || (item.classs == 3 && item.subclass != 7) || ($WH.in_array(ModelViewer.validSlots, item.slotbak) >= 0 && item.displayid > 0) || item.modelviewer) { ++nComparable } else { item.__nochk = 1 } }
                        if (nComparable > 0) { this.mode = Listview.MODE_CHECKBOX; this._nComparable = nComparable }
                }, createCbControls: function (div, topBar) {
                        if (!topBar && this._nComparable < 15) { return }
                        ...
                        var rows = this.getCheckedRows(); if (!rows.length) { return }
                        var data = ''; $WH.array_walk(rows, function (x) {
                                if (x.slot == 0 || x.slot == 18) { return }
                                data += x.id + ';'
                        }); su_addToSaved($WH.rtrim(data, ';'), rows.length)
                }, viewIn3d: function () {
                        var rows = this.getCheckedRows(); if (!rows.length) { return }
                        var hasData = !1, repeatData = !1, badData = !1; var data = {}; var model = null; $WH.array_walk(rows, function (x) {
                                if ($WH.in_array(ModelViewer.validSlots, x.slotbak) >= 0 && x.displayid > 0) {
                                        var slot = ModelViewer.slotMap[x.slotbak]; if (data[slot]) { repeatData = !0 }
                                        data[slot] = x.displayid; hasData = !0
                                } else if (x.modelviewer) { model = x.modelviewer } else { badData = !0 }
                        }); var message = null; if (model) {
                                if (hasData || badData) { message = LANG.dialog_cantdisplay }
                                ModelViewer.show({ type: model.type, displayId: model.displayid, slot: model.slot, message: message })
                        } else {
                                if (repeatData || badData) { message = LANG.dialog_cantdisplay }
                                var equipList = []; for (var i in data) { equipList.push(parseInt(i)); equipList.push(data[i]) }
                                if (equipList.length > 0) { ModelViewer.show({ type: 4, equipList: equipList, message: message }) } else { alert(LANG.message_nothingtoviewin3d) }
                        }
                ...
                sort: [1], nItemsPerPage: 75, searchable: 1, filtrable: 1, columns: [{
                        ...
                        id: 'classes', name: LANG.classes, type: 'text', width: '20%', getVisibleText: function (itemSet) {
                                ...
                                if (itemSet.reqclass) {
                                        var classes = Listview.funcBox.assocBinFlags(itemSet.reqclass, g_chr_classes); var d = $WH.ce('div'); d.style.width = (26 * classes.length) + 'px'; d.style.margin = '0 auto'; for (var i = 0, len = classes.length; i < len; ++i) { var icon = Icon.create('class_' + g_file_classes[classes[i]], 0, null, '?class=' + classes[i]); icon.style.cssFloat = icon.style.styleFloat = 'left'; g_addTooltip(icon, g_chr_classes[classes[i]], 'c' + classes[i]); $WH.ae(d, icon) }
                                        $WH.ae(td, d)
                                }
                        }, sortFunc: function (a, b, col) { return Listview.funcBox.assocArrCmp(Listview.funcBox.assocBinFlags(a.reqclass, g_chr_classes), Listview.funcBox.assocBinFlags(b.reqclass, g_chr_classes), g_chr_classes) }
                }], getItemLink: function (itemSet) { return '?itemset=' + itemSet.id }
        }, npc: {
                sort: [1], nItemsPerPage: 100, searchable: 1, filtrable: 1, columns: [{
                        id: 'name', name: LANG.name, type: 'text', align: 'left', value: 'name', compute: function (npc, td) {
                                var wrapper = $WH.ce('div'); if (npc.boss) { wrapper.className = 'icon-boss-padded' }
                                ...
                                var buff = npc.name; if (npc.tag) { buff += ' <' + npc.tag + '>' }
                                if (npc.boss) { buff += ' boss skull' }
                                return buff
                        }, sortFunc: function (a, b, col) { return $WH.strcmp(b.boss, a.boss) || $WH.strcmp(a.name, b.name) || $WH.strcmp(a.hasQuests, b.hasQuests) }
                }, {
                        id: 'level', name: LANG.level, type: 'range', width: '10%', getMinValue: function (npc) { return npc.minlevel }, getMaxValue: function (npc) { return npc.maxlevel }, compute: function (npc, td) {
                                if (npc.classification) { var d = $WH.ce('div'); d.className = 'small'; $WH.ae(d, $WH.ct(g_npc_classifications[npc.classification])); $WH.ae(td, d) }
                                ...
                                var buff = ''; if (npc.classification) { buff += ' ' + g_npc_classifications[npc.classification] }
                                ...
                                return buff
                        }, sortFunc: function (a, b, col) { if (col > 0) { return $WH.strcmp(a.minlevel, b.minlevel) || $WH.strcmp(a.maxlevel, b.maxlevel) || $WH.strcmp(a.classification, b.classification) } else { return $WH.strcmp(a.maxlevel, b.maxlevel) || $WH.strcmp(a.minlevel, b.minlevel) || $WH.strcmp(a.classification, b.classification) } }
                }, { id: 'location', name: LANG.location, type: 'text', compute: function (npc, td) { return Listview.funcBox.location(npc, td) }, getVisibleText: function (npc) { return Listview.funcBox.arrayText(npc.location, g_zones) }, sortFunc: function (a, b, col) { return Listview.funcBox.assocArrCmp(a.location, b.location, g_zones) } }, {
                        id: 'react', name: LANG.react, type: 'text', width: '10%', value: 'react', filtrable: 0, compute: function (npc, td) {
                                if (npc.react == null) { return -1 }
                                var sides = [LANG.lvnpc_alliance, LANG.lvnpc_horde]; var c = 0; for (var k = 0; k < 2; ++k) {
                                        if (npc.react[k] != null) {
                                                if (c++ > 0) { $WH.ae(td, $WH.ct(' ')) }
                                                var sp = $WH.ce('span'); sp.className = (npc.react[k] < 0 ? 'q10' : (npc.react[k] > 0 ? 'q2' : 'q')); $WH.ae(sp, $WH.ct(sides[k])); $WH.ae(td, sp)
                                        }
                                }
                        }
                ...
                sort: [1], nItemsPerPage: 100, searchable: 1, filtrable: 1, columns: [{
                        id: 'name', name: LANG.name, type: 'text', align: 'left', value: 'name', compute: function (object, td) {
                                var wrapper = $WH.ce('div'); var a = $WH.ce('a'); a.style.fontFamily = 'Verdana, sans-serif'; a.href = this.getItemLink(object); $WH.ae(a, $WH.ct(object.name)); if (object.hasQuests != null) { a.className += " icontiny tinyspecial"; a.style.backgroundImage = "url(" + g_staticUrl + "/images/wow/icons/tiny/quest_start.gif)" }
                                if (g_user.roles & U_GROUP_PREMIUM_PERMISSIONS)
                ...
                }, { id: 'location', name: LANG.location, type: 'text', compute: function (object, td) { return Listview.funcBox.location(object, td) }, getVisibleText: function (object) { return Listview.funcBox.arrayText(object.location, g_zones) }, sortFunc: function (a, b, col) { return Listview.funcBox.assocArrCmp(a.location, b.location, g_zones) } }, { id: 'skill', name: LANG.skill, width: '10%', value: 'skill', hidden: !0 }, { id: 'type', name: LANG.type, type: 'text', width: '12%', compute: function (object, td) { td.className = 'small q1'; var a = $WH.ce('a'); a.href = '?objects=' + object.type; $WH.ae(a, $WH.ct(g_object_types[object.type])); $WH.ae(td, a) }, getVisibleText: function (object) { return g_object_types[object.type] }, sortFunc: function (a, b, col) { return $WH.strcmp(g_object_types[a.type], g_object_types[b.type]) } }], getItemLink: function (object) { return '?object=' + object.id }
        }, quest: {
                sort: [1, 2], nItemsPerPage: 100, searchable: 1, filtrable: 1, columns: [{
                        id: 'name', name: LANG.name, type: 'text', align: 'left', value: 'name', compute: function (quest, td) {
                                var wrapper = $WH.ce('div'); var a = $WH.ce('a'); a.style.fontFamily = 'Verdana, sans-serif'; a.href = this.getItemLink(quest); $WH.ae(a, $WH.ct(quest.name)); $WH.ae(wrapper, a); if (quest.reqclass) {
                                        var d = $WH.ce('div'); d.className += ' small2'; var classes = Listview.funcBox.assocBinFlags(quest.reqclass, g_chr_classes); for (var i = 0, len = classes.length; i < len; ++i) {
                                ...
                                if (quest.wflags & 1 || (quest.wflags & 32) || (quest.reqrace && quest.reqrace != -1)) {
                                        wrapper.style.position = 'relative'; var d = $WH.ce('div'); d.className = 'small'; d.style.fontStyle = 'italic'; d.style.position = 'absolute'; d.style.right = '3px'; d.style.bottom = '3px'; d.style.textAlign = 'right'; if (quest.wflags & 1) { var s = $WH.ce('span'); s.style.color = 'red'; $WH.ae(s, $WH.ct(LANG.lvquest_disabled)); $WH.ae(d, s) }
                                        if (quest.wflags & 32) {
                                                if (quest.wflags & 1) { $WH.ae(d, $WH.ce('br')); wrapper.style.height = '33px' }
                                        ...
                                        if (quest.reqrace && quest.reqrace != -1) {
                                                var races = Listview.funcBox.assocBinFlags(quest.reqrace, g_chr_races); if (races.length && (quest.wflags & 1 || (quest.wflags & 32))) { $WH.ae(d, $WH.ce('br')); wrapper.style.height = '33px' }
                        ...
                        id: 'rewards', name: LANG.rewards, compute: function (quest, td) {
                                ...
                                if (quest.titlereward && g_titles[quest.titlereward]) { var title = g_titles[quest.titlereward]['name_' + Locale.getName()]; title = title.replace('%s', '<span class="q0">&lt;' + LANG.name + '&gt;</span>'); var span = $WH.ce('a'); span.className = 'q1'; span.href = '?title=' + quest.titlereward; span.innerHTML = title; $WH.ae(td, span); $WH.ae(td, $WH.ce('br')) }
                        }, getVisibleText: function (quest) {
                                var buff = ''; if (quest.itemchoices && quest.itemchoices.length) { buff += ' ' + LANG.lvquest_pickone; if (quest.itemrewards && quest.itemrewards.length) { buff += ' ' + LANG.lvquest_alsoget } }
                                if (quest.titlereward && g_titles[quest.titlereward]) { buff += ' ' + g_titles[quest.titlereward]['name_' + Locale.getName()] }
                                return buff
                        }, sortFunc: function (a, b, col) { var lenA = (a.itemchoices != null ? a.itemchoices.length : 0) + (a.itemrewards != null ? a.itemrewards.length : 0); var lenB = (b.itemchoices != null ? b.itemchoices.length : 0) + (b.itemrewards != null ? b.itemrewards.length : 0); var titleA = (a.titlereward && g_titles[a.titlereward] ? g_titles[a.titlereward]['name_' + Locale.getName()] : ''); var titleB = (b.titlereward && g_titles[b.titlereward] ? g_titles[b.titlereward]['name_' + Locale.getName()] : ''); return $WH.strcmp(lenA, lenB) || $WH.strcmp(titleA, titleB) }
                }, { id: 'experience', name: LANG.exp, value: 'xp' }, {
                        id: 'money', name: LANG.money, compute: function (quest, td) {
                                if (quest.money > 0 || quest.currencyrewards != null) {
                                        if (quest.money > 0) { Listview.funcBox.appendMoney(td, quest.money); if (quest.currencyrewards != null) { $WH.ae(td, $WH.ct(' + ')) } }
                                        if (quest.currencyrewards != null) { Listview.funcBox.appendMoney(td, null, quest.side, null, quest.currencyrewards) }
                                }
                        }, getVisibleText: function (quest) {
                                var buff = ''; for (var i = 0; quest.currencyrewards && i < quest.currencyrewards.length; ++i) { if (g_gatheredcurrencies[quest.currencyrewards[i][0]]) { buff += ' ' + g_gatheredcurrencies[quest.currencyrewards[i][0]]['name_' + Locale.getName()] } }
                                return buff
                        }, sortFunc: function (a, b, col) {
                                var
                ...
                }, { id: 'reputation', name: LANG.reputation, width: '14%', value: 'id', hidden: !0 }, { id: 'category', name: LANG.category, type: 'text', compute: function (quest, td) { if (quest.category != 0) { td.className = 'small q1'; var a = $WH.ce('a'); a.href = '?quests=' + quest.category2 + '.' + quest.category; $WH.ae(a, $WH.ct(Listview.funcBox.getQuestCategory(quest.category))); $WH.ae(td, a) } }, getVisibleText: function (quest) { return Listview.funcBox.getQuestCategory(quest.category) }, sortFunc: function (a, b, col) { var _ = Listview.funcBox.getQuestCategory; return $WH.strcmp(_(a.category), _(b.category)) } }], getItemLink: function (quest) { return '?quest=' + quest.id }
        }, reputationhistory: {
                sort: ['when'], searchable: 0, filterable: 0, columns: [{
                        id: 'action', name: LANG.reputationaction, type: 'text', align: 'left', value: 'title', compute: function (rep, td) {
                                if (rep.param && rep.param > 0) {
                                        var link = $('<a>').attr('href', '?go-to-comment&id=' + rep.param).text(l_reputation_names[rep.action]); if (rep.action == 11 || rep.action == 12 || rep.action == 13) { link.attr('href', '?website-achievement=' + rep.param + '#user:' + g_user.name) }
                                        $(td).append(link)
                                } else { $(td).append(l_reputation_names[rep.action]) }
                                return
                        }, getVisibleText: function (rep) { return l_reputation_names[rep.action] }
                }, {
                        id: 'amount', name: LANG.amount, type: 'text', width: '250px', compute: function (rep, td) {
                                var span = $('<span>').text(rep.amount); if (rep.amount > 0) { span.addClass('q2').prepend('+') } else if (rep.amount < 0) { span.addClass('q10') }
                                $(td).append(span); return
                        }, getVisibleText: function (rep) { return rep.amount }, sortFunc: function (a, b) {
                                if (a.when == b.when) { return 0 }
                                return a.when < b.when ? 1 : -1
                        }
                }, {
                        id: 'when', name: LANG.date, type: 'text', width: '250px', compute: function (rep, td) {
                                var
                                postedOn = new Date(rep.when), elapsed = (g_serverTime - postedOn) / 1000; a = $WH.ce('span'); g_formatDate(a, elapsed, postedOn); $WH.ae(td, a)
                        }, sortFunc: function (a, b) {
                                if (a.when == b.when) { return 0 }
                                return a.when < b.when ? 1 : -1
                        }
                ...
                sort: [1], mode: Listview.MODE_FLEXGRID, clickable: !1, nItemsPerPage: 150, cellMinWidth: 85, poundable: 1, sortOptions: [{ id: 'name', name: LANG.name, sortFunc: function (f, c) { return $WH.stringCompare(f.name, c.name) } }], columns: [], value: 'name', compute: function (_icon, td, tr) {
                        var cell = $WH.ce('div'); cell.className = 'icon-cell'; $(cell).mouseenter(function () { setTimeout((function () { this.className += ' animate' }).bind(this), 1) }).mouseleave(function () { this.className = this.className.replace(/ *animate\b/, '') }); $WH.ae(cell, Icon.create(_icon.icon, 2, null, this.getItemLink(_icon))); var overlay = $WH.ce('div', { className: 'icon-cell-overlay' }); $(overlay).mouseleave(function () { $('.fa-check', this).removeClass('fa-check').addClass('fa-clipboard') }); $WH.ae(overlay, Icon.create(_icon.icon, 2, null, this.getItemLink(_icon))); var ovlName = $WH.ce('div', { className: 'icon-cell-overlay-name' }); var o = function () { this.focus(); this.select() }; $WH.ae(ovlName, $WH.ce('input', { type: 'text', value: _icon.name, onclick: o, onfocus: o })); $WH.ae(ovlName, $WH.ce('input', {
                                type: 'button', className: 'button-copy', onclick: function () {
                                        var btn = $(this); var iName = btn.siblings('input[type="text"]'); iName.focus().select(); var error = !1; try {
                                                        if (!document.execCommand('copy'))
                                                                error = !0
                                                } catch (x) { error = !0 }
                                        iName.blur(); if (error) { btn.css({ 'pointer-events': 'none', 'background-image': 'url(' + g_staticUrl + '/images/icons/report.png)' }).blur(); setTimeout(function () { $('.icon-cell-overlay-name .button-copy').fadeOut(1000) }, 600) } else btn.css('background-image', 'url(' + g_staticUrl + '/images/icons/tick.png)')
                                }
                        })); $WH.ae(overlay, ovlName); var t = $WH.ce('div', { className: 'icon-cell-overlay-counts' }); var types = [3, 6, 10, 17, 9, 13]; var c = 0; for (var h = 0, m; m = types[h]; h++) { var p = g_types[m] + 'count'; if (_icon[p]) { c += _icon[p]; var g = g_types[m]; var s = _icon[p] == 1 ? LANG.types[m][1] : LANG.types[m][3]; $WH.ae(t, $WH.ce('div', null, $WH.ce('a', { href: this.getItemLink(_icon) + '#used-by-' + g, innerHTML: $WH.sprintf(LANG.entitycount, _icon[p], s) }))) } }
                        if (!c)
                                $WH.ae(t, $WH.ce('div', { innerHTML: LANG.unused })); $WH.ae(overlay, t); var k = $WH.ce('div', { className: 'icon-cell-overlay-placer' }, overlay); $WH.ae(cell, k); $WH.ae(td, cell)
                }, sortFunc: function (a, b) { return $WH.stringCompare(a.name, b.name) }, getItemLink: function (icon) { return "?icon=" + icon.id }
        }, topusers: {
                sort: ['reputation'], searchable: 1, filtrable: 0, columns: [{
                        id: 'username', name: LANG.username, type: 'text', align: 'left', compute: function (user, td) {
                                var a = $('<a>'); var color = g_GetStaffColorFromRoles(user.groups); if (color != '')
                                        a.addClass(color); else a.css('color', 'white'); a.text(user.username); a.addClass('listview-cleartext'); a.attr('href', '?user=' + user.username); $(td).append(a); return
                        }, getVisibleText: function (user) { return user.username }, sortFunc: function (a, b) { return $WH.stringCompare(a.username, b.username) }, getItemLink: function (user) { return '?user=' + user.username }
                }, {
                        id: 'reputation', name: LANG.reputation, type: 'text', compute: function (user, td) { $(td).append($WH.number_format(user.reputation)); return }, sortFunc: function (a, b) {
                                if (b.reputation == a.reputation)
                                        return 0; return a.reputation < b.reputation ? 1 : -1
                        }
                }, {
                        id: 'achievements', name: LANG.achievements, type: 'text', compute: function (user, td) {
                                var sp = $('<span>').addClass('wsach-pts').css('font-size', 'inherit'); var buf = ''; if (user.gold)
                                        buf += '<i>' + user.gold + '</i>&middot;'; if (user.silver)
                                        buf += '<b>' + user.silver + '</b>&middot;'; buf += '<u>' + user.copper + '</u>'; sp.html(buf); $(td).append(sp); return
                        }, sortFunc: function (a, b) {
                                var sumA = (a.gold * 1000 * 1000) + (a.silver * 1000) + a.copper; var sumB = (b.gold * 1000 * 1000) + (b.silver * 1000) + b.copper; if (sumA == sumB)
                                        return 0; return sumA < sumB ? 1 : -1
                        }
                }, {
                        id: 'comments', name: LANG.comments, type: 'text', compute: function (user, td) { $(td).append($WH.number_format(user.comments)); return }, sortFunc: function (a, b) {
                                if (a.comments == b.comments)
                                        return 0; return a.comments < b.comments ? 1 : -1
                        }
                }, {
                        id: 'posts', name: LANG.posts, type: 'text', compute: function (user, td) { $(td).append($WH.number_format(user.posts)); return }, sortFunc: function (a, b) {
                                if (a.posts == b.posts)
                                        return 0; return a.posts < b.posts ? 1 : -1
                        }
                }, {
                        id: 'screenshots', name: LANG.screenshots, type: 'text', compute: function (user, td) { $(td).append($WH.number_format(user.screenshots)); return }, sortFunc: function (a, b) {
                                if (a.screenshots == b.screenshots)
                                        return 0; return a.screenshots < b.screenshots ? 1 : -1
                        }
                }, {
                        id: 'reports', name: LANG.reports, type: 'text', compute: function (user, td) { $(td).append($WH.number_format(user.reports)); return }, sortFunc: function (a, b) {
                                if (a.reports == b.reports)
                                        return 0; return a.reports < b.reports ? 1 : -1
                        }
                }, {
                        id: 'votes', name: LANG.votes, type: 'text', compute: function (user, td) { $(td).append($WH.number_format(user.votes)); return }, sortFunc: function (a, b) {
                                if (a.votes == b.votes)
                                        return 0; return a.votes < b.votes ? 1 : -1
                        }
                }, {
                        id: 'uploads', name: LANG.uploads, type: 'text', compute: function (user, c) { $(c).append($WH.number_format(user.uploads)); return }, sortFunc: function (a, c) {
                                if (a.uploads == c.uploads)
                                        return 0; return a.uploads < c.uploads ? 1 : -1
                        }
                }, {
                        id: 'created', name: LANG.created, type: 'text', hidden: 1, compute: function (user, td) { var date = new Date(user.creation), diff = (g_serverTime - date) / 1000; sp = $WH.ce('span'); g_formatDate(sp, diff, date); $WH.ae(td, sp) }, sortFunc: function (a, b) {
                                if (a.creation == b.creation)
                                        return 0; return a.creation < b.creation ? 1 : -1
                        }
                }], getItemLink: function (user) { return '?user=' + user.username }
        }, skill: {
                sort: [1], searchable: 1, filtrable: 1, columns: [{
                        id: 'name', name: LANG.name, type: 'text', align: 'left', value: 'name', span: 2, compute: function (skill, td, tr) {
                                var i = $WH.ce('td'); i.style.width = '1px'; i.style.padding = '0'; i.style.borderRight = 'none'; $WH.ae(i, Icon.create(skill.icon, 0, null, this.getItemLink(skill))); $WH.ae(tr, i); td.style.borderLeft = 'none'; var wrapper = $WH.ce('div'); var a = $WH.ce('a'); a.style.fontFamily = 'Verdana, sans-serif'; a.href = this.getItemLink(skill); $WH.ae(a, $WH.ct(skill.name)); if (skill.expansion) { var sp = $WH.ce('span'); sp.className = g_GetExpansionClassName(skill.expansion); $WH.ae(sp, a); $WH.ae(wrapper, sp) } else { $WH.ae(wrapper, a) }
                                $WH.ae(td, wrapper)
                        }, getVisibleText: function (skill) { var buff = skill.name + Listview.funcBox.getExpansionText(skill); return buff }
                ...
                sort: ['name', 'skill', 'level'], searchable: 1, filtrable: 1, columns: [{
                        id: 'name', name: LANG.name, type: 'text', align: 'left', span: 2, value: 'name', compute: function (spell, td, tr) {
                                var
                                ...
                                if (this.showRecipeClass && spell.reqclass) {
                                        var d = $WH.ce('div'); d.className = 'small2'; var classes = Listview.funcBox.assocBinFlags(spell.reqclass, g_chr_classes); for (var i = 0, len = classes.length; i < len; ++i) {
                                                if (i > 0) { $WH.ae(d, $WH.ct(', ')) }
                                                var a = $WH.ce('a'); a.href = '?class=' + classes[i]; a.className = 'c' + classes[i]; $WH.st(a, g_chr_classes[classes[i]]); $WH.ae(d, a)
                                        }
                                        $WH.ae(wrapper, d)
                                ...
                                if (spell.reqrace) {
                                        wrapper.style.position = 'relative'; var d = $WH.ce('div'); d.className = 'small'; d.style.fontStyle = 'italic'; d.style.position = 'absolute'; d.style.right = d.style.bottom = '3px'; if ((spell.reqrace & 1791) == 1101) { $WH.ae(d, $WH.ct(g_sides[1])) } else if ((spell.reqrace & 1791) == 690) { $WH.ae(d, $WH.ct(g_sides[2])) } else {
                                                var races = Listview.funcBox.assocBinFlags(spell.reqrace, g_chr_races); d.className += ' q1'; for (var i = 0, len = races.length; i < len; ++i) {
                                                        if (i > 0) { $WH.ae(d, $WH.ct(LANG.comma)) }
                                                        var a = $WH.ce('a'); a.href = '?race=' + races[i]; $WH.st(a, g_chr_races[races[i]]); $WH.ae(d, a)
                                                }
                                        }
                                ...
                                var buff = spell.name; if (spell.rank) { buff += ' ' + spell.rank }
                                if (spell.reqclass) {
                                        var classes = Listview.funcBox.assocBinFlags(spell.reqclass, g_chr_classes); for (var i = 0, len = classes.length; i < len; ++i) {
                                                if (i > 0) { buff += LANG.comma }
                        ...
                        id: 'tier', name: LANG.tier, width: '10%', value: 'level', compute: function (spell, td) {
                                if (spell.level > 0) {
                                        var
                                        baseLevel = (!this._petTalents ? 10 : 20), tierPoints = (!this._petTalents ? 5 : 12); return Math.floor((spell.level - baseLevel) / tierPoints) + 1
                                } else { return 1 }
                        }, hidden: !0
                }, { id: 'level', name: LANG.level, width: '10%', value: 'level', compute: function (spell, td) { if (spell.level > 0) { return spell.level } }, hidden: !0 }, {
                        id: 'trainingcost', name: LANG.cost, width: '10%', hidden: !0, getValue: function (row) { if (row.trainingcost) { return row.trainingcost } }, compute: function (row, td) { if (row.trainingcost) { Listview.funcBox.appendMoney(td, row.trainingcost) } }, sortFunc: function (a, b, col) {
                                if (a.trainingcost == null) { return -1 } else if (b.trainingcost == null) { return 1 }
                                if (a.trainingcost < b.trainingcost) { return -1 } else if (a.trainingcost > b.trainingcost) { return 1 }
                                return 0
                        }
                }, {
                        id: 'classes', name: LANG.classes, type: 'text', hidden: !0, width: '20%', getVisibleText: function (spell) {
                                var str = ''; if (spell.reqclass) {
                                        var classes = Listview.funcBox.assocBinFlags(spell.reqclass, g_chr_classes); for (var i = 0, len = classes.length; i < len; ++i) {
                                                if (i > 0) { str += LANG.comma }
                                                str += g_chr_classes[classes[i]]
                                        }
                ...
                }, { id: 'singleclass', name: LANG.classs, type: 'text', hidden: !0, width: '15%', compute: function (spell, td) { if (spell.reqclass) { var classes = Listview.funcBox.assocBinFlags(spell.reqclass, g_chr_classes); var classId = classes[0]; var text = $WH.ce('a'); text.style.backgroundImage = 'url("' + g_staticUrl + '/images/wow/icons/tiny/class_' + g_file_classes[classId] + '.gif")'; text.className = 'icontiny tinyspecial c' + classId; text.href = '?class=' + classId; $WH.ae(text, $WH.ct(g_chr_classes[classId])); $WH.ae(td, text) } }, sortFunc: function (a, b, col) { return Listview.funcBox.assocArrCmp(Listview.funcBox.assocBinFlags(a.reqclass, g_chr_classes), Listview.funcBox.assocBinFlags(b.reqclass, g_chr_classes), g_chr_classes) } }, { id: 'glyphtype', name: LANG.glyphtype, type: 'text', hidden: !0, width: '10%', compute: function (spell, td) { if (spell.glyphtype) { return g_item_glyphs[spell.glyphtype] } } }, {
                        id: 'schools', name: LANG.school, type: 'text', width: '10%', hidden: !0, compute: function (spell, td) {
                                var text = ''; var schools = spell.schools ? spell.schools : spell.school; for (var i = 0; i < 32; ++i) {
                                        if (!(schools & (1 << i))) { continue }
                                        if (text != '') { text += ', ' }
                                        text += g_spell_resistances[i]
                                }
                                return text
                        }, sortFunc: function (a, b, col) { return $WH.strcmp(this.compute(a), this.compute(b)) }
                }, {
                        id: 'type', name: LANG.type, type: 'text', width: '14%', hidden: !0, compute: function (spell, td) {
                                var d = $WH.ce('div'); d.className = 'small'; if ($WH.in_array([7, -2, -7, -12, -11, -13], spell.cat) != -1) {
                                        var a1 = $WH.ce('a'); a1.className = 'q0'; a1.href = '?spells=' + spell.cat; $WH.ae(a1, $WH.ct(g_spell_categories[spell.cat])); $WH.ae(d, a1); $WH.ae(d, $WH.ce('br')); if (spell.talentspec) {
                                                for (var i = 0, len = spell.talentspec.length; i < len; ++i) {
                                                        if (i > 0) { $WH.ae(d, $WH.ct(LANG.comma)) }
                                                        var a2 = $WH.ce('a'); a2.className = 'q1'; a2.href = '?spells=' + spell.cat + '.' + (1 + Math.log(spell.reqclass) / Math.LN2) + '.' + spell.talentspec[i]; $WH.ae(a2, $WH.ct(g_chr_specs[spell.talentspec[i]])); $WH.ae(d, a2)
                                                }
                                        } else if (g_spell_types[spell.cat]) { if (spell.cat == -11 && (spell.type == 1 || spell.type == 2)) { var a2 = $WH.ce('a'); a2.className = 'q1'; a2.href = '?spells=' + spell.cat + '.' + (spell.type == 2 ? '8' : '6'); $WH.ae(a2, $WH.ct(g_spell_types[spell.cat][spell.type])); $WH.ae(d, a2) } else { $WH.ae(d, $WH.ct(g_spell_types[spell.cat][spell.type])) } } else if (spell.reqclass) { var a2 = $WH.ce('a'); a2.className = 'q1'; a2.href = '?spells=' + spell.cat + '.' + (1 + Math.log(spell.reqclass) / Math.LN2) + (g_item_glyphs[spell.glyphtype] ? '&filter=gl=' + spell.glyphtype : ''); $WH.ae(a2, $WH.ct((g_item_glyphs[spell.glyphtype] ? g_item_glyphs[spell.glyphtype] + ' ' : '') + g_chr_classes[(1 + Math.log(spell.reqclass) / Math.LN2)])); $WH.ae(d, a2) }
                                } else if (g_spell_categories[spell.cat]) { var a1 = $WH.ce('a'); a1.className = 'q1'; a1.href = '?spells=' + spell.cat; $WH.ae(a1, $WH.ct(g_spell_categories[spell.cat])); $WH.ae(d, a1) }
                                $WH.ae(td, d)
                        }, getVisibleText: function (spell) {
                                var buff = g_spell_categories[spell.cat]; if ($WH.in_array([7, -2, -12, -11, -13], spell.cat) != -1) {
                                        if (spell.talentspec) { buff += ' ' + Listview.funcBox.arrayText(spell.talentspec, g_chr_specs) } else if (g_spell_types[spell.cat]) { buff += ' ' + g_spell_types[spell.cat][spell.type] } else {
                                                if (g_item_glyphs[spell.glyphtype]) { buff += ' ' + g_item_glyphs[spell.glyphtype] }
                                ...
                                var
                                typeA = (g_spell_types[a.cat] ? g_spell_types[a.cat][a.type] : a.type), typeB = (g_spell_types[b.cat] ? g_spell_types[b.cat][b.type] : a.type); return $WH.strcmp(a.cat, b.cat) || $WH.strcmp(typeA, typeB)
                        }
                }, {
                        id: 'reagents', name: LANG.reagents, width: '9%', hidden: !0, getValue: function (spell) { return (spell.reagents ? spell.reagents.length : 0) }, compute: function (spell, td) {
                                var hasIcons = (spell.reagents != null); if (hasIcons) {
                                        td.style.padding = '0'; var d = $WH.ce('div'); var items = spell.reagents; d.style.width = (44 * items.length) + 'px'; d.style.margin = '0 auto'; for (var i = 0, len = items.length; i < len; ++i) { var id = items[i][0]; var num = items[i][1]; var icon = g_items.createIcon(id, 1, num); icon.style.cssFloat = icon.style.styleFloat = 'left'; $WH.ae(d, icon) }
                                        $WH.ae(td, d)
                                }
                        }, sortFunc: function (a, b) { var lena = (a.reagents != null ? a.reagents.length : 0); var lenb = (b.reagents != null ? b.reagents.length : 0); if (lena > 0 && lena == lenb) { return $WH.strcmp(a.reagents.toString(), b.reagents.toString()) } else { return $WH.strcmp(lena, lenb) } }
                }, { id: 'tp', name: LANG.tp, tooltip: LANG.tooltip_trainingpoints, width: '7%', hidden: !0, value: 'tp', compute: function (spell, col) { if (spell.tp > 0) { return spell.tp } } }, {
                        id: 'source', name: LANG.source, type: 'text', width: '12%', hidden: !0, compute: function (spell, td) {
                                if (spell.source != null) {
                                        var buff = ''; for (var i = 0, len = spell.source.length; i < len; ++i) {
                                                if (i > 0) { buff += LANG.comma }
                                                buff += g_sources[spell.source[i]]
                                        }
                                        return buff
                                }
                        }, sortFunc: function (a, b, col) { return Listview.funcBox.assocArrCmp(a.source, b.source, g_sources) }
                }, {
                        id: 'skill', name: LANG.skill, type: 'text', width: '16%', getValue: function (spell) { return spell.learnedat }, compute: function (spell, td, tr, colNo) {
                                if (spell.skill != null) {
                                        this.skillsColumn = colNo; var div = $WH.ce('div'); div.className = 'small'; if (spell.cat == -7 && spell.pettype != null) { spell.skill = []; var petTalents = { 0: 410, 1: 409, 2: 411 }; for (var i = 0, len = spell.pettype.length; i < len; ++i) { spell.skill.push(petTalents[spell.pettype[i]]) } }
                                        for (var i = 0, len = spell.skill.length; i < len; ++i) {
                                                if (i > 0) { $WH.ae(div, $WH.ct(LANG.comma)) }
                                                if (spell.skill[i] == -1) { $WH.ae(div, $WH.ct(LANG.ellipsis)) } else {
                                                        if ($WH.in_array([7, -2, -3, -5, -6, -7, 11, 9], spell.cat) != -1) {
                                                                var a = $WH.ce('a'); a.className = 'q1'; if ($WH.in_array([-5, -6], spell.cat) != -1) { a.href = '?spells=' + spell.cat } else { a.href = '?spells=' + spell.cat + '.' + ((spell.reqclass && (spell.cat == 7 || spell.cat == -2)) ? (1 + Math.log(spell.reqclass) / Math.LN2) + '.' : '') + spell.skill[i] }
                                                                ...
                                                                $WH.ae(a, $WH.ct(spell.cat == -7 && spell.pettype != null ? g_pet_types[spell.pettype[i]] : g_spell_skills[spell.skill[i]])); $WH.ae(div, a)
                                                        } else { $WH.ae(div, $WH.ct(g_spell_skills[spell.skill[i]])) }
                                                }
                                        }
                                        if (spell.learnedat > 0) {
                                                $WH.ae(div, $WH.ct(' (')); var spLearnedAt = $WH.ce('span'); if (spell.learnedat == 9999) { spLearnedAt.className = 'q0'; $WH.ae(spLearnedAt, $WH.ct('??')) } else if (spell.learnedat > 0) { $WH.ae(spLearnedAt, $WH.ct(spell.learnedat)); spLearnedAt.style.fontWeight = 'bold' }
                                                $WH.ae(div, spLearnedAt); $WH.ae(div, $WH.ct(')'))
                                        }
                                        $WH.ae(td, div); if (spell.colors != null) {
                                                this.template.columns[colNo].type = null; var colors = spell.colors, nColors = 0; for (var i = 0; i < colors.length; ++i) { if (colors[i] > 0) { ++nColors; break } }
                                                if (nColors > 0) {
                                                        nColors = 0; div = $WH.ce('div'); div.className = 'small'; div.style.fontWeight = 'bold'; for (var i = 0; i < colors.length; ++i) {
                                                                if (colors[i] > 0) {
                                                                        if (nColors++ > 0) { $WH.ae(div, $WH.ct(' ')) }
                                ...
                                var buff = Listview.funcBox.arrayText(spell.skill, g_spell_skills); if (spell.learnedat > 0) { buff += ' ' + (spell.learnedat == 9999 ? '??' : spell.learnedat) }
                                return buff
                        }, sortFunc: function (a, b) {
                                var skill = [a.learnedat, b.learnedat]; for (var s = 0; s < 2; ++s) {
                                        var sp = (s == 0 ? a : b); if (skill[s] == 9999 && sp.colors != null) {
                                                var i = 0; while (sp.colors[i] == 0 && i < sp.colors.length) { i++ }
                        ...
                        id: 'stackRules', name: LANG.asr_behaviour, type: 'text', width: '20%', hidden: !0, compute: function (spell, td) {
                                if (!spell.stackRule) { return }
                                ...
                                td.className = 'small'; td.style.lineHeight = '18px'; var span = $WH.ce('span'); span.className = !spell.stackRule || spell.stackRule == 3 ? 'q2' : 'q10'; $WH.ae(span, $WH.ct(buff)); $WH.ae(td, span); if (buff2) { var sp2 = $WH.ce('span'); sp2.style.whiteSpace = 'nowrap'; sp2.className = 'q0'; $WH.ae(td, $WH.ce('br')); $WH.st(sp2, buff2); $WH.ae(td, sp2) }
                        }, getVisibleText: function (spell) {
                                if (!spell.stackRule) { return }
                                ...
                                return buff
                        }, sortFunc: function (a, b, col) { return $WH.strcmp(a.stackRule, b.stackRule) }
                }, {
                        id: 'linkedTrigger', name: LANG.ls_trigger, type: 'text', width: '50%', hidden: !0, compute: function (spell, td) {
                                if (!spell.linked) { return }
                                ...
                                $WH.ae(td, a); var span = $WH.ce('span'); span.className = 'q0'; $WH.st(span, buff); $WH.ae(td, $WH.ce('br')); $WH.ae(td, span)
                        }, getVisibleText: function (spell) {
                                if (!spell.linked) { return }
                        ...
                        id: 'linkedEffect', name: LANG.ls_effects, type: 'text', width: '50%', hidden: !0, compute: function (spell, td) {
                                if (!spell.linked) { return }
                ...
                }], getItemLink: function (spell) { return '?spell=' + spell.id }
        }, zone: {
                sort: [1], nItemsPerPage: -1, searchable: 1, filtrable: 1, columns: [{
                        id: 'name', name: LANG.name, type: 'text', align: 'left', value: 'name', compute: function (zone, td) {
                                var
                                ...
                                $WH.ae(td, wrapper); if (zone.subzones) {
                                        if (zone.subzones.length == 1 && zone.subzones[0] == zone.id)
                                                ...
                                                nRows++; wrapper.style.position = 'relative'; wrapper.style.minHeight = ((nRows * 12) + 19) + 'px'; var d = $WH.ce('div'); d.className = 'small'; d.style.fontStyle = 'italic'; d.style.position = 'absolute'; d.style.right = '2px'; d.style.bottom = '2px'; d.style.textAlign = 'right'; for (i in zone.subzones) {
                                                        if (!g_gatheredzones[zone.subzones[i]]) { continue }
                                                        if (i > 0) { $WH.ae(d, $WH.ct(LANG.comma)) }
                                ...
                                var buff = zone.name + Listview.funcBox.getExpansionText(zone); if (zone.instance == 5 || zone.instance == 8) { buff += ' heroic' }
                                return buff
                        }
                }, { id: 'level', name: LANG.level, type: 'range', width: '10%', getMinValue: function (zone) { return zone.minlevel }, getMaxValue: function (zone) { return zone.maxlevel }, compute: function (zone, td) { if (zone.minlevel > 0 && zone.maxlevel > 0) { if (zone.minlevel != zone.maxlevel) { return zone.minlevel + LANG.hyphen + zone.maxlevel } else { return zone.minlevel } } }, sortFunc: function (a, b, col) { if (col > 0) { return $WH.strcmp(a.minlevel, b.minlevel) || $WH.strcmp(a.maxlevel, b.maxlevel) } else { return $WH.strcmp(a.maxlevel, b.maxlevel) || $WH.strcmp(a.minlevel, b.minlevel) } } }, {
                        id: 'players', name: LANG.players, type: 'text', hidden: !0, compute: function (zone, td) {
                                if (zone.instance > 0) {
                                        var sp = $WH.ce('span'); if (zone.nplayers == -2) { zone.nplayers = '10/25' }
                                        ...
                                        $WH.ae(sp, $WH.ct(buff)); $WH.ae(td, sp)
                                }
                        }, getVisibleText: function (zone) {
                                if (zone.instance > 0) {
                                        if (zone.nplayers == -2) { zone.nplayers = '10/25' }
                        ...
                        }, sortFunc: function (a, b, col) { return $WH.strcmp(a.nplayers, b.nplayers) }
                }, {
                        id: 'territory', name: LANG.territory, type: 'text', width: '13%', compute: function (zone, td) {
                                var sp = $WH.ce('span'); switch (zone.territory) { case 0: sp.className = 'icon-alliance'; break; case 1: sp.className = 'icon-horde'; break; case 4: sp.className = 'icon-ffa'; break }
                                $WH.ae(sp, $WH.ct(g_zone_territories[zone.territory])); $WH.ae(td, sp)
                        }, getVisibleText: function (zone) { return g_zone_territories[zone.territory] }, sortFunc: function (a, b, col) { return $WH.strcmp(g_zone_territories[a.territory], g_zone_territories[b.territory]) }
                }, {
                        id: 'instancetype', name: LANG.instancetype, type: 'text', compute: function (zone, td) {
                                if (zone.instance > 0) {
                                        var sp = $WH.ce('span'); if ((zone.instance >= 1 && zone.instance <= 5) || zone.instance == 7 || zone.instance == 8) { sp.className = 'icon-instance' + zone.instance }
                                ...
                                if (zone.instance > 0) {
                                        var buff = g_zone_instancetypes[zone.instance]; if (zone.nplayers && ((zone.instance != 2 && zone.instance != 5) || zone.nplayers > 5)) { if (zone.instance == 4) { buff += ' ' + $WH.sprintf(LANG.lvzone_xvx, zone.nplayers, zone.nplayers) } else { buff += ' ' + $WH.sprintf(LANG.lvzone_xman, zone.nplayers) } }
                                        if (zone.instance == 5 || zone.instance == 8) { buff += ' heroic' }
                                        return buff
                                }
                        }, sortFunc: function (a, b, col) { return $WH.strcmp(g_zone_instancetypes[a.instance], g_zone_instancetypes[b.instance]) || $WH.strcmp(a.instance, b.instance) || $WH.strcmp(a.nplayers, b.nplayers) }
                }, { id: 'category', name: LANG.category, type: 'text', width: '15%', compute: function (zone, td) { td.className = 'small q1'; var a = $WH.ce('a'); a.href = '?zones=' + zone.category; $WH.ae(a, $WH.ct(g_zone_categories[zone.category])); $WH.ae(td, a) }, getVisibleText: function (zone) { return g_zone_categories[zone.category] }, sortFunc: function (a, b, col) { return $WH.strcmp(g_zone_categories[a.category], g_zone_categories[b.category]) } }], getItemLink: function (zone) { return '?zone=' + zone.id }
        }, holiday: {
                sort: [2, 1], nItemsPerPage: -1, searchable: 1, filtrable: 1, columns: [{ id: 'name', name: LANG.name, type: 'text', align: 'left', value: 'name', span: 2, compute: function (holiday, td, tr) { var i = $WH.ce('td'); i.style.width = '1px'; i.style.padding = '0'; i.style.borderRight = 'none'; $WH.ae(i, g_holidays.createIcon(holiday.id, 0)); $WH.ae(tr, i); td.style.borderLeft = 'none'; var a = $WH.ce('a'); a.style.fontFamily = 'Verdana, sans-serif'; a.href = this.getItemLink(holiday); $WH.ae(a, $WH.ct(holiday.name)); $WH.ae(td, a) }, getVisibleText: function (holiday) { return holiday.name } }, {
                        id: 'date', name: LANG.date, type: 'text', width: '16%', allText: !0, compute: function (holiday, td, tr) {
                                if (holiday.startDate && holiday.endDate) {
                                        var dates = Listview.funcBox.getEventNextDates(holiday.startDate, holiday.endDate, holiday.rec || 0); if (dates[0] && dates[1]) {
                                                var
                                                ...
                                                $WH.ae(td, sp); if (dates[0] <= g_serverTime && dates[1] >= g_serverTime) { tr.className = 'checked'; sp.className = 'q2 tip'; g_addTooltip(sp, LANG.tooltip_activeholiday, 'q') }
                                        }
                                }
                                ...
                                if (a.startDate && b.startDate) { var datesA = Listview.funcBox.getEventNextDates(a.startDate, a.endDate, a.rec || 0); var datesB = Listview.funcBox.getEventNextDates(b.startDate, b.endDate, b.rec || 0); if (datesA[0] && datesB[0]) { return datesA[0] - datesB[0] } } else if (a.startDate) { return -1 } else if (b.startDate) { return 1 }
                                return 0
                        }
                }, { id: 'category', name: LANG.category, type: 'text', width: '16%', compute: function (holiday, td) { td.className = 'small q1'; var a = $WH.ce('a'), href = '?events=' + holiday.category; a.href = href; $WH.ae(a, $WH.ct(g_holiday_categories[holiday.category])); $WH.ae(td, a) }, getVisibleText: function (holiday) { return g_holiday_categories[holiday.category] }, sortFunc: function (a, b, col) { return $WH.strcmp(g_holiday_categories[a.category], g_holiday_categories[b.category]) } }], getItemLink: function (holiday) { return '?event=' + holiday.id }
        }, holidaycal: {
                sort: [1], mode: 4, startOnMonth: new Date(g_serverTime.getFullYear(), 0, 1), nMonthsToDisplay: 12, rowOffset: g_serverTime.getMonth(), poundable: 2, columns: [], compute: function (holiday, div, i) {
                        if (!holiday.events || !holiday.events.length) { return }
                        for (var i = 0; i < holiday.events.length; ++i) { var icon = g_holidays.createIcon(holiday.events[i].id, 1); icon.onmouseover = Listview.funcBox.dateEventOver.bind(icon, holiday.date, holiday.events[i]); icon.onmousemove = $WH.Tooltip.cursorUpdate; icon.onmouseout = $WH.Tooltip.hide; icon.style.cssFloat = icon.style.styleFloat = 'left'; $WH.ae(div, icon) }
                }, sortFunc: function (a, b) {
                        if (a.startDate && b.startDate) {
                                var datesA = Listview.funcBox.getEventNextDates(a.startDate, a.endDate, a.rec || 0); var datesB = Listview.funcBox.getEventNextDates(b.startDate, b.endDate, b.rec || 0); for (var i = 0; i < 2; ++i) {
                                        var
                ...
                sort: [-5, 4, 6, -3, 2], mode: 2, nItemsPerPage: 40, poundable: 2, columns: [{ value: 'number' }, { value: 'id' }, { value: 'rating' }, { value: 'deleted' }, { value: 'sticky' }, { value: 'outofdate' }], compute: function (comment, div, i) {
                        var container = $('<div></div>'); var hidden = (comment.__minPatch && g_getPatchVersion.T[comment.__minPatch] > new Date(comment.date)); comment.locale = (this.id == 'english-comments' ? 'www' : ''); container.append('<table><tr><td class="vote-column">' + '<p class="upvote">+</p><div class="rating-container"></div><p class="downvote">-</p><p class="comment-sticky" title="' + LANG.stickycomment_tip + '">STICKY</p>' + '</td><td class="main-body">' + '<div class="comment-header"><table><tr><td class="comment-author"></td><td class="comment-controls"></td><td class="comment-notification"></td></tr></table></div>' + '<div class="text comment-body"></div>' + '<div class="comment-replies"></div>' + '<p class="comment-replies-control" style="display: none"></p>' + '</td></tr></table>'); comment.colorClass = 'comment' + (i % 2); container.addClass('comment').addClass(comment.colorClass); if (comment.rating < -4) { container.addClass('comment-lowrated') }
                        if (comment.deleted)
                                container.addClass('comment-deleted'); if (comment.outofdate)
                                container.addClass('comment-outofdate'); comment.container = container; comment.voteCell = container.find('.vote-column'); comment.commentCell = container.find('.main-body'); comment.repliesCell = container.find('.comment-replies'); comment.repliesControl = container.find('.comment-replies-control'); comment.headerCell = container.find('.comment-header'); comment.commentBody = container.find('.comment-body'); comment.commentAuthor = container.find('.comment-author'); comment.commentControls = container.find('.comment-controls'); this.template.updateVoteCell(comment, !0); this.template.updateCommentCell(comment); this.template.updateStickyStatus(comment); this.template.updateReplies(comment); if (comment.deleted || comment.rating < -4) {
                                        comment.headerCell.css('cursor', 'pointer'); comment.headerCell.bind('click', function (e) {
                                                if ($WH.$E(e)._target.nodeName == 'A') { return }
                        ...
                        $(div).addClass('comment-wrapper').html(container); if (hidden)
                                $(div).hide(); else $(div).show()
                }, updateReplies: function (comment) {
                        this.updateRepliesCell(comment); this.updateRepliesControl(comment); SetupReplies(comment.container, comment); if (!comment.alreadyLoadedBefore) {
                                comment.alreadyLoadedBefore = !0; var matches = location.hash.match('#comments:id=([0-9]+):reply=([0-9]+)'); if (matches) {
                                        var commentId = matches[1]; var replyId = matches[2]; if (comment.id == commentId)
                                                this.highlightReply(comment, replyId)
                                }
                        }
                }, highlightReply: function (comment, replyId, fetchedMoreReplies) {
                        var reply = null; for (var idx in comment.replies) { if (comment.replies[idx].id == replyId) { reply = comment.replies[idx]; break } }
                        if (!reply) {
                        ...
                        container.html(''); for (var idx in controls) {
                                var data = controls[idx]; var control = $('<a></a>'); var insideContainer = $('<span></span>'); if (idx != 0)
                        ...
                        if (comment.lastEdit)
                                ...
                                this.addEditedByOrDeletedByText(commentBody, LANG.lvcomment_deletedby, comment.deletedInfo[1], comment.deletedInfo[0], null); comment.divBody = commentBody[0]; if (comment.deleted || comment.rating < -4) { commentBody.hide() }
                }, addEditedByOrDeletedByText: function (commentBody, text, username, when, count) { var editedOn = new Date(when); var elapsed = (g_serverTime - editedOn) / 1000; count = count > 1 ? LANG.dash + $WH.sprintf(LANG.lvcomment_nedits, count) : ''; var lastEdit = $('<div></div>'); lastEdit.addClass('comment-lastedit'); lastEdit.html($WH.sprintf('$1 <a></a> $2 $3$4', text, g_formatDate(null, elapsed, editedOn), $WH.sprintf(LANG.lvcomment_patch, g_getPatchVersion(editedOn)), count)); lastEdit.find('a').text(username).attr('href', '?user=' + username); commentBody.append(lastEdit) }, updateVoteCell: function (comment, onInit) {
                        var upvoteControl = comment.voteCell.find('.upvote'); var downvoteControl = comment.voteCell.find('.downvote'); var ratingContainer = comment.voteCell.find('.rating-container'); var ratingText = $('<p class="rating"></p>'); var template = this; ratingContainer.html(ratingText); if (comment.userRating > 0)
                        ...
                        this.updateVoteCell(comment); $.get('?comment=vote', { id: comment.id, rating: rating }, function (response) {
                                if (response.error) { comment.rating = backupCommentRating; comment.userRating = backupUserRating; template.updateVoteCell(comment) }
                                if (response.message)
                                        template.voteError(comment, response.message)
                        }, 'json')
                }, voteError: function (comment, error) { MessageBox(comment.voteCell, error) }, showVoteBreakdown: function (comment, ratingText) {
                        ratingText.find('p').html('<img src="' + g_staticUrl + '/images/icons/ajax.gif" />'); $.get('?comment=rating&id=' + comment.id, null, function (data) {
                                if (data.success)
                                        ratingText.html($WH.sprintf('<p class="rating-up">$1</p><div class="rating-separator"></div><p class="rating-down">$2</p>', (data.up ? ('+' + data.up) : 0), -data.down)); else MessageBox(ratingText, 'An error has occurred while fetching vote counts. Try refreshing the page.')
                        }, 'json')
                }, sticky: function (comment) {
                        var notificationDiv = comment.commentCell.find('.comment-notification'); comment.sticky = !comment.sticky; this.updateStickyStatus(comment); this.updateCommentCell(comment); if (comment.sticky)
                                MessageBox(notificationDiv, 'This comment is now sticky.'); else MessageBox(notificationDiv, 'This comment is no longer sticky.'); $.post('?comment=sticky', { id: comment.id, sticky: comment.sticky ? 1 : 0 })
                }, createNote: function (div) {
                        var sm = $WH.ce('small'); if (!g_user.commentban) {
                                var a = $WH.ce('a'); if (g_user.id > 0) { a.href = 'javascript:;'; a.onclick = co_addYourComment } else { a.href = '?account=signin' }
                        ...
                        select.onchange = Listview.funcBox.coFilterByPatchVersion.bind(this, select); $WH.ae(sm, $WH.ct(LANG.lvcomment_patchfilter)); $WH.ae(sm, select); $WH.ae(div, sm); if (this.tabClick) { $('a, select', sm).click(this.tabClick) }
                }, onNoData: function (div) {
                        var s = '<b>' + LANG.lvnodata_co1 + '</b><div class="pad2"></div>'; if (g_user.id > 0) { var foo = LANG.lvnodata_co2; foo = foo.replace('<a>', '<a href="javascript:;" onclick="co_addYourComment()" onmousedown="return false">'); s += foo } else { var foo = LANG.lvnodata_co3; foo = foo.replace('<a>', '<a href="?account=signin">'); foo = foo.replace('<a>', '<a href="?account=signup">'); s += foo }
                        div.style.padding = '1.5em 0'; div.innerHTML = s
                }, onBeforeCreate: function () {
                        if (location.hash && location.hash.match(/:id=([0-9]+)/) != null) {
                                var sort = $WH.gc()['comments_sort']; switch (sort) { case '1': this.setSort([-5, 4, -1, -2], !1, !1); break; case '2': this.setSort([-5, 4, 1, 2], !1, !1); break; default: case '3': this.setSort([-5, 4, -3, 2], !1, !1); break }
                                var rowNo = $WH.in_array(this.data, parseInt(RegExp.$1), function (x) { return x.id }); this.rowOffset = this.getRowOffset(rowNo); return this.data[rowNo]
                        }
                }, onAfterCreate: function (result) { if (result != null) { var div = result.__div; this.tabs.__st = div } }
        }, commentpreview: {
                sort: [4], nItemsPerPage: 75, columns: [{ id: 'subject', name: LANG.subject, align: 'left', value: 'subject', compute: function (comment, td) { var a = $WH.ce('a'); a.style.fontFamily = 'Verdana, sans-serif'; a.href = this.getItemLink(comment); $WH.ae(a, $WH.ct(comment.subject)); $WH.ae(td, a); if (LANG.types[comment.type]) { var d = $WH.ce('div'); d.className = 'small'; $WH.ae(d, $WH.ct(LANG.types[comment.type][0])); $WH.ae(td, d) } } }, {
                        id: 'preview', name: LANG.preview, align: 'left', width: '50%', value: 'preview', compute: function (comment, td, tr) {
                                var d = $WH.ce('div'); d.className = 'crop'; if (comment.rating >= 10) { d.className += ' comment-green' }
                                $WH.ae(d, $WH.ct(Markup.removeTags(comment.preview, { mode: Markup.MODE_ARTICLE }))); $WH.ae(td, d); if (comment.rating !== null || comment.deleted) {
                                        d = $WH.ce('div'); d.className = 'small3'; if (comment.rating) { $WH.ae(d, $WH.ct(LANG.lvcomment_rating + (comment.rating > 0 ? '+' : '') + comment.rating)) }
                                        var s = $WH.ce('span'), foo = ''; s.className = 'q10'; if (comment.deleted) { foo = LANG.lvcomment_deleted }
                                        $WH.ae(s, $WH.ct(foo)); $WH.ae(d, s); tr.__status = s; $WH.ae(td, d)
                                }
                        }
                }, { id: 'author', name: LANG.author, value: 'user', compute: function (comment, td) { td.className = 'q1'; var a = $WH.ce('a'); a.href = '?user=' + comment.user; $WH.ae(a, $WH.ct(comment.user)); $WH.ae(td, a) } }, {
                        id: 'posted', name: LANG.posted, width: '16%', value: 'elapsed', compute: function (comment, td) {
                                var
                                postedOn = new Date(comment.date), elapsed = (g_serverTime - postedOn) / 1000; var s = $WH.ce('span'); g_formatDate(s, elapsed, postedOn, 0, 1); $WH.ae(td, s)
                        }
                }], getItemLink: function (comment) { return '?' + g_types[comment.type] + '=' + comment.typeId + (comment.id != null ? '#comments:id=' + comment.id : '') }
        }, replypreview: {
                sort: [4], nItemsPerPage: 75, columns: [{ id: 'subject', name: LANG.subject, align: 'left', value: 'subject', compute: function (reply, td) { var a = $WH.ce('a'); a.style.fontFamily = 'Verdana, sans-serif'; a.href = this.getItemLink(reply); $WH.ae(a, $WH.ct(reply.subject)); $WH.ae(td, a); if (LANG.types[reply.type]) { var d = $WH.ce('div'); d.className = 'small'; $WH.ae(d, $WH.ct(LANG.types[reply.type][0])); $WH.ae(td, d) } } }, {
                        id: 'preview', name: LANG.preview, align: 'left', width: '50%', value: 'preview', compute: function (reply, td, tr) {
                                var d = $WH.ce('div'); d.className = 'crop'; if (reply.rating >= 10)
                                        d.className += ' comment-green'; $WH.ae(d, $WH.ct(Markup.removeTags(reply.preview, { mode: Markup.MODE_ARTICLE }))); $WH.ae(td, d); if (reply.rating) { d = $WH.ce('div'); d.className = 'small3'; $WH.ae(d, $WH.ct(LANG.lvcomment_rating + (reply.rating > 0 ? '+' : '') + reply.rating)); var s = $WH.ce('span'), foo = ''; s.className = 'q10'; $WH.ae(s, $WH.ct(foo)); $WH.ae(d, s); tr.__status = s; $WH.ae(td, d) }
                        }
                }, {
                        id: 'author', name: LANG.author, value: 'user', compute: function (reply, td) {
                                td.className = 'q1'; var a = $WH.ce('a'); a.href = '?user=' + reply.user; $WH.ae(a, $WH.ct(reply.user))
                                $WH.ae(td, a)
                        }
                }, { id: 'posted', name: LANG.posted, width: '16%', value: 'elapsed', compute: function (reply, td) { var postedOn = new Date(reply.date), elapsed = (g_serverTime - postedOn) / 1000; var s = $WH.ce('span'); g_formatDate(s, elapsed, postedOn, 0, 1); $WH.ae(td, s) } }], getItemLink: function (reply) {
                        if (reply.url)
                                return '?' + reply.url; if (!g_types[reply.type])
                                return '?go-to-reply&id=' + reply.id; return $WH.sprintf('?$1=$2#comments:id=$3:reply=$4', g_types[reply.type], reply.typeId, reply.commentid, reply.id)
                }
        }, screenshot: {
                sort: [], mode: 3, nItemsPerPage: 40, nItemsPerRow: 4, poundable: 2, columns: [], compute: function (screenshot, td, i) {
                        var
                        ...
                        $WH.ae(d, s); $WH.ae(d, $WH.ct(' ' + LANG.dash + ' ')); var a = $WH.ce('a'); a.href = 'javascript:;'; a.onclick = ContactTool.show.bind(ContactTool, { mode: 3, screenshot: screenshot }); a.className = 'icon-report'; g_addTooltip(a, LANG.report_tooltip, 'q2'); $WH.ae(a, $WH.ct(LANG.report)); $WH.ae(d, a); $WH.ae(td, d); d = $WH.ce('div'); d.style.position = 'relative'; d.style.height = '1em'; var hasCaption = (screenshot.caption != null && screenshot.caption.length); var hasSubject = (screenshot.subject != null && screenshot.subject.length); if (hasCaption || hasSubject) {
                                var d2 = $WH.ce('div'); d2.className = 'screenshot-caption'; if (hasSubject) {
                                        var foo1 = $WH.ce('small'); $WH.ae(foo1, $WH.ct(LANG.types[screenshot.type][0] + LANG.colon)); var foo2 = $WH.ce('a'); $WH.ae(foo2, $WH.ct(screenshot.subject)); foo2.href = '?' + g_types[screenshot.type] + '=' + screenshot.typeId; $WH.ae(foo1, foo2); $WH.ae(d2, foo1); if (hasCaption && screenshot.caption.length) { $WH.ae(foo1, $WH.ct(' (...)')) }
                                        $WH.ae(foo1, $WH.ce('br'))
                                }
                        ...
                        $WH.aE(td, 'click', Listview.funcBox.ssCellClick.bind(this, i)); $WH.ae(td, d)
                }, createNote: function (div) {
                        if (typeof g_pageInfo == 'object' && g_pageInfo.type > 0) {
                                var sm = $WH.ce('small'); var a = $WH.ce('a'); if (g_user.id > 0) { a.href = 'javascript:;'; a.onclick = ss_submitAScreenshot } else { a.href = '?account=signin' }
                                $WH.ae(a, $WH.ct(LANG.lvscreenshot_submit)); $WH.ae(sm, a); $WH.ae(div, sm)
                        }
                }, onNoData: function (div) {
                        if (typeof g_pageInfo == 'object' && g_pageInfo.type > 0) {
                                var s = '<b>' + LANG.lvnodata_ss1 + '</b><div class="pad2"></div>'; if (g_user.id > 0) { var foo = LANG.lvnodata_ss2; foo = foo.replace('<a>', '<a href="javascript:;" onclick="ss_submitAScreenshot()" onmousedown="return false">'); s += foo } else { var foo = LANG.lvnodata_ss3; foo = foo.replace('<a>', '<a href="?account=signin">'); foo = foo.replace('<a>', '<a href="?account=signup">'); s += foo }
                                div.style.padding = '1.5em 0'; div.innerHTML = s
                        } else { return -1 }
                }, onBeforeCreate: function () { if (location.hash && location.hash.match(/:id=([0-9]+)/) != null) { var rowNo = $WH.in_array(this.data, parseInt(RegExp.$1), function (x) { return x.id }); this.rowOffset = this.getRowOffset(rowNo); return rowNo } }, onAfterCreate: function (rowNo) { if (rowNo != null) { setTimeout((function () { ScreenshotViewer.show({ screenshots: this.data, pos: rowNo }) }).bind(this), 1) } }
        }, sound: {
                searchable: 1, filtrable: 1, columns: [{ id: 'name', name: LANG.name, type: 'text', align: 'left', value: 'name', compute: function (sound, td) { var a = $WH.ce('a'); a.className = 'listview-cleartext'; a.href = '?sound=' + sound.id; $WH.st(a, sound.name); $WH.ae(td, a) } }, {
                        id: 'type', name: LANG.type, type: 'text', compute: function (sound, td) {
                                var type = ''; if (!this.hasOwnProperty('soundtypecache')) {
                                        var cache = {}; for (var i in mn_sounds)
                                ...
                                if (this.soundtypecache.hasOwnProperty(sound.type))
                                        type = this.soundtypecache[sound.type]; $WH.st(td, type)
                        }, sortFunc: function (a, b) {
                                ...
                                var aType = (this.soundtypecache.hasOwnProperty(a.type)) ? this.soundtypecache[a.type] : ''; var bType = (this.soundtypecache.hasOwnProperty(b.type)) ? this.soundtypecache[b.type] : ''; return $WH.strcmp(aType, bType) || $WH.strcmp(a.name, b.name)
                        }
                }, {
                        id: 'activity', name: LANG.activity, hidden: !0, type: 'text', compute: function (sound, td) {
                                if (!sound.hasOwnProperty('activity'))
                                        return ''; if (LANG.sound_activities.hasOwnProperty(sound.activity))
                                        return LANG.sound_activities[sound.activity]; else return sound.activity
                        }, sortFunc: function (a, b) { return $WH.strcmp(this.compute(a), this.compute(b)) }
                }, { id: 'sound', name: LANG.types[19][2], span: 2, compute: function (sound, td, tr) { var td2 = $WH.ce('td'); td2.className = 'nowrap'; $WH.ae(tr, td2); td2.style.borderRight = 'none'; td.className = 'nowrap'; td.style.borderLeft = 'none'; var div = $WH.ce('div'); $WH.ae(td2, div); (new AudioControls()).init(sound.files, div, { listview: this, trackdisplay: td }) } }], getItemLink: function (sound) { return '?sound=' + sound.id }
        }, video: {
                sort: [], mode: 3, nItemsPerPage: 40, nItemsPerRow: 4, poundable: 2, columns: [], compute: function (video, td, i) {
                        var
                        ...
                        var s = $WH.ce('span'); if (hasUser) { g_formatDate(s, elapsed, postedOn) } else { g_formatDate(s, elapsed, postedOn, 0, 1) }
                        $WH.ae(d, s); $WH.ae(d, $WH.ct(' ' + LANG.dash + ' ')); var a = $WH.ce('a'); a.href = 'javascript:;'; a.onclick = ContactTool.show.bind(ContactTool, { mode: 5, video: video }); a.className = 'icon-report'; g_addTooltip(a, LANG.report_tooltip, 'q2'); $WH.ae(a, $WH.ct(LANG.report)); $WH.ae(d, a); $WH.ae(td, d); d = $WH.ce('div'); d.style.position = 'relative'; d.style.height = '1em'; var hasCaption = (video.caption != null && video.caption.length); var hasSubject = (video.subject != null && video.subject.length); if (hasCaption || hasSubject) {
                                var d2 = $WH.ce('div'); d2.className = 'screenshot-caption'; if (hasSubject) {
                                        var foo1 = $WH.ce('small'); $WH.ae(foo1, $WH.ct(LANG.types[video.type][0] + LANG.colon)); var foo2 = $WH.ce('a'); $WH.ae(foo2, $WH.ct(video.subject)); foo2.href = '?' + g_types[video.type] + '=' + video.typeId; $WH.ae(foo1, foo2); $WH.ae(d2, foo1); if (hasCaption && video.caption.length) { $WH.ae(foo1, $WH.ct(' (...)')) }
                                        $WH.ae(foo1, $WH.ce('br'))
                                }
                        ...
                        $WH.aE(td, 'click', Listview.funcBox.viCellClick.bind(this, i)); $WH.ae(td, d)
                }, createNote: function (div) {
                        if (g_user && g_user.roles & (U_GROUP_ADMIN | U_GROUP_BUREAU | U_GROUP_VIDEO)) {
                                if (typeof g_pageInfo == 'object' && g_pageInfo.type > 0) {
                                        var sm = $WH.ce('small'); var a = $WH.ce('a'); if (g_user.id > 0) { a.href = 'javascript:;'; a.onclick = vi_submitAVideo } else { a.href = '?account=signin' }
                                        $WH.ae(a, $WH.ct(LANG.lvvideo_suggest)); $WH.ae(sm, a); $WH.ae(div, sm)
                                }
                        }
                }, onNoData: function (div) {
                        if (typeof g_pageInfo == 'object' && g_pageInfo.type > 0) {
                                var s = '<b>' + LANG.lvnodata_vi1 + '</b><div class="pad2"></div>'; if (g_user.id > 0) { var foo = LANG.lvnodata_vi2; foo = foo.replace('<a>', '<a href="javascript:;" onclick="vi_submitAVideo()" onmousedown="return false">'); s += foo } else { var foo = LANG.lvnodata_vi3; foo = foo.replace('<a>', '<a href="?account=signin">'); foo = foo.replace('<a>', '<a href="?account=signup">'); s += foo }
                                div.style.padding = '1.5em 0'; div.innerHTML = s
                        } else { return -1 }
                }, onBeforeCreate: function () { if (location.hash && location.hash.match(/:id=([0-9]+)/) != null) { var rowNo = $WH.in_array(this.data, parseInt(RegExp.$1), function (x) { return x.id }); this.rowOffset = this.getRowOffset(rowNo); return rowNo } }, onAfterCreate: function (rowNo) { if (rowNo != null) { setTimeout((function () { VideoViewer.show({ videos: this.data, pos: rowNo }) }).bind(this), 1) } }
        }, pet: {
                sort: [1], nItemsPerPage: -1, searchable: 1, filtrable: 1, columns: [{
                        id: 'name', name: LANG.name, type: 'text', align: 'left', value: 'name', span: 2, compute: function (pet, td, tr) {
                                var i = $WH.ce('td'); i.style.width = '1px'; i.style.padding = '0'; i.style.borderRight = 'none'; $WH.ae(i, Icon.create(pet.icon, 0)); $WH.ae(tr, i); td.style.borderLeft = 'none'; var wrapper = $WH.ce('div'); var a = $WH.ce('a'); a.style.fontFamily = 'Verdana, sans-serif'; a.href = this.getItemLink(pet); $WH.ae(a, $WH.ct(pet.name)); if (pet.expansion) { var sp = $WH.ce('span'); sp.className = g_GetExpansionClassName(pet.expansion); $WH.ae(sp, a); $WH.ae(wrapper, sp) } else { $WH.ae(wrapper, a) }
                                ...
                                var buff = pet.name + Listview.funcBox.getExpansionText(pet); if (pet.exotic) { buff += ' ' + LANG.lvpet_exotic }
                                return buff
                        }
                }, { id: 'level', name: LANG.level, type: 'range', getMinValue: function (pet) { return pet.minlevel }, getMaxValue: function (pet) { return pet.maxlevel }, compute: function (pet, td) { if (pet.minlevel > 0 && pet.maxlevel > 0) { if (pet.minlevel != pet.maxlevel) { return pet.minlevel + LANG.hyphen + pet.maxlevel } else { return pet.minlevel } } else { return -1 } }, sortFunc: function (a, b, col) { if (col > 0) { return $WH.strcmp(a.minlevel, b.minlevel) || $WH.strcmp(a.maxlevel, b.maxlevel) } else { return $WH.strcmp(a.maxlevel, b.maxlevel) || $WH.strcmp(a.minlevel, b.minlevel) } } }, { id: 'damage', name: LANG.damage, value: 'damage', hidden: 1, compute: function (pet, col) { $WH.ae(col, this.template.getStatPct(pet.damage)) } }, { id: 'armor', name: LANG.armor, value: 'armor', hidden: 1, compute: function (pet, col) { $WH.ae(col, this.template.getStatPct(pet.armor)) } }, { id: 'health', name: LANG.health, value: 'health', hidden: 1, compute: function (pet, col) { $WH.ae(col, this.template.getStatPct(pet.health)) } }, {
                        id: 'abilities', name: LANG.abilities, type: 'text', getValue: function (pet) {
                                if (!pet.spells) { return '' }
                                if (pet.spells.length > 0) {
                                        var spells = ''; for (var i = 0, len = pet.spells.length; i < len; ++i) { if (pet.spells[i]) { spells += g_spells[pet.spells[i]]['name_' + Locale.getName()] } }
                                        return spells
                                }
                        }, compute: function (pet, td) {
                                if (!pet.spells) { return '' }
                                if (pet.spells.length > 0) { td.style.padding = '0'; Listview.funcBox.createCenteredIcons(pet.spells, td, '', 1) }
                        }, sortFunc: function (a, b) {
                                if (!a.spells || !b.spells) { return 0 }
                                return $WH.strcmp(a.spellCount, b.spellCount) || $WH.strcmp(a.spells, b.spells)
                        }, hidden: !0
                }, {
                        id: 'diet', name: LANG.diet, type: 'text', compute: function (pet, td) {
                                if (td) { td.className = 'small' }
                                var i = 0, foods = ''; for (var food in g_pet_foods) {
                                        if (pet.diet & food) {
                                                if (i++ > 0) { foods += LANG.comma }
                                                foods += g_pet_foods[food]
                                        }
                                }
                                return foods
                        }, sortFunc: function (a, b) { return $WH.strcmp(b.foodCount, a.foodCount) || Listview.funcBox.assocArrCmp(a.diet, b.diet, g_pet_foods) }
                }, { id: 'type', name: LANG.type, type: 'text', compute: function (pet, td) { if (pet.type != null) { td.className = 'small q1'; var a = $WH.ce('a'); a.href = '?pets=' + pet.type; $WH.ae(a, $WH.ct(g_pet_types[pet.type])); $WH.ae(td, a) } }, getVisibleText: function (pet) { if (pet.type != null) { return g_pet_types[pet.type] } }, sortFunc: function (a, b, col) { return $WH.strcmp(g_pet_types[a.type], g_pet_types[b.type]) } }], getItemLink: function (pet) { return '?pet=' + pet.id }, getStatPct: function (modifier) {
                        var _ = $WH.ce('span'); if (!isNaN(modifier) && modifier > 0) { _.className = 'q2'; $WH.ae(_, $WH.ct('+' + modifier + '%')) } else if (!isNaN(modifier) && modifier < 0) { _.className = 'q10'; $WH.ae(_, $WH.ct(modifier + '%')) }
                        return _
                }
        }, achievement: {
                sort: [1, 2], nItemsPerPage: 100, searchable: 1, filtrable: 1, columns: [{
                        id: 'name', name: LANG.name, type: 'text', align: 'left', value: 'name', span: 2, compute: function (achievement, td, tr) {
                                var rel = null; if (achievement.who && achievement.completed) { rel = 'who=' + achievement.who + '&when=' + achievement.completed.getTime() }
                                var i = $WH.ce('td'); i.style.width = '1px'; i.style.padding = '0'; i.style.borderRight = 'none'; $WH.ae(i, g_achievements.createIcon(achievement.id, 1)); Icon.getLink(i.firstChild).href = this.getItemLink(achievement); Icon.getLink(i.firstChild).rel = rel; $WH.ae(tr, i); td.style.borderLeft = 'none'; var a = $WH.ce('a'); a.style.fontFamily = 'Verdana, sans-serif'; a.href = this.getItemLink(achievement); a.rel = rel; $WH.ae(a, $WH.ct(achievement.name)); $WH.ae(td, a); if (achievement.description != null) { var d = $WH.ce('div'); d.className = 'small'; $WH.ae(d, $WH.ct(achievement.description)); $WH.ae(td, d) }
                        }, getVisibleText: function (achievement) {
                                var buff = achievement.name; if (achievement.description) { buff += ' ' + achievement.description }
                                return buff
                        }
                }, { id: 'location', name: LANG.location, type: 'text', width: '15%', hidden: 1, compute: function (achievement, td) { if (achievement.zone) { var a = $WH.ce('a'); a.className = 'q1'; a.href = '?zones=' + achievement.zone; $WH.ae(a, $WH.ct(g_zones[achievement.zone])); $WH.ae(td, a) } }, getVisibleText: function (achievement) { return Listview.funcBox.arrayText(achievement.zone, g_zones) }, sortFunc: function (a, b, col) { return Listview.funcBox.assocArrCmp(a.zone, b.zone, g_zones) } }, { id: 'side', name: LANG.side, type: 'text', compute: function (achievement, td) { if (achievement.side && achievement.side != 3) { var sp = $WH.ce('span'); sp.className = (achievement.side == 1 ? 'icon-alliance' : 'icon-horde'); g_addTooltip(sp, g_sides[achievement.side]); $WH.ae(td, sp) } }, getVisibleText: function (achievement) { if (achievement.side) { return g_sides[achievement.side] } }, sortFunc: function (a, b, col) { return $WH.strcmp(g_sides[a.side], g_sides[b.side]) } }, { id: 'points', name: LANG.points, type: 'number', width: '10%', value: 'points', compute: function (achievement, td) { if (achievement.points) { Listview.funcBox.appendMoney(td, 0, null, 0, 0, achievement.points) } } }, {
                        id: 'rewards', name: LANG.rewards, type: 'text', width: '20%', compute: function (achievement, td) {
                                if (achievement.rewards) {
                                        ...
                                        if (itemrewards.length > 0) {
                                                for (var i = 0; i < itemrewards.length; i++) {
                                                        if (!g_items[itemrewards[i]]) { return }
                                        ...
                                        if (spellrewards.length > 0) {
                                                for (var i = 0; i < spellrewards.length; i++) {
                                                        if (!g_spells[spellrewards[i]]) { return }
                                        ...
                                        if (titlerewards.length > 0) {
                                                for (var i = 0; i < titlerewards.length; i++) {
                                                        if (!g_titles[titlerewards[i]]) { return }
                                                        var title = g_titles[titlerewards[i]]['name_' + Locale.getName()]; title = title.replace('%s', '<span class="q0">&lt;' + LANG.name + '&gt;</span>'); var span = $WH.ce('a'); span.className = 'q1'; span.href = '?title=' + titlerewards[i]; span.innerHTML = title; $WH.ae(td, span); $WH.ae(td, $WH.ce('br'))
                                                }
                                        }
                                } else if (achievement.reward) { var span = $WH.ce('span'); span.className = 'q1'; $WH.ae(span, $WH.ct(achievement.reward)); $WH.ae(td, span) }
                        }, getVisibleText: function (achievement) {
                                var buff = ''; if (achievement.rewards) { for (var i = 0; i < achievement.rewards.length; i++) { if (achievement.rewards[i][0] == 11) { buff += ' ' + g_titles[achievement.rewards[i][1]]['name_' + Locale.getName()].replace('%s', '<' + LANG.name + '>') } else if (achievement.rewards[i][0] == 3) { buff += ' ' + g_items[achievement.rewards[i][1]]['name_' + Locale.getName()] } else if (achievement.rewards[i][0] == 6) { buff += ' ' + g_spells[achievement.rewards[i][1]]['name_' + Locale.getName()] } } } else if (achievement.reward) { buff += ' ' + achievement.reward }
                                return buff
                        }, sortFunc: function (a, b, col) {
                                var text1 = this.getVisibleText(a); var text2 = this.getVisibleText(b); if (text1 != '' && text2 == '') { return -1 }
                                if (text2 != '' && text1 == '') { return 1 }
                                return $WH.strcmp(text1, text2)
                        }
                }, {
                        id: 'category', name: LANG.category, type: 'text', width: '15%', compute: function (achievement, td) {
                                statistic = achievement.type ? achievement.type + '.' : ''; td.className = 'small'; path = '?achievements=' + statistic; if (achievement.category != -1 && achievement.parentcat != -1) { var a2 = $WH.ce('a'); a2.className = 'q0'; a2.href = '?achievements=' + statistic + achievement.parentcat; $WH.ae(a2, $WH.ct(g_achievement_categories[achievement.parentcat])); $WH.ae(td, a2); $WH.ae(td, $WH.ce('br')); path = a2.href + '.' }
                                var a = $WH.ce('a'); a.className = 'q1'; a.href = path + achievement.category; $WH.ae(a, $WH.ct(g_achievement_categories[achievement.category])); $WH.ae(td, a)
                        }, getVisibleText: function (achievement) { return g_achievement_categories[achievement.category] }, sortFunc: function (a, b, col) { return $WH.strcmp(g_achievement_categories[a.category], g_achievement_categories[b.category]) }, hidden: !0
                }], getItemLink: function (achievement) { return '?achievement=' + achievement.id }
        }, title: {
                sort: [1], nItemsPerPage: -1, searchable: 1, filtrable: 1, columns: [{
                        id: 'name', name: LANG.name, type: 'text', align: 'left', value: 'name', compute: function (title, td, tr) {
                                var
                                ...
                                if (title.expansion) { var i = $WH.ce('span'); i.className = g_GetExpansionClassName(title.expansion); $WH.ae(i, sp); $WH.ae(td, i) } else { $WH.ae(td, sp) }
                        }, sortFunc: function (a, b, col) {
                                var
                                aName = $WH.trim(a.name.replace('%s', '').replace(/^[\s,]*(,|the |of the |of )/i, '')); bName = $WH.trim(b.name.replace('%s', '').replace(/^[\s,]*(,|the |of the |of )/i, '')); return $WH.strcmp(aName, bName)
                        }, getVisibleText: function (title) { var buff = title.name + Listview.funcBox.getExpansionText(title); return buff }
                }, { id: 'gender', name: LANG.gender, type: 'text', value: 'gender', compute: function (title, td) { if (title.gender && title.gender != 3) { var gender = g_file_genders[title.gender - 1]; var sp = $WH.ce('span'); sp.className = 'icon-' + gender; g_addTooltip(sp, LANG[gender]); $WH.ae(td, sp) } }, getVisibleText: function (title) { if (title.gender && title.gender != 3) { return LANG[g_file_genders[title.gender - 1]] } }, sortFunc: function (a, b, col) { return $WH.strcmp(a.gender, b.gender) } }, { id: 'side', name: LANG.side, type: 'text', compute: function (title, td) { if (title.side && title.side != 3) { var sp = $WH.ce('span'); sp.className = (title.side == 1 ? 'icon-alliance' : 'icon-horde'); g_addTooltip(sp, g_sides[title.side]); $WH.ae(td, sp) } }, getVisibleText: function (title) { if (title.side) { return g_sides[title.side] } }, sortFunc: function (a, b, col) { return $WH.strcmp(g_sides[a.side], g_sides[b.side]) } }, {
                        id: 'source', name: LANG.source, type: 'text', compute: function (title, td) {
                                if (title.source) {
                                        $WH.nw(td); td.className = 'small'; td.style.lineHeight = '18px'; var n = 0; for (var s in title.source) {
                                                title.source[s].sort(function (a, b) { return b.s - a.s }); for (var i = 0, len = title.source[s].length; i < len; ++i) {
                                                        var sm = title.source[s][i]; var type = 0; if (title.faction && typeof sm != 'string' && sm.s !== undefined && sm.s != -1 && sm.s != 2 - title.faction) { continue }
                                                        if (n++ > 0) { $WH.ae(td, $WH.ce('br')) }
                                                        if (typeof sm == 'string') { $WH.ae(td, $WH.ct(sm)) } else if (sm.t) {
                                                                type = sm.t; var a = $WH.ce('a'); a.href = '?' + g_types[sm.t] + '=' + sm.ti; a.className = 'q1'; if (sm.s == 1) { a.className += ' icon-alliance' }
                                ...
                                var buff = ''; if (title.source) { for (var s in title.source) { for (var i = 0, len = title.source[s].length; i < len; ++i) { var sm = title.source[s][i]; if (typeof sm == 'string') { buff += ' ' + sm } else if (sm.t) { buff += ' ' + sm.n } } } }
                                return buff
                        }, sortFunc: function (a, b, col) { return $WH.strcmp(this.getVisibleText(a), this.getVisibleText(b)) }
                }, { id: 'category', name: LANG.category, type: 'text', width: '15%', compute: function (title, td) { $WH.nw(td); td.className = 'small q1'; var a = $WH.ce('a'); a.href = '?titles=' + title.category; $WH.ae(a, $WH.ct(g_title_categories[title.category])); $WH.ae(td, a) }, getVisibleText: function (title) { return g_title_categories[title.category] }, sortFunc: function (a, b, col) { return $WH.strcmp(g_title_categories[a.category], g_title_categories[b.category]) }, hidden: !0 }], getItemLink: function (title) { return '?title=' + title.id }
        }, profile: {
                sort: [], nItemsPerPage: 50, searchable: 1, filtrable: 1, columns: [{
                        id: 'name', name: LANG.name, value: 'name', type: 'text', align: 'left', span: 2, compute: function (profile, td, tr) {
                                if (profile.level) {
                                        ...
                                        $WH.ae(i, ic); $WH.ae(tr, i); td.style.borderLeft = 'none'
                                } else { td.colSpan = 2 }
                                ...
                                $WH.ae(wrapper, d); $WH.ae(td, wrapper)
                        }, getVisibleText: function (profile) {
                                var buff = profile.name; if (profile.guild && typeof (profile.guild) != 'number') { buff += ' ' + profile.guild }
                                return buff
                        }
                }, {
                        id: 'faction', name: LANG.faction, type: 'text', compute: function (profile, td) {
                                if (!profile.size && profile.members === undefined && !profile.level) { return }
                                var d = $WH.ce('div'), d2 = $WH.ce('div'), icon; icon = Icon.create('faction_' + g_file_factions[parseInt(profile.faction) + 1], 0); icon.style.cssFloat = icon.style.syleFloat = 'left'; g_addTooltip(icon, g_sides[parseInt(profile.faction) + 1]); d.style.margin = '0 auto'; d.style.textAlign = 'left'; d.style.width = '26px'; d2.className = 'clear'; $WH.ae(d, icon); $WH.ae(td, d); $WH.ae(td, d2)
                        }, getVisibleText: function (profile) { return g_sides[profile.faction + 1] }, sortFunc: function (a, b, col) { return $WH.strcmp(this.getVisibleText(a), this.getVisibleText(b)) }
                }, { id: 'members', name: LANG.members, value: 'members', hidden: 1 }, { id: 'size', name: 'Size', value: 'size', hidden: 1 }, { id: 'rank', name: 'Rank', value: 'rank', hidden: 1 }, {
                        id: 'race', name: LANG.race, type: 'text', compute: function (profile, td) {
                                if (profile.race) {
                                        var
                        ...
                        }, getVisibleText: function (profile) { return g_file_genders[profile.gender] + ' ' + g_chr_races[profile.race] }, sortFunc: function (a, b, col) { return $WH.strcmp(g_chr_races[a.race], g_chr_races[b.race]) }, hidden: 1
                }, {
                        id: 'classs', name: LANG.classs, type: 'text', compute: function (profile, td) {
                                if (profile.classs) {
                                        var
                        ...
                        }, getVisibleText: function (profile) { if (profile.classs) { return g_chr_classes[profile.classs] } }, sortFunc: function (a, b, col) { return $WH.strcmp(this.getVisibleText(a), this.getVisibleText(b)) }, hidden: 1
                }, { id: 'level', name: LANG.level, value: 'level', hidden: 1 }, {
                        id: 'talents', name: LANG.talents, type: 'text', compute: function (profile, td) {
                                if (!profile.level) { return }
                                var spent = [profile.talenttree1, profile.talenttree2, profile.talenttree3]; var specData = pr_getSpecFromTalents(profile.classs, spent); var a = $WH.ce('a'); a.className = 'icontiny tinyspecial tip q1'; a.style.backgroundImage = 'url(' + g_staticUrl + '/images/wow/icons/tiny/' + specData.icon.toLowerCase() + '.gif)'; a.rel = 'np'; a.href = this.getItemLink(profile) + '#talents'; g_addTooltip(a, specData.name); $WH.ae(a, $WH.ct(profile.talenttree1 + ' / ' + profile.talenttree2 + ' / ' + profile.talenttree3)); $WH.ae(td, a)
                        }, getVisibleText: function (profile) { if (profile.talenttree1 || profile.talenttree2 || profile.talenttree3) { if (profile.talentspec > 0) { return g_chr_specs[profile.classs][profile.talentspec - 1] } else { return g_chr_specs[0] } } else { return g_chr_specs['-1'] } }, sortFunc: function (a, b, col) { return $WH.strcmp(this.getVisibleText(a), this.getVisibleText(b)) }, hidden: 1
                }, {
                        id: 'gearscore', name: LANG.gearscore, tooltip: LANG.gearscore_real, value: 'gearscore', compute: function (profile, td) {
                                var level = (profile.level ? profile.level : (profile.members !== undefined ? 80 : 0)); if (isNaN(profile.gearscore) || !level)
                                        return; td.className = 'q' + pr_getGearScoreQuality(level, profile.gearscore, ($WH.in_array([2, 6, 7, 11], profile.classs) != -1)); return (profile.gearscore ? $WH.number_format(profile.gearscore) : 0)
                        }, hidden: 1
                }, { id: 'achievementpoints', name: LANG.points, value: 'achievementpoints', tooltip: LANG.tooltip_achievementpoints, compute: function (profile, td) { if (profile.achievementpoints) { Listview.funcBox.appendMoney(td, 0, null, 0, 0, profile.achievementpoints) } }, hidden: 1 }, { id: 'wins', name: LANG.wins, value: 'wins', hidden: 1 }, {
                        id: 'losses', name: LANG.losses, compute: function (profile, td) { return profile.games - profile.wins }, sortFunc: function (a, b, col) {
                                var
                                lossA = a.games - a.wins, lossB = b.games - b.wins; if (lossA > lossB)
                                        return 1; if (lossA < lossB)
                                        return -1; return 0
                        }, hidden: 1
                }, { id: 'guildrank', name: LANG.guildrank, value: 'guildrank', compute: function (profile, td) { if (typeof guild_ranks !== "undefined" && guild_ranks[profile.guildrank]) { var sp = $WH.ce('span', null, $WH.ct(guild_ranks[profile.guildrank])); g_addTooltip(sp, $WH.sprintf(LANG.rankno, profile.guildrank)); $WH.ae(td, sp) } else if (profile.guildrank > 0) { return $WH.sprintf(LANG.rankno, profile.guildrank) } else if (profile.guildrank == 0) { var b = $WH.ce('b'); $WH.ae(b, $WH.ct(LANG.guildleader)); $WH.ae(td, b) } }, getVisibleText: function (profile) { if (profile.guildrank > 0) { return $WH.sprintf(LANG.rankno, profile.guildrank) } else if (profile.guildrank == 0) { return LANG.guildleader } }, sortFunc: function (a, b, col) { return $WH.strcmp((a.guildrank >= 0 ? a.guildrank : 11), (b.guildrank >= 0 ? b.guildrank : 11)) }, hidden: 1 }, { id: 'rating', name: LANG.rating, value: 'rating', hidden: 1 }, {
                        id: 'location', name: LANG.location, type: 'text', compute: function (profile, td) {
                                var a; if (profile.region) {
                                        if (profile.realm) { a = $WH.ce('a'); a.className = 'q1'; a.href = '?profiles=' + profile.region + '.' + profile.realm; $WH.ae(a, $WH.ct(profile.realmname)); $WH.ae(td, a); $WH.ae(td, $WH.ce('br')) }
                                ...
                                var buff = ''; if (profile.region) { buff += ' ' + profile.region }
                                if (profile.battlegroup) { buff += ' ' + profile.battlegroup }
                                if (profile.realm) { buff += ' ' + profile.realm }
                                return $WH.trim(buff)
                        }, sortFunc: function (a, b, col) {
                                if (a.region != b.region) { return $WH.strcmp(a.region, b.region) }
                                if (a.battlegroup != b.battlegroup) { return $WH.strcmp(a.battlegroup, b.battlegroup) }
                                return $WH.strcmp(a.realm, b.realm)
                        }
                }, {
                        id: 'guild', name: LANG.guild, value: 'guild', type: 'text', compute: function (profile, td) {
                                if (!profile.region || !profile.realm || !profile.guild || typeof (profile.guild) == 'number') { return }
                                var a = $WH.ce('a'); a.className = 'q1'; a.href = '?guild=' + profile.region + '.' + profile.realm + '.' + g_urlize(profile.guild); $WH.ae(a, $WH.ct(profile.guild)); $WH.ae(td, a)
                        }, sortFunc: function (a, b, col) { return $WH.strcmp(a.guild, b.guild) }
                }], getItemLink: function (profile) { if (profile.size !== undefined) { return '?arena-team=' + profile.region + '.' + profile.realm + '.' + g_urlize(profile.name) } else if (profile.members !== undefined) { return '?guild=' + profile.region + '.' + profile.realm + '.' + g_urlize(profile.name) } else { return g_getProfileUrl(profile) } }
        }, model: {
                sort: [], mode: 3, nItemsPerPage: 40, nItemsPerRow: 4, poundable: 2, columns: [], compute: function (model, td, i) { td.className = 'screenshot-cell'; td.vAlign = 'bottom'; var a = $WH.ce('a'); a.href = 'javascript:;'; a.onclick = this.template.modelShow.bind(this.template, model.npcId, model.displayId, !1); var img = $WH.ce('img'); img.src = g_staticUrl + '/modelviewer/thumbs/npc/' + model.displayId + '.png'; $WH.ae(a, img); $WH.ae(td, a); var d = $WH.ce('div'); d.className = 'screenshot-cell-user'; a = $WH.ce('a'); a.href = '?npcs=1&filter=' + (model.family ? 'fa=' + model.family + ';' : '') + 'minle=1;cr=35;crs=0;crv=' + model.skin; $WH.ae(a, $WH.ct(model.skin)); $WH.ae(d, a); $WH.ae(d, $WH.ct(' (' + model.count + ')')); $WH.ae(td, d); d = $WH.ce('div'); d.style.position = 'relative'; d.style.height = '1em'; var d2 = $WH.ce('div'); d2.className = 'screenshot-caption'; var s = $WH.ce('small'); $WH.ae(s, $WH.ct(LANG.level + ': ')); $WH.ae(s, $WH.ct((model.minLevel == 9999 ? '??' : model.minLevel) + (model.minLevel == model.maxLevel ? '' : LANG.hyphen + (model.maxLevel == 9999 ? '??' : model.maxLevel)))); $WH.ae(s, $WH.ce('br')); $WH.ae(d2, s); $WH.ae(d, d2); $WH.ae(td, d); $WH.aE(td, 'click', this.template.modelShow.bind(this.template, model.npcId, model.displayId, !0)) }, modelShow: function (npcId, displayId, sp, e) {
                        if (sp) {
                                e = $WH.$E(e); if (e.shiftKey || e.ctrlKey) { return }
                                var j = 0, el = e._target; while (el && j < 3) {
                                        if (el.nodeName == 'A') { return }
        ...
        }, currency: { sort: [1], searchable: 1, filtrable: 1, columns: [{ id: 'name', name: LANG.name, type: 'text', align: 'left', span: 2, value: 'name', compute: function (currency, td, tr) { var i = $WH.ce('td'); i.style.width = '1px'; i.style.padding = '0'; i.style.borderRight = 'none'; $WH.ae(i, Icon.create(currency.icon, 0, null, this.getItemLink(currency))); $WH.ae(tr, i); td.style.borderLeft = 'none'; var wrapper = $WH.ce('div'); var a = $WH.ce('a'); a.style.fontFamily = 'Verdana, sans-serif'; a.href = this.getItemLink(currency); $WH.ae(a, $WH.ct(currency.name)); $WH.ae(wrapper, a); $WH.ae(td, wrapper) } }, { id: 'category', name: LANG.category, type: 'text', width: '15%', compute: function (currency, td) { td.className = 'small'; var a = $WH.ce('a'); a.className = 'q1'; a.href = '?currencies=' + currency.category; $WH.ae(a, $WH.ct(g_currency_categories[currency.category])); $WH.ae(td, a) }, getVisibleText: function (currency) { return g_currency_categories[currency.category] }, sortFunc: function (a, b, col) { return $WH.strcmp(g_currency_categories[a.category], g_currency_categories[b.category]) } }], getItemLink: function (currency) { return '?currency=' + currency.id } }, classs: {
                sort: [1], searchable: 1, filtrable: 1, columns: [{
                        id: 'name', name: LANG.name, type: 'text', align: 'left', span: 2, value: 'name', compute: function (classs, td, tr) {
                                var i = $WH.ce('td'); i.style.width = '1px'; i.style.padding = '0'; i.style.borderRight = 'none'; $WH.ae(i, Icon.create('class_' + g_file_classes[classs.id], 0, null, this.getItemLink(classs))); $WH.ae(tr, i); td.style.borderLeft = 'none'; td.style.width = '270px'; var wrapper = $WH.ce('div'); var a = $WH.ce('a'); a.className = 'c' + classs.id; a.style.fontFamily = 'Verdana, sans-serif'; a.href = this.getItemLink(classs); $WH.ae(a, $WH.ct(classs.name)); if (classs.expansion) { var sp = $WH.ce('span'); sp.className = g_GetExpansionClassName(classs.expansion); $WH.ae(sp, a); $WH.ae(wrapper, sp) } else { $WH.ae(wrapper, a) }
                        ...
                        id: 'races', name: LANG.races, type: 'text', compute: function (classs, td) {
                                if (classs.races) {
                                        var races = Listview.funcBox.assocBinFlags(classs.races, g_chr_races); td.className = 'q1'; for (var i = 0, len = races.length; i < len; ++i) {
                                                if (i > 0) { $WH.ae(td, $WH.ct(LANG.comma)) }
                                                var a = $WH.ce('a'); a.href = '?race=' + races[i]; $WH.ae(a, $WH.ct(g_chr_races[races[i]])); $WH.ae(td, a)
                                        }
                                }
                        }, getVisibleText: function (classs) { if (classs.races) { return Listview.funcBox.arrayText(Listview.funcBox.assocBinFlags(classs.races, g_chr_races), g_chr_races) } }, sortFunc: function (a, b, col) { return Listview.funcBox.assocArrCmp(Listview.funcBox.assocBinFlags(a.races, g_chr_races), Listview.funcBox.assocBinFlags(b.races, g_chr_races), g_chr_races) }
                }], getItemLink: function (classs) { return '?class=' + classs.id }
        }, race: {
                sort: [1], searchable: 1, filtrable: 1, columns: [{
                        id: 'name', name: LANG.name, type: 'text', align: 'left', span: 2, value: 'name', compute: function (race, td, tr) {
                                var
                ...
                }, { id: 'side', name: LANG.side, type: 'text', compute: function (race, td) { if (race.side && race.side != 3) { var sp = $WH.ce('span'); sp.className = (race.side == 1 ? 'icon-alliance' : 'icon-horde'); g_addTooltip(sp, g_sides[race.side]); $WH.ae(td, sp) } }, getVisibleText: function (race) { if (race.side) { return g_sides[race.side] } }, sortFunc: function (a, b, col) { return $WH.strcmp(g_sides[a.side], g_sides[b.side]) } }, {
                        id: 'classes', name: LANG.classes, type: 'text', compute: function (race, td) {
                                if (race.classes) {
                                        var classes = Listview.funcBox.assocBinFlags(race.classes, g_chr_classes); var d = $WH.ce('div'); d.style.width = (26 * classes.length) + 'px'; d.style.margin = '0 auto'; for (var i = 0, len = classes.length; i < len; ++i) { var icon = Icon.create('class_' + g_file_classes[classes[i]], 0, null, '?class=' + classes[i]); g_addTooltip(icon, g_chr_classes[classes[i]], 'c' + classes[i]); icon.style.cssFloat = icon.style.styleFloat = 'left'; $WH.ae(d, icon) }
                                        $WH.ae(td, d)
                                }
                        }, getVisibleText: function (race) { if (race.classes) { return Listview.funcBox.arrayText(Listview.funcBox.assocBinFlags(race.classes, g_chr_classes), g_chr_classes) } }, sortFunc: function (a, b, col) { return Listview.funcBox.assocArrCmp(Listview.funcBox.assocBinFlags(a.classes, g_chr_classes), Listview.funcBox.assocBinFlags(b.classes, g_chr_classes), g_chr_classes) }
                }], getItemLink: function (race) { return '?race=' + race.id }
        }
}; var MENU_IDX_ID = 0; var MENU_IDX_NAME = 1; var MENU_IDX_URL = 2; var MENU_IDX_SUB = 3; var MENU_IDX_OPT = 4; var Menu = new function () {
        var self = this; self.add = function (node, menu, opt) { if (!opt) opt = $.noop; var $node = $(node); $node.data('menu', menu); if (opt.showAtCursor) { $node.click(rootNodeClick) } else { $node.mouseover(rootNodeOver).mouseout(rootNodeOut) } }; self.remove = function (node) { $(node).data('menu', null).unbind('click', rootNodeClick).unbind('mouseover', rootNodeOver).unbind('mouseout', rootNodeOut) }; self.show = function (menu, node) { var $node = $(node); show(menu, $node) }
        ...
        self.addButtons = function (dest, menu) {
                var $dest = $(dest); if (!$dest.length)
                        return; var $buttons = $('<span class="menu-buttons"></span>'); $.each(menu, function (idx, menuItem) {
                                if (isSeparator(menuItem)) return; var $a = $('<a></a>')
        ...
        }; self.linkifyItem = function (menuItem, $a) {
                var opt = self.getItemOpt(menuItem); if (!menuItem[MENU_IDX_URL]) { $a.attr('href', 'javascript:;'); $a.addClass('unlinked'); return }
                if (typeof menuItem[MENU_IDX_URL] == 'function') { $a.attr('href', 'javascript:;'); $a.click(hide); $a.click(menuItem[MENU_IDX_URL]) } else {
                        var url = self.getItemUrl(menuItem); $a.attr('href', url); if (opt.newWindow || g_isExternalUrl(url))
        ...
        }; self.updateItem = function (menuItem) {
                var $a = menuItem.$a; if (!$a)
        ...
        }; self.hasMenu = function (node) { var $node = $(node); return $node.data('menu') != null }
        self.modifyUrl = function (menuItem, params, opt) { var json = { params: params, opt: opt }; traverseMenuItem(menuItem, function (x) { x.modifyUrl = json }); PageTemplate.updateBreadcrumb() }; self.fixUrls = function (menu, url, opt) { opt = opt || {}; opt.hash = (opt.hash ? '#' + opt.hash : ''); fixUrls(menu, url, opt, 0) }; self.sort = function (menu) {
                if (hasSeparators(menu))
                        sortSeparatedMenu(menu); else sortMenu(menu)
        }; self.sortSubmenus = function (menu, paths) {
                $.each(paths, function (idx, path) {
                        var submenu = self.findItem(menu, path); if (submenu && submenu[MENU_IDX_SUB])
                                self.sort(submenu[MENU_IDX_SUB])
                })
        }; self.implode = function (menu, opt) {
                if (!opt) opt = $.noop; var result = []; var groupedMenu; if (opt.createHeadinglessGroup) { groupedMenu = []; result.push([0, '', null, groupedMenu]) }
                $.each(menu, function (idx, menuItem) {
                        if (isSeparator(menuItem)) { groupedMenu = []; result.push([0, menuItem[MENU_IDX_NAME], null, groupedMenu]) } else {
                                if (groupedMenu)
                                        groupedMenu.push(menuItem); else result.push(menuItem)
                        }
                }); return result
        }; self.findItem = function (menu, path) { return self.getFullPath(menu, path).pop() }; self.getFullPath = function (menu, path) {
                var result = []; for (var i = 0; i < path.length; ++i) { var pos = findItemPosById(menu, path[i]); if (pos != -1) { var menuItem = menu[pos]; menuItem.parentMenu = menu; menu = menuItem[MENU_IDX_SUB]; result.push(menuItem) } }
                return result
        }; self.getItemUrl = function (menuItem) {
                var url = menuItem[MENU_IDX_URL]; if (!url)
                        return null; var opt = self.getItemOpt(menuItem); if (menuItem.modifyUrl)
                        url = g_modifyUrl(url, menuItem.modifyUrl.params, menuItem.modifyUrl.opt); return url
        }; self.getItemOpt = function (menuItem) {
                if (!menuItem[MENU_IDX_OPT])
                        menuItem[MENU_IDX_OPT] = {}; return menuItem[MENU_IDX_OPT]
        }; self.removeItemById = function (menu, id) {
                var pos = findItemPosById(menu, id); if (pos != -1)
                        menu.splice(pos, 1)
        }; var DELAY_BEFORESHOW = 25; var DELAY_BEFOREHIDE = 333; var MAX_COLUMNS = 4; var SIZE_SHADOW_WIDTH = 6; var SIZE_SHADOW_HEIGHT = 6; var SIZE_BORDER_HEIGHT = 3; var SIZE_MENUITEM_HEIGHT = 26; var inited = !1; var timer; var $rootNode; var visibleMenus = {}; var openLinks = {}; var divCache = {}; var menuItemsCache = {}; var nextUniqueId = 0; function init() {
                if (inited) return; inited = !0; var $div = $('<div class="menu"><a href="#"><span>ohai</span></a></div>').css({ left: '-1000px', top: '-1000px' }).appendTo(document.body); var height = $div.children('a').outerHeight(); $div.remove(); if (height > 15)
        ...
        function hide() {
                if ($rootNode) { $rootNode.removeClass('open'); $rootNode = null }
                hideMenus(0)
        }
        ...
        function createDiv(depth) {
                if (divCache[depth]) { var $div = divCache[depth]; $div.children().detach(); return $div }
        ...
        function createMenuItems(menu) {
                var uid = getUniqueId(menu); if (menuItemsCache[uid])
                        return menuItemsCache[uid]; var nextSeparator; var menuItems = []; $.each(menu, function (idx, menuItem) {
                                if (!isItemVisible(menuItem)) return; $a = createMenuItem(menuItem); if (isSeparator(menuItem)) { nextSeparator = $a; return }
                                if (nextSeparator) { menuItems.push(nextSeparator); nextSeparator = null }
                                menuItems.push($a)
                        }); var $menuItems = $(menuItems); menuItemsCache[menu] = $menuItems; return $menuItems
        }
        function createMenuItem(menuItem) {
                beforeCreateMenuItem(menuItem); var $a = $('<a></a>'); menuItem.$a = $a; $a.data('menuItem', menuItem); self.linkifyItem(menuItem, $a); self.updateItem(menuItem); if (isSeparator(menuItem)) { $a.addClass('separator'); $a.text(menuItem[MENU_IDX_NAME]); return $a }
        ...
        function getFirstMenuPosition($div, depth, x, y) {
                var viewport = g_getViewport(); var menuWidth = $div.width(); var menuHeight = $div.height(); var menuWidthShadow = menuWidth + SIZE_SHADOW_WIDTH; var menuHeightShadow = menuHeight + SIZE_SHADOW_HEIGHT; var showingAtCursor = (x != null && y != null); if (showingAtCursor) {
                        if (y + menuHeightShadow > viewport.b)
                                y = Math.max(viewport.t, viewport.b - menuHeightShadow)
                } else {
                        var $a = $rootNode; var offset = $a.offset(); x = offset.left
        ...
        function getSubmenuPosition($div, depth) {
                var viewport = g_getViewport(); var menuWidth = $div.width(); var menuHeight = $div.height(); var menuWidthShadow = menuWidth + SIZE_SHADOW_WIDTH; var menuHeightShadow = menuHeight + SIZE_SHADOW_HEIGHT; var $a = openLinks[depth - 1]; var offset = $a.offset(); var openToTheLeft = !1; x = offset.left + $a.outerWidth() - 5; y = offset.top - 2; if (x + menuWidthShadow > viewport.r)
        ...
        function hasPermissions(menuItem) {
                var opt = self.getItemOpt(menuItem); if (opt.requiredAccess && !User.hasPermissions(opt.requiredAccess))
                        return !1; return !0
        }
        function isItemVisible(menuItem) {
                if (!hasPermissions(menuItem))
                        return !1; if (hasSubmenu(menuItem)) {
        ...
        function traverseMenuItem(menuItem, func) {
                func(menuItem); if (hasSubmenu(menuItem))
                        traverseMenu(menuItem[MENU_IDX_SUB], func)
        }
        function fixUrls(menu, url, opt, depth) {
                $.each(menu, function (idx, menuItem) {
                        if (menuItem === undefined) return; if (isSeparator(menuItem)) return; if (menuItem[MENU_IDX_URL] == null)
                                menuItem[MENU_IDX_URL] = url + menuItem[MENU_IDX_ID] + opt.hash; if (hasSubmenu(menuItem)) {
                                        var accumulate = !0; if (opt.useSimpleIds)
        ...
        function sortSeparatedMenu(menu) { var implodedMenu = self.implode(menu, { createHeadinglessGroup: !0 }); $.each(implodedMenu, function (idx, submenu) { sortMenu(submenu[MENU_IDX_SUB]) }); explodeInto(menu, implodedMenu) }; function explodeInto(menu, implodedMenu) {
                menu.splice(0, menu.length); $.each(implodedMenu, function (idx, menuItem) {
        ...
        function beforeCreateMenuItem(menuItem) {
                var opt = self.getItemOpt(menuItem); if (opt.checkedUrl && location.href.match(opt.checkedUrl))
                        menuItem.checked = !0
        }
        function beforeShowMenu(menu) {
                if (menu.onBeforeShow)
                        menu.onBeforeShow(menu); $.each(menu, function (idx, menuItem) {
                                var opt = self.getItemOpt(menuItem); if (opt.onBeforeShow)
                                        opt.onBeforeShow(menuItem)
                        })
        }
        function rootNodeOver(event) {
                clearTimeout(timer); var $node = $(this); if (!isMenuVisible()) { timer = setTimeout(show.bind(null, $node.data('menu'), $node), DELAY_BEFORESHOW); return }
                show($node.data('menu'), $node)
        }
        function rootNodeOut(event) {
                clearTimeout(timer); if (isMenuVisible())
...
var ProgressBar = function (opt) { this.opts = { text: '', hoverText: '', color: 'rep6', width: 0, progress: 0 }; this.elements = { text: null, hoverText: null, textContainer: null, progress: null, container: null }; $WH.cO(this.opts, opt); this.build() }; ProgressBar.prototype.build = function () {
        var el = $('<a/>', { 'class': 'progressbar', href: 'javascript:;' }); if (this.opts.width > 0)
        ...
        el.append(textDiv); var div = $('<div/>', { 'class': 'progressbar-' + this.opts.color, css: { width: this.opts.progress + '%' }, text: String.fromCharCode(160) }); el.append(div); if (this.opts.text)
                textDiv.append($('<div/>', { 'class': 'progressbar-text progressbar-hidden', text: this.opts.text })); this.elements.container = el; this.elements.progress = div; this.elements.textContainer = textDiv; return el
}; ProgressBar.prototype.setText = function (text) {
        this.opts.text = text; if (this.elements.text)
                this.elements.text.text(this.opts.text); else {
                        this.elements.text = $('<del/>', { text: this.opts.text }); if (this.opts.hoverText)
                                this.opts.hoverText.before(this.elements.text); else this.elements.textContainer.append(this.elements.text)
        }
}; ProgressBar.prototype.setHoverText = function (text) {
        this.opts.hoverText = text; if (this.elements.hoverText)
                this.elements.hoverText.text(this.opts.hoverText); else { this.elements.hoverText = $('<ins/>', { text: this.opts.hoverText }); this.elements.textContainer.append(this.elements.hoverText) }
}; ProgressBar.prototype.setProgress = function (percent) { this.opts.progress = percent; this.elements.progress.css('width', this.opts.progress + '%') }; ProgressBar.prototype.setWidth = function (width) {
        this.opts.width = width; if (this.opts.width > 0)
                this.elements.container.css('width', this.opts.width + 'px'); else this.elements.container.css('width', 'auto')
}; ProgressBar.prototype.getText = function () { return this.opts.text }; ProgressBar.prototype.getHoverText = function () { return this.opts.hoverText }; ProgressBar.prototype.getWidth = function () { return this.opts.width }; ProgressBar.prototype.getContainer = function () { return this.elements.container }; var Icon = {
        sizes: ['small', 'medium', 'large'], sizes2: [18, 36, 56], sizeIds: { small: 0, medium: 1, large: 2 }, premiumOffsets: [[-56, -36], [-56, 0], [0, 0]], premiumBorderClasses: ['-premium', '-gold', '', '-premiumred', '-red'], STANDARD_BORDER: 2, privilegeBorderClasses: { uncommon: '-q2', rare: '-q3', epic: '-q4', legendary: '-q5' }, create: function (name, size, UNUSED, url, num, qty, noBorder, rel, span) {
                var
                ...
                Icon.setTexture(icon, size, name); if (url) {
                        var a = $WH.ce('a'); a.href = url; if (url.indexOf('wowhead.com') == -1 && url.substr(0, 5) == 'http:') { a.target = "_blank" }
                        $WH.ae(icon, a)
                } else if (name) { var _ = icon.firstChild.style; var avatarIcon = (_.backgroundImage.indexOf('/avatars/') != -1); if (!avatarIcon) { icon.onclick = Icon.onClick; if (url !== !1) { var a = $WH.ce('a'); a.href = "javascript:;"; $WH.ae(icon, a) } } }
                if (rel && typeof a != 'undefined') { a.rel = rel }
                Icon.setNumQty(icon, num, qty); return icon
        }, createUser: function (avatar, avatarMore, size, url, premiumLevel, noBorder, reputationLevel) {
                if (avatar == 2) { avatarMore = g_staticUrl + '/uploads/avatars/' + avatarMore + '.jpg' }
                ...
                var buff = !1; if (reputation >= 5000) { buff = 'uncommon' }
        ...
        }, moveTexture: function (icon, size, x, y, exact) { var _ = icon.firstChild.style; if (x || y) { if (exact) { _.backgroundPosition = x + 'px ' + y + 'px' } else { _.backgroundPosition = (-x * Icon.sizes2[size]) + 'px ' + (-y * Icon.sizes2[size]) + 'px' } } else if (_.backgroundPosition) { _.backgroundPosition = '' } }, setNumQty: function (icon, num, qty) {
                var _ = $WH.gE(icon, 'span'); for (var i = 0, len = _.length; i < len; ++i) { if (_[i]) { $WH.de(_[i]) } }
                ...
                if (qty != null && qty > 0) { _ = g_createGlow('(' + qty + ')', 'q'); _.style.left = '0'; _.style.top = '0'; _.style.position = 'absolute'; $WH.ae(icon, _) }
        }, getLink: function (icon) { return $WH.gE(icon, 'a')[0] }, showIconName: function (x) {
                if (x.firstChild) {
                        var _ = x.firstChild.style; if (_.backgroundImage.length && (_.backgroundImage.indexOf(g_staticUrl) >= 4 || g_staticUrl == '')) {
                                var
                                start = _.backgroundImage.lastIndexOf('/'), end = _.backgroundImage.indexOf('.jpg'); if (start != -1 && end != -1) { Icon.displayIcon(_.backgroundImage.substring(start + 1, end)) }
                        }
                }
        }, onClick: function () { Icon.showIconName(this) }, displayIcon: function (icon) {
                if (!Dialog.templates.icondisplay) {
                        ...
                        Dialog.templates.icondisplay = {
                                title: LANG.icon, width: w, buttons: [['arrow', LANG.original], ['cancel', LANG.close]], fields: [{ id: 'icon', label: LANG.dialog_imagename, required: 1, type: 'text', labelAlign: 'left', compute: function (field, value, form, td) { var wrapper = $WH.ce('div'); td.style.width = '300px'; wrapper.style.position = 'relative'; wrapper.style.cssFloat = 'left'; wrapper.style.paddingRight = '6px'; field.style.width = '200px'; var divIcon = this.iconDiv = $WH.ce('div'); divIcon.style.position = 'absolute'; divIcon.style.top = '-12px'; divIcon.style.right = '-70px'; divIcon.update = function () { setTimeout(function () { field.focus(); field.select() }, 10); $WH.ee(divIcon); $WH.ae(divIcon, Icon.create(field.value, 2)) }; $WH.ae(divIcon, Icon.create(value, 2)); $WH.ae(wrapper, divIcon); $WH.ae(wrapper, field); $WH.ae(td, wrapper) } }, { id: 'location', label: " ", required: 1, type: 'caption', compute: function (field, value, form, th, tr) { $WH.ee(th); th.style.padding = '3px 3px 0 3px'; th.style.lineHeight = '17px'; th.style.whiteSpace = 'normal'; var wrapper = $WH.ce('div'); wrapper.style.position = 'relative'; wrapper.style.width = '250px'; var span = $WH.ce('span'); var text = LANG.dialog_seeallusingicon; text = text.replace('$1', '<a href="?items&filter=cr=142;crs=0;crv=' + this.data.icon + '">' + LANG.types[3][3] + '</a>'); text = text.replace('$2', '<a href="?spells&filter=cr=15;crs=0;crv=' + this.data.icon + '">' + LANG.types[6][3] + '</a>'); text = text.replace('$3', '<a href="?achievements&filter=cr=10;crs=0;crv=' + this.data.icon + '">' + LANG.types[10][3] + '</a>'); span.innerHTML = text; $WH.ae(wrapper, span); $WH.ae(th, wrapper) } }], onInit: function (form) { this.updateIcon = this.template.updateIcon.bind(this, form) }, onShow: function (form) {
                                        ...
                                        location.hash = hash
                                }, onHide: function (form) { if (this.oldHash) { location.hash = this.oldHash } else { location.hash = '#.' } }, updateIcon: function (form) { this.iconDiv.update() }, onSubmit: function (unused, data, button, form) {
                                        if (button == 'arrow') { var win = window.open(g_staticUrl + '/images/wow/icons/large/' + data.icon.toLowerCase() + '.jpg', '_blank'); win.focus(); return !1 }
                                        return !0
                                }
                        }
                }
                ...
                if (location.hash && location.hash.indexOf('#icon') == 0) {
                        var parts = location.hash.split(':'); var icon = !1; if (parts.length == 2) { icon = parts[1] } else if (parts.length == 1 && $WH.isset('g_pageInfo')) { switch (g_pageInfo.type) { case 3: icon = g_items[g_pageInfo.typeId].icon.toLowerCase(); break; case 6: icon = g_spells[g_pageInfo.typeId].icon.toLowerCase(); break; case 10: icon = g_achievements[g_pageInfo.typeId].icon.toLowerCase(); break } }
                        if (icon) { Icon.displayIcon(icon) }
                }
        }
}; DomContentLoaded.addEvent(Icon.checkPound); function Rectangle(left, top, width, height) { this.l = left; this.t = top; this.r = left + width; this.b = top + height }
function g_getViewport() { var win = $(window); return new Rectangle(win.scrollLeft(), win.scrollTop(), win.width(), win.height()) }
Rectangle.prototype = { intersectWith: function (rect) { var result = !(this.l >= rect.r || rect.l >= this.r || this.t >= rect.b || rect.t >= this.b); return result }, contains: function (rect) { var result = (this.l <= rect.l && this.t <= rect.t && this.r >= rect.r && this.b >= rect.b); return result }, containedIn: function (rect) { return rect.contains(this) } }; var RedButton = {
        create: function (text, enabled, func) {
                var
                a = $WH.ce('a'), em = $WH.ce('em'), b = $WH.ce('b'), i = $WH.ce('i'), span = $WH.ce('span'); a.href = 'javascript:;'; a.className = 'button-red'; $WH.ae(b, i); $WH.ae(em, b); $WH.ae(em, span); $WH.ae(a, em); RedButton.setText(a, text); RedButton.enable(a, enabled); RedButton.setFunc(a, func); return a
        }, setText: function (button, text) { $WH.st(button.firstChild.childNodes[0].firstChild, text); $WH.st(button.firstChild.childNodes[1], text) }, enable: function (button, enabled) { if (enabled || enabled == null) { button.className = button.className.replace('button-red-disabled', '') } else if (button.className.indexOf('button-red-disabled') == -1) { button.className += ' button-red-disabled' } }, setFunc: function (button, func) { button.onclick = (func ? func : null) }
        ...
        function hide(e) {
                if (e && !g_isLeftClick(e)) { return }
                if (container) { container.style.display = 'none' }
        }
        function highlight(match, $1) { return ($1 ? match : '<b><u>' + match + '</u></b>') }
        function display(textbox, search, suggz, dataz) {
                prepare(); show(); lastA = null; hasData = 1; selection = null; while (container.firstChild) { $WH.de(container.firstChild) }
                search = search.replace(/[^a-z0-9\-]/gi, ' '); search = $WH.trim(search.replace(/\s+/g, ' ')); var regex = g_createOrRegex(search, (search == 's' ? '[%]' : null)); for (var i = 0, len = suggz.length; i < len; ++i) {
        ...
        function fetch(textbox, search) {
                var url = '?search=' + $WH.urlencode(search); if (textbox._type) { url += '&json&type=' + textbox._type } else { url += '&opensearch' }
        ...
        function onKeyUp(e) {
                e = $WH.$E(e); var textbox = e._target; switch (e.keyCode) {
                        case 48: case 96: case 107: case 109: if ($WH.Browser.firefox && e.ctrlKey) { adjust(textbox); break }
                                break
                }
        ...
        this.attach = function (textbox) {
                textbox = $(textbox); if (!textbox.length) { return }
                textbox = textbox[0]; if (textbox.getAttribute('autocomplete') == 'off') { return }
                textbox.setAttribute('autocomplete', 'off'); $WH.aE(textbox, 'focus', onFocus); $WH.aE(textbox, 'keyup', onKeyUp); $WH.aE(textbox, 'keydown', onKeyDown)
        ...
        function onResize(fake) {
                if (fake != 1234) { if (funcs.onResize) { funcs.onResize() } }
        ...
        this.setSize = function (w, h, auto) {
                if (!$WH.Browser.ie) { inner.style.visibility = 'hidden' }
                ...
                funcs = _funcs || {}; prepare(); hookEvents(); if (lastId != id && divs[lastId] != null) { divs[lastId].style.display = 'none' }
                ...
                onResize(1234); reveal(); g_enableScroll(!1)
        }; this.reveal = function () { reveal() }; this.hide = function () { hide() }; this.isVisible = function () { return (overlay && overlay.style.display != 'none') }
}; function Mapper(opt, noScroll) {
        $WH.cO(this, opt); if (this.parent && !this.parent.nodeName) { this.parent = $WH.ge(this.parent) } else if (!this.parent) { return }
        ...
        if (opt.objectives)
                ...
                this.setZones(opt.zoneparent, opt.zones); this.updateMap(noScroll)
}; Mapper.sizes = [[488, 325, 'normal'], [772, 515, 'zoom'], [1002, 668, 'original'], [224, 149, 'small']]; Mapper.onlyOneFloor = { 4265: !0, 4264: !0, 4416: !0, 4415: !0, 4493: !0, 4500: !0, 4603: !0, 4723: !0, 4809: !0, 4813: !0, 4820: !0 }; Mapper.zoneLevelOffset = { 4273: 0 }; Mapper.zoneDefaultLevel = { 3456: 4, 4812: 4 }; Mapper.remappedLevels = { 4273: { 6: 5 } }; Mapper.multiLevelZones = {}; Mapper.prototype = {
        getMap: function () { return this.parent }, update: function (opt, noScroll) {
                if (opt.zoom != null) { this.zoom = opt.zoom }
                ...
                var level = (opt.level === undefined ? 0 : this.fixLevel(parseInt(opt.level))); if (!opt.preservelevel)
                        ...
                        mapperData = g_mapper_data; if (mapperData && mapperData[this.zone] && !opt.coords) {
                                var zone = mapperData[this.zone]; var maxCount = -1; for (var i in zone) {
                                        i = parseInt(i); var iLevel = this.fixLevel(i); if (opt.level === undefined && zone[i].count > maxCount) { level = parseInt(iLevel); maxCount = zone[i].count }
                                        if (zone[i].coords)
                                ...
                                var lowestLevel = 999; for (var i in opt.coords) {
                                        i = parseInt(i); var iLevel = this.fixLevel(i); this.setCoords(opt.coords[i], iLevel); if (iLevel < lowestLevel)
                                                lowestLevel = iLevel
                                }
                        ...
                        this.setLink(opt.link); this.updateMap(noScroll)
                ...
                if (Mapper.zoneLevelOffset[this.zone] !== undefined)
                        ...
                        level = Mapper.remappedLevels[this.zone][level]; return level
        }, getZone: function () { return this.zone }, setZone: function (zone, level, noScroll) {
                this.pins = []; this.nCoords = 0; if (this.floorPins[this.level])
                        $WH.de(this.floorPins[this.level]); this.floorPins = {}; if (this.floorButton) { $WH.de(this.floorButton); this.floorButton = null }
                this.zone = zone; this.level = level | 0; this.updateMap(noScroll); return !0
        }, showFloors: function (event) {
                if (!Mapper.multiLevelZones[this.zone])
                        return; var menu = []; var _ = Mapper.multiLevelZones[this.zone]; var src = g_zone_areas; for (var i = 0; i < _.length; ++i) {
                                var menuItem; if (!src[this.zone])
                ...
                var type = Locale.getName(); if ($WH.isset('g_ptr') && g_ptr)
                        ...
                        this.overlaySpan.style.background = 'url(' + g_staticUrl + '/images/wow/maps/overlay/' + Mapper.sizes[this.zoom][2] + '/' + map + '.png)'; if (this.sZoneLink) {
                                var zoneText = ''; var zoneId = parseInt(this.zone); var found = g_zones[zoneId] != null; var src = g_zone_areas; if (found) {
                                        if (this.zoneLink)
                                                zoneText += '<a href="?zone=' + zoneId + '">' + g_zones[zoneId] + '</a>'; if (Mapper.multiLevelZones[zoneId]) {
                                                        if (this.zoneLink)
                ...
                var startEnd = { start: 1, end: 1, startend: 1, sourcestart: 1, sourceend: 1 }; for (var z in obj) {
                        var zone = obj[z]; if (g_mapperData[z] === undefined)
                                g_mapperData[z] = {}; var objectiveIndex = {}; var nextIndex = 0; for (var l in zone.levels) { var level = zone.levels[l]; var results = ShowOnMap.combinePins(level); var pins = results[0]; g_mapperData[z][l] = { count: pins.length, coords: [] }; for (var i = 0; i < pins.length; ++i) { var tooltip = ShowOnMap.buildTooltip(pins[i].list); g_mapperData[z][l].coords.push([pins[i].coord[0], pins[i].coord[1], { type: tooltip[1], url: tooltip[2], menu: tooltip[3], label: tooltip[0] }]) } }
                }
        }, setZones: function (div, zones) {
                div = $WH.ge(div); if (!div || !zones || zones.length == 0 || !this.objectives)
                        return; var zoneLinks = function (self, container, zoneList, zoneTypes) {
                                var maxIdx = [!1, -1]; for (var i = 0; i < zoneList.length; ++i) {
                                        if (i > 0) $WH.ae(span, $WH.ct(i == zoneList.length - 1 ? LANG.and : LANG.comma)); var entry = null; if (self.objectives[zoneList[i][0]].mappable > 0) { entry = $WH.ce('a'); entry.href = 'javascript:;'; $WH.ae(entry, $WH.ct(self.objectives[zoneList[i][0]].zone)); entry.onclick = function (link, zone) { self.update({ zone: zone }); g_setSelectedLink(link, 'mapper') }.bind(self, entry, zoneList[i][0]); entry.isLink = !0 } else { entry = $WH.ce('a'); entry.href = '?zone=' + zoneList[i][0]; $WH.ae(entry, $WH.ct(self.objectives[zoneList[i][0]].zone)); g_addTooltip(entry, LANG.tooltip_zonelink) }
                                ...
                                var ret = []; for (var i = 0; i < zoneList.length; ++i) {
                                        if (zoneTypes[zoneList[i][0]][type])
                                                ret.push(zoneList[i])
                                }
                                return ret
                        }; var typesByZone = {}; var types = { start: [], end: [], objective: [] }; for (var zoneId in this.objectives) {
                ...
                var h3 = $WH.ce('h3'); $WH.ae(h3, $WH.ct(LANG.mapper_relevantlocs)); $WH.ae(div, h3); if (zones.length == 1 && this.missing == 0) { var span = $WH.ce('span'); span.innerHTML = LANG.mapper_entiretyinzone.replace('$$', '<b>' + this.objectives[zones[0][0]].zone + '</b>.'); $WH.ae(div, span); this.update({ zone: zones[0][0] }) } else if (this.missing > 0) {
                        ...
                        var parts = LANG.mapper_happensin.split('$$'); var span = $WH.ce('span'); $WH.ae(span, $WH.ct(parts[0])); var primaryLink = zoneLinks(this, span, zones, typesByZone); $WH.ae(span, parts[1]); $WH.ae(div, span); if (primaryLink && primaryLink.isLink)
                                primaryLink.click()
                }
        }, setSize: function (w, h) { this.tempWidth = w; this.tempHeight = h; this.updateMap(!0) }, getZoom: function () { return this.zoom }, setZoom: function (zoom, noScroll) { this.zoom = zoom; this.tempWidth = this.tempHeight = null; this.updateMap(noScroll) }, toggleZoom: function (e) {
                this.zoom = 1 - this.zoom; this.updateMap(!0); if (e)
                        this.getMousePos(e); if (this.sZoom) {
        ...
        }, getShow: function () { return this.show }, setShow: function (show) {
                this.show = show; var d = this.show ? '' : 'none'; for (var i in this.floorPins)
                        this.floorPins[i].style.display = d; RedButton.setText(this.sToggle, (this.show ? LANG.mapper_hidepins : LANG.mapper_showpins))
        }, toggleShow: function () { this.setShow(!this.show) }, getCoords: function () {
                var a = []; for (var i in this.pins)
                        if (!this.pins[i].free)
                                a.push([this.pins[i].x, this.pins[i].y]); return a
        }, clearPins: function () { for (var i in this.pins) { this.pins[i].style.display = 'none'; this.pins[i].free = !0 } }, setCoords: function (coords, coordsLevel) {
                var _; var level, noLevel; if (coordsLevel === undefined) {
                        this.clearPins(); if (coords.length) { noLevel = !0; level = 0 } else {
                                for (var i in coords) { level = i; break }
                                if (level == null) return; coords = coords[level]
                        }
                        level = parseInt(level); if (!noLevel) { level = this.fixLevel(level); this.level = level }
                } else { level = coordsLevel }
                this.nCoords = coords.length; for (var i in coords) {
                        var
                        ...
                        if (opt && opt.tooltip) {
                                _.a.tt = ''; var printedFooter = !1; for (var name in opt.tooltip) {
                                        if (_.a.tt != '')
                                                _.a.tt += '<br />'; _.a.tt += '<b class="q">' + name + '</b> ($)<br />'; for (var line in opt.tooltip[name].info)
                                                _.a.tt += '<div>' + opt.tooltip[name].info[line] + '</div>'; if (!printedFooter && opt.tooltip[name].footer) { _.a.tt += opt.tooltip[name].footer + '<br />'; printedFooter = !0 }
                                }
                        } else if (opt && opt.label)
                                _.a.tt = opt.label; else _.a.tt = '$'; if (opt && opt.menu) { _.a.menu = opt.menu; Menu.add(_.a, _.a.menu, { showAtCursor: !0 }) }
                        if (opt && opt.type)
                                _.className += ' pin-' + opt.type; _.a.tt = $WH.str_replace(_.a.tt, '$', _.x.toFixed(1) + ', ' + _.y.toFixed(1)); if (opt && opt.lines) {
                ...
                var s = ''; for (var i in this.pins)
                        if (!this.pins[i].free && this.pins[i].floor == this.level)
                                s += (this.pins[i].x < 10 ? '0' : '') + (this.pins[i].x < 1 ? '0' : '') + (this.pins[i].x * 10).toFixed(0) + (this.pins[i].y < 10 ? '0' : '') + (this.pins[i].y < 1 ? '0' : '') + (this.pins[i].y * 10).toFixed(0); return (this.zone ? this.zone : '') + (Mapper.multiLevelZones[this.zone] && this.level != 0 ? '.' + this.level : '') + (s ? ':' + s : '')
        }, setLink: function (link, noScroll) {
                var a = []; link = link.split(':'); var zone = link[0]; var level = 0; if (zone.indexOf('.') != -1) { var zoneLevel = zone.split('.'); zone = zoneLevel[0]; level = parseInt(zoneLevel[1]) }
                ...
                this.setCoords(a, level); return !0
        }, updateMap: function (noScroll) {
                this.parent.style.width = this.span.style.width = (this.tempWidth ? this.tempWidth : Mapper.sizes[this.zoom][0]) + 'px'; this.parent.style.height = this.span.style.height = (this.tempHeight ? this.tempHeight : Mapper.sizes[this.zoom][1]) + 'px'; if (!this.editable)
                        this.parent.style.cssFloat = this.parent.style.styleFloat = 'left'; if (this.zone == '0')
                        this.span.style.background = 'black'; else {
                                var level = this.level; if (level == 1 && Mapper.onlyOneFloor[this.zone])
                ...
                if (this.sToggle)
                        ...
                        $WH.g_scrollTo(this.parent, 3); if (this.floorPins[level]) {
                                var lines = this.floorPins[level].getElementsByClassName('line'); for (i in lines) { if (i < lines.length) { lines[i].style.display = 'none' } }
                ...
                this.onMapUpdate && this.onMapUpdate(this)
        }, cleanPin: function (i, floor) { var _ = this.pins[i]; _.style.display = ''; _.free = !1; _.className = 'pin'; _.a.onmousedown = $WH.rf; _.a.onmouseup = $WH.rf; _.a.href = 'javascript:;'; _.a.style.cursor = 'default'; _.floor = floor; return _ }, getPin: function (floor) {
                for (var i = 0; i < this.pins.length; ++i)
                        if (this.pins[i].free)
                                return this.cleanPin(i, floor); var _ = $WH.ce('div'), a = $WH.ce('a'); _.className = 'pin'; _.appendChild(a); _.a = a; _.floor = floor; a.onmouseover = this.pinOver; a.onmouseout = $WH.Tooltip.hide; a.onclick = $WH.sp; this.pins.push(_); this.cleanPin(this.pins.length - 1, floor); if (!this.floorPins[floor]) {
                                        this.floorPins[floor] = $WH.ce('div'); this.floorPins[floor].style.display = this.show ? '' : 'none'; if (floor == this.level)
                                                $WH.ae(this.span, this.floorPins[floor])
                                }
                $WH.ae(this.floorPins[floor], _); return _
        }, addPin: function (e) { e = $WH.$E(e); if (e._button >= 2) return; this.getMousePos(e); var p = this.getPin(this.level); p.x = this.mouseX; p.y = this.mouseY; p.style.left = p.x.toFixed(1) + '%'; p.style.top = p.y.toFixed(1) + '%'; p.a.onmouseup = this.delPin.bind(this, p); p.a.tt = p.x.toFixed(1) + ', ' + p.y.toFixed(1); this.onPinUpdate && this.onPinUpdate(this); return !1 }, delPin: function (pin, e) { e = $WH.$E(e); pin.style.display = 'none'; pin.free = !0; $WH.sp(e); this.onPinUpdate && this.onPinUpdate(this); return }, pinOver: function () { $WH.Tooltip.show(this, this.tt, 4, 0) }, getMousePos: function (e) {
                e = $WH.$E(e); var c = $WH.ac(this.parent); var scroll = $WH.g_getScroll(); this.mouseX = Math.floor((e.clientX + scroll.x - c[0] - 3) / Mapper.sizes[this.zoom][0] * 1000) / 10; this.mouseY = Math.floor((e.clientY + scroll.y - c[1] - 3) / Mapper.sizes[this.zoom][1] * 1000) / 10; if (this.mouseX < 0) this.mouseX = 0; else if (this.mouseX > 100) this.mouseX = 100; if (this.mouseY < 0) this.mouseY = 0; else if (this.mouseY > 100) this.mouseY = 100; if (this.mouse)
                        g_setTextNodes(this.sMouse, '(' + this.mouseX.toFixed(1) + ', ' + this.mouseY.toFixed(1) + ')')
        }
}; var MapViewer = new function () {
        var imgWidth, imgHeight, scale, desiredScale, mapper, oldOnClick, oldOnUpdate, oldParent, oldSibling, tempParent, placeholder, container, screen, imgDiv, aCover; function computeDimensions() {
                var availHeight = Math.max(50, Math.min(618, $WH.g_getWindowSize().h - 72)); desiredScale = 1; scale = 1; if (desiredScale > 1)
        ...
        function getPound(extra) {
                var extraBits = function (map, s) {
                        s += ':' + map.zone; if (map.level)
                                s += '.' + map.level; return s
                }; var buff = '#map'; if (tempParent)
                        buff += '=' + mapper.getLink(); else if (Mapper.zoneDefaultLevel[mapper.zone]) {
        ...
        function render(resizing) {
                if (resizing && (scale == desiredScale) && $WH.g_getWindowSize().h > container.offsetHeight)
                        return; container.style.visibility = 'hidden'; computeDimensions(0); if (!resizing) {
                                if (!placeholder) { placeholder = $WH.ce('div'); placeholder.style.height = '325px'; placeholder.style.padding = '3px'; placeholder.style.marginTop = '10px' }
        ...
        function onHide() {
                ...
                else mapper.onMapUpdate = null; oldOnUpdate = null; mapper.span.style.cursor = ''; mapper.span.onmouseover = null; mapper.span.onmouseout = null; if (!tempParent) {
                        $WH.de(placeholder); $WH.de(mapper.parent); mapper.parent.style.borderWidth = ''; mapper.parent.style.marginTop = ''; if (oldSibling)
                                oldParent.insertBefore(mapper.parent, oldSibling); else $WH.ae(oldParent, mapper.parent); oldParent = oldSibling = null
                } else { $WH.de(tempParent); tempParent = null }
        ...
        this.checkPound = function () {
                if (location.hash && location.hash.indexOf('#map') == 0) {
                        var parts = location.hash.split('='); if (parts.length == 2) { var link = parts[1]; if (link) { MapViewer.show({ link: link }) } } else {
                                parts = location.hash.split(':'); var map = $WH.ge('sjdhfkljawelis'); if (map)
                                        map.onclick(); if (parts.length == 2) {
                                                if (!map)
        ...
        function render() {
                var flashVars = { model: model, modelType: modelType, contentPath: g_staticUrl + '/modelviewer/' }; var params = { quality: 'high', allowscriptaccess: 'always', allowfullscreen: !0, menu: !1, bgcolor: '#181818', wmode: 'direct' }; var attributes = {}; if (modelType == 16 && equipList.length) { flashVars.equipList = equipList.join(',') }
                swfobject.embedSWF(g_staticUrl + '/modelviewer/ZAMviewerfp11.swf', 'modelviewer-generic', '600', '400', "11.0.0", g_staticUrl + '/modelviewer/expressInstall.swf', flashVars, params, attributes); var foo = getRaceSex(), race = foo.r, sex = foo.s; if (!optBak.noPound) {
                        var url = '#modelviewer'; var foo = $WH.ge('view3D-button'); if (!foo) { switch (optBak.type) { case 1: url += ':1:' + optBak.displayId + ':' + (optBak.humanoid | 0); break; case 2: url += ':2:' + optBak.displayId; break; case 3: url += ':3:' + optBak.displayId + ':' + (optBak.slot | 0); break; case 4: url += ':4:' + equipList.join(';'); break } }
        ...
        function onSelChange() {
                var
                foo = getRaceSex(), race = foo.r, sex = foo.s; if (!race) {
                        if (!sexSel.is(':visible'))
                                return; sexSel.hide(); model = equipList[1]; switch (optBak.slot) { case 1: modelType = 2; break; case 3: modelType = 4; break; default: modelType = 1 }
                } else {
                        if (!sexSel.is(':visible'))
        ...
        function onAnimationChange() {
                var viewer = $('#modelviewer-generic'); if (viewer.length == 0)
        ...
        function onAnimationMouseover() {
                ...
                var anims = {}; var numAnims = viewer.getNumAnimations(); for (var i = 0; i < numAnims; ++i) {
                        var a = viewer.getAnimation(i); if (a && a != 'EmoteUseStanding')
                                anims[a] = 1
                }
        ...
        function initRaceSex(allowNoRace, opt) {
                ...
                if (allowNoRace) {
                        sel = raceSel1; offset = 1; raceSel1.show(); raceSel1[0].selectedIndex = -1; raceSel2.hide(); if (sex == -1)
                                sexSel.hide()
                } else {
                        if (race == -1 && sex == -1) {
                                var
        ...
        function onHide() {
                if (!optBak.noPound) { if (!optBak.fromTag && oldHash && oldHash.indexOf('modelviewer') == -1) { location.replace(oldHash) } else { location.replace('#.') } }
        ...
        function onShow(dest, first, opt) {
                ...
                switch (opt.type) {
                        case 1: modelDiv.hide(); if (opt.humanoid) { modelType = 32 } else { modelType = 8 }
                                model = opt.displayId; break; case 2: modelDiv.hide(); modelType = 64; model = opt.displayId; break; case 3: equipList = [opt.slot, opt.displayId]; if ($WH.in_array([4, 5, 6, 7, 8, 9, 10, 16, 19, 20], opt.slot) != -1) { initRaceSex(0, opt) } else {
                                        switch (opt.slot) { case 1: modelType = 2; break; case 3: modelType = 4; break; default: modelType = 1 }
        ...
        this.checkPound = function () {
                if (location.hash && location.hash.indexOf('#modelviewer') == 0) {
                        var parts = location.hash.split(':'); if (parts.length >= 3) {
                                parts.shift(); var type = parseInt(parts.shift()); var opt = { type: type }; switch (type) {
                                        case 1: opt.displayId = parseInt(parts.shift()); var humanoid = parseInt(parts.shift()); if (humanoid == 1) { opt.humanoid = 1 }
                                ...
                                if (readExtraPound != null) { if (parts.length > 0 && parts[parts.length - 1]) { readExtraPound(parts[parts.length - 1]) } }
                        } else if (readExtraPound != null && parts.length == 2 && parts[1]) { readExtraPound(parts[1]) } else { var foo = $WH.ge('view3D-button'); if (foo) { foo.onclick() } }
                }
        }; this.addExtraPound = function (func) { readExtraPound = func }; this.show = function (opt) { optBak = opt; Lightbox.show('modelviewer', { onShow: onShow, onHide: onHide }, opt) }; DomContentLoaded.addEvent(this.checkPound)
...
$(document).ready(function () { g_setupChangeWarning($("form[name=addcomment]"), [$("textarea[name=commentbody]")], LANG.message_startedpost) }); var ContactTool = new function () {
        ...
        }; this.checkPound = function () { if (location.hash && location.hash == '#contact') { ContactTool.show() } }; var dialog_contacttitle = LANG.ct_dialog_contactwowhead; this.init = function () {
                _dialog = new Dialog(); Dialog.templates.contactus = {
                        title: dialog_contacttitle, width: 550, buttons: [['okay', LANG.ok], ['cancel', LANG.cancel]], fields: [{
                                id: 'reason', type: 'select', label: LANG.ct_dialog_reason, required: 1, options: [], compute: function (field, value, form, td) {
                                        $WH.ee(field); for (var i = 0; i < this.data.reasons.length; ++i) {
                                                var id = this.data.reasons[i][0]; var check = this.data.reasons[i][1]; var valid = !1; if (typeof check == 'function') { valid = check(this.extra) } else { valid = check }
                                                if (!valid) { continue }
                                                var o = $WH.ce('option'); o.value = id; if (value && value == id) { o.selected = !0 }
                                                $WH.ae(o, $WH.ct(g_contact_reasons[id])); $WH.ae(field, o)
                                        }
                                        field.onchange = function () { if (this.value == 1 || this.value == 2 || this.value == 3) { form.currenturl.parentNode.parentNode.style.display = ''; form.relatedurl.parentNode.parentNode.style.display = '' } else { form.currenturl.parentNode.parentNode.style.display = 'none'; form.relatedurl.parentNode.parentNode.style.display = 'none' } }.bind(field); td.style.width = '98%'
                                }, validate: function (newValue, data, form) {
                                        var error = ''; if (!newValue || newValue.length == 0) { error = LANG.ct_dialog_error_reason }
                                        if (error == '') { return !0 }
                                        ContactTool.displayError(form.reason, error); form.reason.focus(); return !1
                                }
                        }, { id: 'currenturl', type: 'text', disabled: !0, label: LANG.ct_dialog_currenturl, size: 40 }, {
                                id: 'relatedurl', type: 'text', label: LANG.ct_dialog_relatedurl, caption: LANG.ct_dialog_optional, size: 40, validate: function (newValue, data, form) {
                                        var error = ''; var urlRe = /^(http(s?)\:\/\/|\/)?([\w]+:\w+@)?([a-zA-Z]{1}([\w\-]+\.)+([\w]{2,5}))(:[\d]{1,5})?((\/?\w+\/)+|\/?)(\w+\.[\w]{3,4})?((\?\w+=\w+)?(&\w+=\w+)*)?/; newValue = newValue.trim(); if (newValue.length >= 250) { error = LANG.ct_dialog_error_relatedurl } else if (newValue.length > 0 && !urlRe.test(newValue)) { error = LANG.ct_dialog_error_invalidurl }
                                        if (error == '') { return !0 }
                                        ContactTool.displayError(form.relatedurl, error); form.relatedurl.focus(); return !1
                                }
                        }, {
                                id: 'email', type: 'text', label: LANG.ct_dialog_email, caption: LANG.ct_dialog_email_caption, compute: function (field, value, form, td, tr) { if (g_user.email) { this.data.email = g_user.email; tr.style.display = 'none' } else { var func = function () { $WH.ge('contact-emailwarn').style.display = g_isEmailValid($WH.ge(form.email).value) ? 'none' : ''; Lightbox.reveal() }; $WH.ge(field).onkeyup = func; $WH.ge(field).onblur = func } }, validate: function (newValue, data, form) {
                                        var error = ''; newValue = newValue.trim(); if (newValue.length >= 100) { error = LANG.ct_dialog_error_emaillen } else if (newValue.length > 0 && !g_isEmailValid(newValue)) { error = LANG.ct_dialog_error_email }
                                        if (error == '') { return !0 }
                                        ContactTool.displayError(form.email, error); form.email.focus(); return !1
                                }
                        }, {
                                id: 'description', type: 'textarea', caption: LANG.ct_dialog_desc_caption, width: '98%', required: 1, size: [10, 30], validate: function (newValue, data, form) {
                                        var error = ''; newValue = newValue.trim(); if (newValue.length == 0 || newValue.length > 10000) { error = LANG.ct_dialog_error_desc }
                                        if (error == '') { return !0 }
                                        ContactTool.displayError(form.description, error); form.description.focus(); return !1
                                }
                        }, { id: 'noemailwarning', type: 'caption', compute: function (field, value, form, td) { var td = $WH.ge(td); td.innerHTML = '<span id="contact-emailwarn" class="q10"' + (g_user.email ? ' style="display: none"' : '') + '>' + LANG.ct_dialog_noemailwarning + '</span>'; td.style.whiteSpace = 'normal'; td.style.padding = '0 4px' } }], onInit: function (form) { }, onShow: function (form) {
                                if (this.data.focus && form[this.data.focus]) { setTimeout(g_setCaretPosition.bind(null, form[this.data.focus], form[this.data.focus].value.length), 100) } else if (form.reason && !form.reason.value) { setTimeout($WH.bindfunc(form.reason.focus, form.reason), 10) } else if (form.relatedurl && !form.relatedurl.value) { setTimeout($WH.bindfunc(form.relatedurl.focus, form.relatedurl), 10) } else if (form.email && !form.email.value) { setTimeout($WH.bindfunc(form.email.focus, form.email), 10) } else if (form.description && !form.description.value) { setTimeout($WH.bindfunc(form.description.focus, form.description), 10) }
                                setTimeout(Lightbox.reveal, 250)
                        }
                }; Dialog.templates.reportform = {
                        title: LANG.ct_dialog_report, width: 550, buttons: [['okay', LANG.ok], ['cancel', LANG.cancel]], fields: [{
                                id: 'reason', type: 'select', label: LANG.ct_dialog_reason, options: [], compute: function (field, value, form, td) {
                                        switch (this.data.mode) {
                                                case 1: form.firstChild.innerHTML = $WH.sprintf(LANG.ct_dialog_reportcomment, '<a href="?user=' + this.data.comment.user + '">' + this.data.comment.user + '</a>'); break; case 2: var rep = '<a href="?user=' + this.data.post.user + '">' + this.data.post.user + '</a>'; if (this.data.post.op) { form.firstChild.innerHTML = $WH.sprintf(LANG.ct_dialog_reporttopic, rep) } else { form.firstChild.innerHTML = $WH.sprintf(LANG.ct_dialog_reportpost, rep) }
                                        ...
                                        $WH.ae(field, $WH.ce('option', { selected: (!value), value: -1 })); for (var i = 0; i < this.data.reasons.length; ++i) {
                                                var id = this.data.reasons[i][0]; var check = this.data.reasons[i][1]; var valid = !1; if (typeof check == 'function') { valid = check(extra) } else { valid = check }
                                                if (!valid) { continue }
                                                var o = $WH.ce('option'); o.value = id; if (value && value == id) { o.selected = !0 }
                                                $WH.ae(o, $WH.ct(g_contact_reasons[id])); $WH.ae(field, o)
                                        }
                                        ...
                                        var error = ''; if (!newValue || newValue == -1 || newValue.length == 0) { error = LANG.ct_dialog_error_reason }
                                        if (error == '') { return !0 }
                                        ContactTool.displayError(form.reason, error); form.reason.focus(); return !1
                                }
                        }, {
                        ...
                        }], onInit: function (form) { }, onShow: function (form) { var reason = $WH.gE(form, 'select')[0]; var description = $WH.gE(form, 'textarea')[0]; if (this.data.focus && form[this.data.focus]) { setTimeout(g_setCaretPosition.bind(null, form[this.data.focus], form[this.data.focus].value.length), 100) } else if (!reason.value) { setTimeout($WH.bindfunc(reason.focus, reason), 10) } else if (!description.value) { setTimeout($WH.bindfunc(description.focus, description), 10) } }
                }
        }; DomContentLoaded.addEvent(this.checkPound)
}; function Line(x1, y1, x2, y2, type) {
        var left = Math.min(x1, x2), right = Math.max(x1, x2), top = Math.min(y1, y2), bottom = Math.max(y1, y2), width = (right - left), height = (bottom - top), length = Math.sqrt(Math.pow(width, 2) + Math.pow(height, 2)), radian = Math.atan2(height, width), sinTheta = Math.sin(radian), cosTheta = Math.cos(radian); var $line = $WH.ce('span'); $line.className = 'line'; $line.style.top = top.toFixed(2) + 'px'; $line.style.left = left.toFixed(2) + 'px'; $line.style.width = width.toFixed(2) + 'px'; $line.style.height = height.toFixed(2) + 'px'; var v = $WH.ce('var'); v.style.width = length.toFixed(2) + 'px'; v.style.OTransform = 'rotate(' + radian + 'rad)'; v.style.MozTransform = 'rotate(' + radian + 'rad)'; v.style.webkitTransform = 'rotate(' + radian + 'rad)'; v.style.filter = "progid:DXImageTransform.Microsoft.Matrix(sizingMethod='auto expand', M11=" + cosTheta + ', M12=' + (-1 * sinTheta) + ', M21=' + sinTheta + ', M22=' + cosTheta + ')'; $WH.ae($line, v); if (!(x1 == left && y1 == top) && !(x2 == left && y2 == top)) { $line.className += ' flipped' }
        if (type != null) { $line.className += ' line-' + type }
        return $line
}
var Links = new function () {
        var dialog = null; var oldHash = null; var validArmoryTypes = { item: 1 }; var extraTypes = { 29: 'icondb' }; this.onShow = function () {
                if (location.hash && location.hash != '#links') { oldHash = location.hash }
        ...
        this.show = function (opt) {
                if (!opt || !opt.type || !opt.typeId) { return }
        ...
        initialize: function () {
                this.parentDiv.style.opacity = '0'; if (this.mode === undefined || this.mode == 1)
                        this.parentDiv.className = 'announcement announcement-contenttop'; else this.parentDiv.className = 'announcement announcement-pagetop'; var div = this.innerDiv = $WH.ce('div'); div.className = 'announcement-inner text'; this.setStyle(this.style); var a = null; var id = parseInt(this.id); if (g_user && (g_user.roles & (U_GROUP_ADMIN | U_GROUP_BUREAU)) > 0 && Math.abs(id) > 0) {
                                if (id < 0) { a = $WH.ce('a'); a.style.cssFloat = a.style.styleFloat = 'right'; a.href = '?admin=announcements&id=' + Math.abs(id) + '&status=2'; a.onclick = function () { return confirm('Are you sure you want to delete ' + this.name + '?') }; $WH.ae(a, $WH.ct('Delete')); var small = $WH.ce('small'); $WH.ae(small, a); $WH.ae(div, small); a = $WH.ce('a'); a.style.cssFloat = a.style.styleFloat = 'right'; a.style.marginRight = '10px'; a.href = '?admin=announcements&id=' + Math.abs(id) + '&status=' + (this.status == 1 ? 0 : 1); a.onclick = function () { return confirm('Are you sure you want to delete ' + this.name + '?') }; $WH.ae(a, $WH.ct((this.status == 1 ? 'Disable' : 'Enable'))); var small = $WH.ce('small'); $WH.ae(small, a); $WH.ae(div, small) }
                ...
                var markupDiv = $WH.ce('div'); markupDiv.id = this.parent + '-markup'; $WH.ae(div, markupDiv); if (id >= 0) {
                        a = $WH.ce('a'); a.id = 'closeannouncement'; a.href = 'javascript:;'; a.className = 'announcement-close'; if (this.nocookie) { a.onclick = this.hide.bind(this) } else { a.onclick = this.markRead.bind(this) }
                        $WH.ae(div, a); g_addTooltip(a, LANG.close)
                }
                $WH.ae(div, $WH.ce('div', { style: { clear: 'both' } })); $WH.ae(this.parentDiv, div); this.setText(this.text); setTimeout(this.show.bind(this), 500)
...
}; var g_audiocontrols = { __windowloaded: !1, }; var g_audioplaylist = {}; if (!window.JSON) {
        window.JSON = {
                parse: function (sJSON) { return eval("(" + sJSON + ")") }, stringify: function (obj) {
                        if (obj instanceof Object) {
                                var str = ''; if (obj.constructor === Array) {
...
AudioControls = function () {
        var fileIdx = -1; var canPlay = !1; var looping = !1; var fullPlayer = !1; var autoStart = !1; var controls = {}; var playlist = []; var url = ''; function updatePlayer(_self, itr, doPlay) {
                var elAudio = $WH.ce('audio'); elAudio.preload = 'none'; elAudio.controls = 'true'; $(elAudio).click(function (s) { s.stopPropagation() }); elAudio.style.marginTop = '5px'; controls.audio.parentNode.replaceChild(elAudio, controls.audio); controls.audio = elAudio; $WH.aE(controls.audio, 'ended', setNextTrack.bind(_self)); if (doPlay) { elAudio.preload = 'auto'; autoStart = !0; $WH.aE(controls.audio, 'canplaythrough', autoplay.bind(this)) }
                if (!canPlay)
                        controls.table.style.visibility = 'visible'; var file; do {
                                fileIdx += itr; if (fileIdx > playlist.length - 1) { fileIdx = 0; if (!canPlay) { var div = $WH.ce('div'); div.className = 'minibox minibox-left'; $WH.st(div, $WH.sprintf(LANG.message_browsernoaudio, file.type)); controls.table.parentNode.replaceChild(div, controls.table); return } }
                                if (fileIdx < 0)
        ...
        this.init = function (files, parent, opt) {
                if (!$WH.is_array(files))
                        ...
                        return; if ((parent.id == '') || g_audiocontrols.hasOwnProperty(parent.id)) {
                                var i = 0; while (g_audiocontrols.hasOwnProperty('auto-audiocontrols-' + (++i))) { }
                ...
                if (g_audioplaylist.isEnabled() && !opt.fromplaylist) {
                        var addBtn = createButton(LANG.add); $WH.ae(controls.controlsdiv, addBtn); $WH.aE(addBtn, 'click', this.btnAddToPlaylist.bind(this, addBtn)); controls.addbutton = addBtn; if (fullPlayer)
                                addBtn.style.verticalAlign = '50%'
                }
                if (g_audiocontrols.__windowloaded)
                        this.btnNextTrack()
        }; function setNextTrack() { updatePlayer(this, 1, (looping || (fileIdx < (playlist.length - 1)))) }
        this.btnNextTrack = function () { updatePlayer(this, 1, (canPlay && (controls.audio.readyState > 1) && (!controls.audio.paused))) }; this.btnPrevTrack = function () { updatePlayer(this, -1, (canPlay && (controls.audio.readyState > 1) && (!controls.audio.paused))) }; this.btnAddToPlaylist = function (_self) {
                if (fullPlayer) {
                        for (var i = 0; i < playlist.length; i++)
                                g_audioplaylist.addSound(playlist[i]);
                } else g_audioplaylist.addSound(playlist[fileIdx]); _self.className += ' button-red-disabled'; RedButton.setText(_self, LANG.added)
        }; this.isPlaying = function () { return !controls.audio.paused }; this.removeSelf = function () { controls.table.parentNode.removeChild(controls.table); delete g_audiocontrols[controls.div] }; function createButton(text, disabled) { return $WH.g_createButton(text, null, { disabled: disabled, style: 'margin:0 12px; display:inline-block; float:inherit; ' }) }
}; $WH.aE(window, 'load', function () {
        g_audiocontrols.__windowloaded = !0; for (var i in g_audiocontrols)
                if (i.substr(0, 2) != '__')
                        g_audiocontrols[i].btnNextTrack();
}); AudioPlaylist = function () {
        var enabled = !1; var playlist = []; var player, container; this.init = function () {
                ...
                if (!enabled)
                        return !1; $WH.localStorage.set('AudioPlaylist', JSON.stringify(playlist))
        }; this.isEnabled = function () { return enabled }; this.addSound = function (track) {
                if (!enabled)
                        return !1; this.init(); playlist.push(track); this.savePlaylist()
                ...
                var buf = []; for (var i = 0; i < playlist.length; i++)
                        buf.push(playlist[i].title); return buf
        }; this.setAudioControls = function (parent) {
                if (!enabled)
                        return !1; container = parent; player = new AudioControls(); player.init(playlist, container, { loop: !0, fromplaylist: !0 })
        }
}; g_audioplaylist = (new AudioPlaylist); g_audioplaylist.init(); $WH.aE(window, 'load', function () {
        if (!(window.JSON && $WH.localStorage.isSupported())) { return }
        ...
        path = path[0]; var makeButton = function (text, url) { var a = $WH.ce('a'); a.className = 'button-red' + (url ? ' button-red-disabled' : ''); var em = $WH.ce('em'); $WH.ae(a, em); var b = $WH.ce('b'); $WH.ae(em, b); var i = $WH.ce('i'); $WH.ae(b, i); $WH.st(i, text); var sp = $WH.ce('span'); $WH.ae(em, sp); $WH.st(sp, text); return a }; for (var i = 0; i < lv.length; i++) {
                var urls = lv[i].urls; for (var j = 0; j < urls.length; j++) {
                        if (urls[j] == path) {
                                var prevUrl = j > 0 ? urls[j - 1] : !1; var nextUrl = (j + 1) < urls.length ? urls[j + 1] : !1; var upUrl = lv[i].path + lv[i].hash; var el = $WH.ge('topbar-browse'); if (!el) { return }
                                ...
                                $WH.ae(el, button); return
                        }
                }
        }
}); function GetN5(num) {
        var absNum = Math.abs(num); if (absNum < 10000)
...
function g_createRange(min, max) {
        range = {}; for (var i = min; i <= max; ++i) { range[i] = i }
        return range
}
...
function g_getCurrentDomain() {
        if (g_getCurrentDomain.CACHE) { return g_getCurrentDomain.CACHE }
        var hostname = location.hostname; if (!g_isIpAddress(hostname)) {
                var parts = hostname.split('.'); if (parts.length > 2) { parts.splice(0, parts.length - 2) }
                hostname = parts.join('.')
        }
...
function g_createOrRegex(search, negativeGroup) {
        search = search.replace(/(\(|\)|\|\+|\*|\?|\$|\^)/g, '\\$1'); var parts = search.split(' '), strRegex = ''; for (var j = 0, len = parts.length; j < len; ++j) {
                if (j > 0) { strRegex += '|' }
                strRegex += parts[j]
        }
...
function g_modifyUrl(url, params, opt) {
        ...
        $.each(params, function (paramName, newValue) {
                var needle; var paramPrefix; var paramValue; var matches = url.match(new RegExp('(&|\\?)?' + paramName + '=?([^&]+)?')); if (matches != null) { needle = matches[0]; paramPrefix = matches[1]; paramValue = decodeURIComponent(matches[2]) }
                if (newValue == null) {
                        if (!needle) { return }
                        paramValue = null
                } else if (newValue.substr(0, 2) == '+=') {
                        if (paramValue && opt.onAppendCollision) { paramValue = opt.onAppendCollision(paramValue, newValue.substr(2), opt.menuUrl) } else if (!paramValue && opt.onAppendEmpty) { paramValue = opt.onAppendEmpty(newValue.substr(2), opt.menuUrl) } else {
                                if (!paramValue) { paramValue = '' }
                                paramValue += $.trim(newValue.substr(2))
                        }
                } else { paramValue = newValue }
                if (needle) {
                        var replacement = ''; if (paramPrefix) { replacement += paramPrefix }
...
var ConditionList = new function () {
        ...
        function _parseEntry(entry) {
                ...
                cnd = entry, str = '', param = ['', null]; strIdx = Math.abs(entry[0]); if (strIdx == 22 || strIdx == 23)
                        ...
                        return g_conditions[strIdx].replace(/\$([^\$:;]*):([^\$:;]*);/, '$' + (entry[0] > 0 ? 1 : 2)); switch (Math.abs(entry[0])) {
                                case 1: case 25: param[0] = '[spell=' + entry[1] + ']'; break; case 2: param[1] = entry[2]; case 3: param[0] = '[item=' + entry[1] + ']'; break; case 4: case 23: param[0] = '[zone=' + entry[1] + ']'; break; case 8: case 9: case 14: case 28: param[0] = '[quest=' + entry[1] + ']'; break; case 12: param[0] = '[event=' + entry[1] + ']'; break; case 17: param[0] = '[achievement=' + entry[1] + ']'; break; case 18: param[0] = '[title=' + entry[1] + ']'; break; case 6: param[0] = g_sides[entry[1]]; break; case 10: param[0] = g_drunk_states[entry[1]]; break; case 22: param[0] = g_zone_categories[entry[1]]; break; case 15: param[0] = _listing(entry[1], g_chr_classes, '[class=$1]'); break; case 16: param[0] = _listing(entry[1], g_chr_races, '[race=$1]'); break; case 7: param[0] = '[skill=' + entry[1] + ']'; if (entry[2])
                                        param[0] += $WH.sprintf(LANG.qty, entry[2]); break; case 5: var standings = {}; for (i in g_reputation_standings)
                                                standings[i * 1 + 1] = g_reputation_standings[i]; param[0] = '[faction=' + entry[1] + ']'; param[1] = _listing(entry[2], standings, '$2'); break; case 20: if (entry[1] == 0) { param[0] = LANG.male; param[1] = g_file_genders[0] } else if (entry[1] == 1) { param[0] = LANG.female; param[1] = g_file_genders[1] } else param[0] = g_npc_types[10]; break; case 29: param[0] = '[npc=' + entry[1] + ']'; param[1] = entry[2]; break; case 30: param[0] = '[object=' + entry[1] + ']'; param[1] = entry[2]; break; case 31: if (entry[2] && entry[1] == 3) { param[0] = '[npc=' + entry[2] + ']'; break } else if (entry[2] && entry[1] == 5) { param[0] = '[object=' + entry[2] + ']'; break } else entry[1] = (1 << entry[1]); case 32: param[0] = _listing(entry[1], g_world_object_types, '$2'); break; case 36: break; case 27: case 37: case 38: param[0] = entry[1]; param[1] = g_operators[entry[2]]; break; case 35: param[0] = entry[2]; param[1] = g_operators[entry[3]]; break; case 11: param[0] = entry[1]; param[1] = entry[2]; break; case 26: var pIndex = 0; while (entry[1]) {
                                                        if (entry[1] & (1 << pIndex)) {
                                                                if (param[0])
        ...
        function _createTab() {
                var
                nTabs = Object.keys(_conditions).length, nSubTabs = [], buff = ''; for (g in _conditions) {
                        nSubTabs[g] = Object.keys(_conditions[g]).length; if (nSubTabs[g] > 1)
                                nTabs += nSubTabs[g]
                }
                if (nTabs > 1)
                        buff += '[tabs name=conditionSource]'; for (g in _conditions) {
                                if (!g_condition_types[g])
                                        continue; var nSubTypes = Object.keys(_conditions[g]).length; for (h in _conditions[g]) {
                                                var
                                                srcGrp = h.split(':'); nGroups = Object.keys(_conditions[g][h]).length, curGroup = 1; if (nTabs > 1) {
                                                        if (nSubTabs[g] > 1) {
                                                                var sName = srcGrp[1]; if ($WH.in_array([18, 21, 23], g) != -1)
                                                                        sName = g_npcs[srcGrp[1]]['name_' + Locale.getName()]; buff += '[tab name="' + g_condition_types[g][0] + ' (' + sName + ')"]'
                                                        } else buff += '[tab name="' + g_condition_types[g][0] + '"]'
                                                }
        ...
        function _createCell() {
                var rows = []; for (g in _conditions) {
                        for (h in _conditions[g]) {
                                for (i in _conditions[g][h]) {
                                        var
                                        subGroup = [], group = _conditions[g][h][i], nEntries = Object.keys(_conditions[g][h][i]).length; for (j in group) {
                                                if (nEntries > 1)
                                                        subGroup.push(_parseEntry(group[j])); else rows.push(_parseEntry(group[j]))
                                        }
                                        if (subGroup.length) {
                                                var buff = ''; for (j in subGroup) {
