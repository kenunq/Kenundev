...
function tc_init() {
    ...
    var classes = ["1", "11", "5", "8", "3", "2", "4", "6", "9", "7"];
    _ = $WH.ge("tc-classes-inner");
    for (var i = 0, len = classes.length; i < len; ++i) {
        var classId = classes[i],
            icon = Icon.create(
                "class_" + g_file_classes[classId],
                1,
                null,
                "javascript:;"
            ),
...
function tc_onChange(tc, info, data) {
    ...
    if (info.classId != tc_classId) {
        ...
        if (tc_classId != -1) {
            ...
            _.className = tc_classIcons[tc_classId].className.replace(
                "iconmedium-gold-selected",
                ""
            );
        }
...
function tc_readPound() {
    if (location.hash) {
        if (location.hash.indexOf("-") != -1) {
            var pounds = location.hash.substr(1).split("-"),
                classs = pounds[0] || "",
                build = pounds[1] || "",
                classId = -1;
